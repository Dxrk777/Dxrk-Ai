...
func TestInjectSDDReference_InjectIntoEmptyFile(t *testing.T) {
	...
	if err := os.WriteFile(promptFile, []byte(""), 0o600); err != nil {
	...
	data, err := os.ReadFile(promptFile) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile: %v", err)
	}

	content := string(data)
	marker := "<!-- dxrk:custom-agent:my-skill -->"
...
func TestInjectSDDReference_ExistingContentPreserved(t *testing.T) {
	...
	if err := os.WriteFile(promptFile, []byte(existing), 0o600); err != nil {
		t.Fatalf("setup: %v", err)
	}
	...
	data, _ := os.ReadFile(promptFile) //nolint:gosec
	content := string(data)

	if !strings.Contains(content, "My System Prompt") {
		t.Errorf("existing content was not preserved;\ngot:\n%s", content)
	}
	if !strings.Contains(content, "<!-- dxrk:custom-agent:my-skill -->") {
		t.Errorf("marker not found in file;\ngot:\n%s", content)
	}
}
...
func TestInjectSDDReference_DuplicateInjection_MarkerReplacedNotDuplicated(t *testing.T) {
	...
	marker := "<!-- dxrk:custom-agent:dedup-skill -->"
