...
// Example: <!-- dxrk:custom-agent:my-skill -->
const markerFormat = "<!-- dxrk:custom-agent:%s -->"
...
func InjectSDDReference(agent *GeneratedAgent, systemPromptPath string) error {
	...
	data, err := os.ReadFile(systemPromptPath) //nolint:gosec
	if err != nil {
	...
	if err := os.WriteFile(systemPromptPath, []byte(content), 0o600); err != nil { //nolint:gosec
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /dxrk:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /dxrk:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- dxrk:custom-agent:my-skill -->" → "my-skill"
