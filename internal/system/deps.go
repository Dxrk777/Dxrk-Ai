...
func defineDependencies(profile PlatformProfile) []Dependency {
	deps := []Dependency{
		{
			Name:        "git",
			Required:    true,
			DetectCmd:   []string{"git", versionFlag},
			InstallHint: installHintGit(profile),
		},
		{
			Name:        "curl",
			Required:    true,
			DetectCmd:   []string{"curl", versionFlag},
		...
		},
		{
			...
			Required:    true,
			MinVersion:  "18.0.0",
			DetectCmd:   []string{"node", versionFlag},
			InstallHint: installHintNode(profile),
		},
		{
			Name:        "npm",
			Required:    true,
			DetectCmd:   []string{"npm", versionFlag},
	...
	if profile.OS == osDarwin {
		deps = append(deps, Dependency{
			Name:        "brew",
			Required:    false,
			DetectCmd:   []string{"brew", versionFlag},
			InstallHint: installHintBrew(),
		})
	}

	// go is optional (needed for DxrkMemory on Linux via go install).
	deps = append(deps, Dependency{
		Name:        "go",
		Required:    false,
...
func detectSingleDep(ctx context.Context, dep Dependency) Dependency {
	...
	cmd := exec.CommandContext(ctx, dep.DetectCmd[0], dep.DetectCmd[1:]...) //nolint:gosec
	out, err := cmd.Output()
	if err != nil {
