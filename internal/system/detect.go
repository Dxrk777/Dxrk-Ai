...
func IsSupportedOS(goos string) bool {
	return goos == osDarwin || goos == osLinux || goos == osWindows
}
...
func resolvePlatformProfile(goos, linuxOSRelease string, tools map[string]ToolStatus) PlatformProfile {
	...
	switch goos {
	case osDarwin:
		profile.PackageManager = pkgBrew
	...
	case osLinux:
		...
		if brew, ok := tools[pkgBrew]; ok && brew.Installed {
			profile.PackageManager = pkgBrew
			profile.Supported = true
			return profile
		}
		...
		case LinuxDistroUbuntu, LinuxDistroDebian:
			profile.PackageManager = pkgApt
			profile.Supported = true
		case LinuxDistroArch:
			profile.PackageManager = pkgPacman
			profile.Supported = true
		case LinuxDistroFedora:
			profile.PackageManager = pkgDnf
			profile.Supported = true
		default:
		...
		return profile
	case osWindows:
		profile.PackageManager = pkgWinget
		profile.Supported = true
		return profile
	default:
		profile.Supported = false
