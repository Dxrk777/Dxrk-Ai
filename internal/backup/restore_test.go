...
func TestRestoreRestoresExistingAndRemovesCreated(t *testing.T) {
	...
	restored, err := os.ReadFile(originalPath) //nolint:gosec
	if err != nil {
...
func TestRestoreCompressedBackup(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(srcFile), 0o750); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	...
	snapshotter := Snapshotter{now: time.Now}
	...
	restored, err := os.ReadFile(srcFile) //nolint:gosec
	if err != nil {
...
func TestRestoreUncompressedBackup(t *testing.T) {
	...
	got, err := os.ReadFile(originalPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() after restore error = %v", err)
	}
...
func TestRestoreCompressedMultipleFiles(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(fileA), 0o750); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	...
	gotA, err := os.ReadFile(fileA) //nolint:gosec
	if err != nil {
	...
	gotB, err := os.ReadFile(fileB) //nolint:gosec
	if err != nil {
