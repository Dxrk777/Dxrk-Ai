...
func TestCreateArchive_CreatesValidTarGz(t *testing.T) {
	...
	if err := os.WriteFile(srcA, []byte("hello"), 0o600); err != nil {
		t.Fatalf("WriteFile a.txt: %v", err)
	}
	if err := os.WriteFile(srcB, []byte("world"), 0o600); err != nil {
		t.Fatalf("WriteFile b.txt: %v", err)
	}

	archivePath := filepath.Join(dir, "snapshot.tar.gz")
	entries := []ArchiveEntry{
		{RelPath: "files/a.txt", SourcePath: srcA, Mode: 0o600},
		{RelPath: "files/b.txt", SourcePath: srcB, Mode: 0o600},
	}
...
func TestCreateArchive_PreservesPermissions(t *testing.T) {
	...
	archivePath := filepath.Join(dir, "snapshot.tar.gz")
	entries := []ArchiveEntry{
		{RelPath: "files/exec.sh", SourcePath: src, Mode: 0o750},
	}
...
func TestExtractArchive_RoundTrip(t *testing.T) {
	...
	if err := os.WriteFile(srcA, []byte("hello world"), 0o600); err != nil {
		t.Fatalf("WriteFile hello.txt: %v", err)
	}
	if err := os.WriteFile(srcB, []byte(`{"key":"value"}`), 0o600); err != nil {
		t.Fatalf("WriteFile data.json: %v", err)
	}

	archivePath := filepath.Join(dir, "snapshot.tar.gz")
	entries := []ArchiveEntry{
		{RelPath: "files/hello.txt", SourcePath: srcA, Mode: 0o600},
		{RelPath: "files/data.json", SourcePath: srcB, Mode: 0o600},
	}
...
func TestExtractArchive_HandlesTypeRegA(t *testing.T) {
	...
	func() {
		...
		hdr := &tar.Header{
			Typeflag: tar.TypeReg, // legacy '\x00'
