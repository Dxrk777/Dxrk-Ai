...
func TestComputeChecksum_OrderIndependent(t *testing.T) {
	...
	if err := os.WriteFile(file1, []byte("zzz"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	if err := os.WriteFile(file2, []byte("aaa"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func makeTestBackup(t *testing.T, backupDir string, m Manifest) {
	t.Helper()
...
func makeBackupAt(t *testing.T, backupDir, id string, createdAt time.Time, pinned bool) {
	t.Helper()
