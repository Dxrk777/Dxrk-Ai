...
func TestPiOnlyAgentContinueSkipsPromptsAndIncludesEngram(t *testing.T) {
	...
	m.Selection.Components = componentsForPreset(model.PresetFullDxrk)
	...
	wantComponents := []model.ComponentID{model.ComponentDxrkMemory}
...
func TestBuildProgressLabelsFromResolvedPlan(t *testing.T) {
	resolved := planner.ResolvedPlan{
		...
		OrderedComponents: []model.ComponentID{model.ComponentDxrkMemory, model.ComponentSDD},
	}
...
func TestShouldShowSDDModeScreen(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			components: []model.ComponentID{model.ComponentDxrkMemory, model.ComponentSDD},
			want:       true,
		},
		{
...
func TestPresetFlowShowsClaudeModelPickerBeforeDependencyTree(t *testing.T) {
	...
	m.Selection.Agents = []model.AgentID{model.AgentClaudeCode}
	m.Selection.Components = []model.ComponentID{model.ComponentDxrkMemory, model.ComponentSDD}
	m.Cursor = 0
...
func TestStandaloneOpenCodePluginsContinueRegistersSelectedPlugins(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(home, ".config", "opencode", "tui.json")) //nolint:gosec
	if err != nil {
...
func TestStartUninstall_FullRemoveHomebrewManagedBinaryAddsManualAction(t *testing.T) {
	...
	restoreExec := setOSExecutableForTest("/opt/homebrew/bin/dxrk", nil)
	defer restoreExec()
	...
	if !strings.Contains(msg.Result.ManualActions[0], "brew uninstall dxrk") {
...
func TestStartUninstall_FullRemoveNonBrewRemovesBinary(t *testing.T) {
	...
	restoreExec := setOSExecutableForTest("/tmp/dxrk", nil)
	defer restoreExec()
	...
	if removedPath != "/tmp/dxrk" {
		t.Fatalf("os.Remove path = %q, want %q", removedPath, "/tmp/dxrk")
	}
}
...
func TestStartUninstall_UsesProfileAwareUninstallWhenConfigured(t *testing.T) {
	...
	called := false
	m.UninstallWithProfilesFn = func(agentIDs []model.AgentID, componentIDs []model.ComponentID, profileNames []string, dxrkMemoryScope model.DxrkMemoryUninstallScope) (componentuninstall.Result, error) {
		called = true
		if !reflect.DeepEqual(profileNames, []string{"cheap"}) {
			t.Fatalf("profileNames = %v, want [cheap]", profileNames)
		}
		if dxrkMemoryScope != model.DxrkMemoryUninstallScopeGlobal {
			t.Fatalf("dxrkMemoryScope = %q, want %q", dxrkMemoryScope, model.DxrkMemoryUninstallScopeGlobal)
		}
		return componentuninstall.Result{}, nil
	}
	m.UninstallFn = func(agentIDs []model.AgentID, componentIDs []model.ComponentID) (componentuninstall.Result, error) {
...
func TestUninstallComponents_ContinueWithEngramProjectScopeNavigatesToSubSelection(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(tempWorkspace, ".dxrk-memory"), 0o750); err != nil {
		t.Fatalf("MkdirAll(.dxrk-memory) error = %v", err)
	}
	...
	m.UninstallMode = model.UninstallModePartial
	m.UninstallAgents = []model.AgentID{model.AgentOpenCode}
	m.UninstallComponents = []model.ComponentID{model.ComponentDxrkMemory}
	m.Cursor = len(screens.UninstallComponentOptions())
	...
	if !state.UninstallDxrkMemoryProjectScopeAvailable {
		t.Fatal("UninstallDxrkMemoryProjectScopeAvailable = false, want true")
	}
	if state.UninstallDxrkMemoryScope != model.DxrkMemoryUninstallScopeGlobal {
		t.Fatalf("UninstallDxrkMemoryScope = %q, want %q", state.UninstallDxrkMemoryScope, model.DxrkMemoryUninstallScopeGlobal)
	}
}
...
func makeDetectionWithAgents(present ...string) system.DetectionResult {
	...
	configs := make([]system.ConfigState, 0, len(known))
...
func TestStrictTDDScreenSkippedWhenNoSDD(t *testing.T) {
	...
	m.Selection.Components = []model.ComponentID{model.ComponentDxrkMemory}
	m.Cursor = sddSingleCursor(t)
...
func TestModelConfigOpenCodePrePopulatesAssignments(t *testing.T) {
	...
	got := state.Selection.ModelAssignments["dxrk-orchestrator"]
	want := preExisting["dxrk-orchestrator"]
	if got != want {
		t.Errorf("dxrk-orchestrator assignment = %+v, want %+v", got, want)
	}
...
func TestModelConfigOpenCodeDoesNotOverwriteExistingSessionAssignments(t *testing.T) {
	...
	m.Selection.ModelAssignments = map[string]model.ModelAssignment{
		"dxrk-orchestrator": sessionAssignment,
	}
