...
import (
	...
	"github.com/Dxrk777/Dxrk-Ai/internal/components/opencodeplugin"
...
var readCurrentAssignmentsFn = sdd.ReadCurrentModelAssignments
...
var readProfilesFn = sdd.DetectProfiles
...
type UninstallWithProfilesFunc func(agentIDs []model.AgentID, componentIDs []model.ComponentID, profileNames []string, dxrkMemoryScope model.DxrkMemoryUninstallScope) (componentuninstall.Result, error)
...
type Model struct {
	...
	// UninstallDxrkMemoryProjectScopeAvailable indicates whether .dxrk-memory project data
	// was detected for the current workspace, enabling project-only cleanup.
	UninstallDxrkMemoryProjectScopeAvailable bool
	// UninstallDxrkMemoryScope controls Dxrk Memory cleanup behavior in uninstall.
	UninstallDxrkMemoryScope model.DxrkMemoryUninstallScope
...
func (m Model) View() string {
	...
	case ScreenUninstallProfiles:
		return screens.RenderUninstallProfiles(m.UninstallProfilesAvailable, m.UninstallProfilesToRemove, m.UninstallDxrkMemoryProjectScopeAvailable, m.UninstallDxrkMemoryScope, m.Cursor)
	case ScreenUninstallConfirm:
		return screens.RenderUninstallConfirm(m.UninstallMode, m.UninstallAgents, m.UninstallComponents, m.UninstallProfilesToRemove, m.UninstallDxrkMemoryScope, m.UninstallDxrkMemoryProjectScopeAvailable, m.Cursor, m.OperationRunning, m.SpinnerFrame)
	case ScreenUninstallResult:
		return screens.RenderUninstallResult(m.UninstallResult, m.UninstallErr, m.UninstallMode, m.UninstallProfilesToRemove, m.UninstallDxrkMemoryScope, m.UninstallDxrkMemoryProjectScopeAvailable, m.SyncCleanInstallFilesChanged, m.SyncCleanInstallErr)
	...
	case ScreenComplete:
		return screens.RenderComplete(screens.CompletePayload{
			...
			GGAInstalled:        hasSelectedComponent(m.Selection.Components, model.ComponentDxrkGuardian),
	...
	if m.Screen == ScreenClaudeModelPicker {
		...
		if handled {
			...
			if updated != nil {
				...
				case m.shouldShowKiroModelPickerScreen():
					m.KiroModelPicker = screens.NewKiroModelPickerState()
					m.setScreen(ScreenKiroModelPicker)
				case m.shouldShowSDDModeScreen():
					m.setScreen(ScreenSDDMode)
				case m.Selection.Preset == model.PresetCustom:
					...
					switch {
					case m.shouldShowStrictTDDScreen():
						m.setScreen(ScreenStrictTDD)
					case m.shouldShowSkillPickerScreen():
	...
	case " ":
		...
		case ScreenUninstallProfiles:
			if m.Cursor < len(m.UninstallProfilesAvailable) {
				...
				m.toggleCurrentUninstallDxrkMemoryScope()
			}
		case ScreenDependencyTree:
...
func (m Model) confirmSelection() (tea.Model, tea.Cmd) {
	...
	case ScreenUninstallProfiles:
		...
		dxrkMemoryScopeOptionCount := 0
		if m.shouldShowUninstallDxrkMemoryScopeSelection() {
			dxrkMemoryScopeOptionCount = 2
		}
		continueIdx := profileCount + dxrkMemoryScopeOptionCount
		switch {
	...
	case ScreenProfiles:
		...
		case m.Cursor < profileCount:
			...
			// The ModelPicker shows dxrk-orchestrator as the base row, so we need
	...
	case ScreenKiroModelPicker:
		if !m.KiroModelPicker.InCustomMode && m.Cursor == screens.KiroModelPickerOptionCount(m.KiroModelPicker)-1 {
			...
			switch {
			case m.shouldShowClaudeModelPickerScreen():
				m.setScreen(ScreenClaudeModelPicker)
			case m.Selection.Preset == model.PresetCustom:
				m.setScreen(ScreenDependencyTree)
			default:
	...
	case ScreenStrictTDD:
		...
		if m.Cursor < len(options) {
			...
			switch {
			case m.shouldShowOpenCodePluginsScreen():
				m.setScreen(ScreenOpenCodePlugins)
			case m.Selection.Preset == model.PresetCustom:
				// Custom preset: dependency plan was already built before SDD mode.
				// Check skill picker before going to review.
...
func (m Model) withResetUninstallState() Model {
	...
	m.UninstallProfileSelection = false
	m.UninstallDxrkMemoryProjectScopeAvailable = false
	m.UninstallDxrkMemoryScope = model.DxrkMemoryUninstallScopeGlobal
...
func (m Model) startUninstall() tea.Cmd {
	...
	dxrkMemoryScope := m.UninstallDxrkMemoryScope
	...
	return func() tea.Msg {
		...
		if uninstallWithProfilesFn != nil && profileSelectionUsed {
			result, err = uninstallWithProfilesFn(agentIDs, componentIDs, profileNamesToRemove, dxrkMemoryScope)
		} else {
		...
		if mode == model.UninstallModeFullRemove {
			...
			if isHomebrewManagedBinary(execPath) {
				result.ManualActions = append(result.ManualActions,
					"Homebrew-managed install detected. Run 'brew uninstall dxrk' to remove the executable cleanly.")
...
func (m *Model) refreshUninstallProfiles() {
	m.UninstallDxrkMemoryProjectScopeAvailable = m.detectProjectDxrkMemoryData()
	m.UninstallDxrkMemoryScope = model.DxrkMemoryUninstallScopeGlobal
...
func (m Model) detectProjectDxrkMemoryData() bool {
	if !hasSelectedComponent(m.UninstallComponents, model.ComponentDxrkMemory) {
		return false
	}
	cwd, err := osGetwdFn()
	...
	info, err := osStatPathFn(filepath.Join(cwd, ".dxrk-memory"))
	if err != nil {
		return false
	}
	return info.IsDir()
...
func (m *Model) toggleCurrentUninstallDxrkMemoryScope() {
	profileCount := len(m.UninstallProfilesAvailable)
	if m.Cursor < profileCount || !m.shouldShowUninstallDxrkMemoryScopeSelection() {
		return
	}
	idx := m.Cursor - profileCount
	if idx == 0 {
		m.UninstallDxrkMemoryScope = model.DxrkMemoryUninstallScopeProject
		return
	}
	if idx == 1 {
		m.UninstallDxrkMemoryScope = model.DxrkMemoryUninstallScopeGlobal
...
func opencodepluginDefinitions() []model.OpenCodeCommunityPluginID {
	return []model.OpenCodeCommunityPluginID{model.OpenCodePluginSubAgentStatusline, model.OpenCodePluginSDDDxrkMemoryManage}
}

func opencodepluginRepoURLs() []string {
	return []string{"https://github.com/Joaquinvesapa/sub-agent-statusline", "https://github.com/j0k3r-dev-rgl/sdd-dxrk-memory-plugin"}
}
...
func piOnlyComponents() []model.ComponentID {
	return []model.ComponentID{model.ComponentDxrkMemory}
}
...
func componentsForPreset(preset model.PresetID) []model.ComponentID {
	...
	case model.PresetEcosystemOnly:
		return []model.ComponentID{model.ComponentDxrkMemory, model.ComponentSDD, model.ComponentSkills, model.ComponentContext7, model.ComponentDxrkGuardian}
...
func (m Model) shouldShowUninstallDxrkMemoryScopeSelection() bool {
	if !hasSelectedComponent(m.UninstallComponents, model.ComponentDxrkMemory) {
		return false
	}
	return m.UninstallDxrkMemoryProjectScopeAvailable
}

func (m Model) shouldShowUninstallSubSelection() bool {
	return m.shouldShowUninstallProfilesSelection() || m.shouldShowUninstallDxrkMemoryScopeSelection()
}
...
func (m Model) startInstallation() (tea.Model, tea.Cmd) {
	...
	return m, tea.Batch(tickCmd(), func() (msg tea.Msg) {
		...
		registryPath := filepath.Join(homeDir(), ".config", "dxrk", "custom-agents.json")
		_ = os.MkdirAll(filepath.Dir(registryPath), 0o750)
