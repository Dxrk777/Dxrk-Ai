...
const SDDOrchestratorPhase = "dxrk-orchestrator"

// ModelPickerRows returns the row labels for the model picker screen.
// Row 0 is "dxrk-orchestrator" (coordinator), row 1 is "Set all phases",
...
func renderPhaseList(
	...
	for idx, row := range rows {
		...
		switch idx {
		case 0:
			// "dxrk-orchestrator" row — coordinator, individual assignment only
	...
	b.WriteString(renderOptions([]string{actionContinue, actionBackArrow}, actionIdx))
	b.WriteString("\n")
