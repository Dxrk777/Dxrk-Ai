...
func TestModelPickerRows_OrchestratorIsFirst(t *testing.T) {
	rows := ModelPickerRows()
	if rows[0] != "dxrk-orchestrator" {
		t.Fatalf("ModelPickerRows()[0] = %q, want %q", rows[0], "dxrk-orchestrator")
	}
}
...
func TestHandleModelNav_OrchestratorRowAssignsOnlyOrchestrator(t *testing.T) {
	state := makeTestState(0) // row 0 = dxrk-orchestrator
	...
	// "dxrk-orchestrator" key must be set
...
func TestHandleModelNav_SetAllPhasesRow_SetsOnlySubAgents(t *testing.T) {
	...
	// dxrk-orchestrator must NOT be touched by "Set all phases"
	if _, exists := updated[SDDOrchestratorPhase]; exists {
		t.Errorf("dxrk-orchestrator should NOT be assigned by 'Set all phases'; assignments: %v", updated)
	}
}
...
func TestHandleModelNav_SubAgentRow_AssignsCorrectPhase(t *testing.T) {
	...
	for i, expectedPhase := range phases {
		t.Run(expectedPhase, func(t *testing.T) {
			...
			if _, exists := updated[SDDOrchestratorPhase]; exists {
				t.Errorf("dxrk-orchestrator should not be assigned; assignments: %v", updated)
			}
		})
	}
}
...
func TestSDDOrchestratorPhaseConstant(t *testing.T) {
	if SDDOrchestratorPhase != "dxrk-orchestrator" {
		t.Fatalf("SDDOrchestratorPhase = %q, want %q", SDDOrchestratorPhase, "dxrk-orchestrator")
	}
}
...
func TestSetAllPhasesLabelSeparateFromIndividualPhases(t *testing.T) {
	...
	handleModelNav("enter", individualState, assignments)
...
func TestHandleEffortNav_SetAllPhasesUpdatesAllPhasesModelAndAllSubAgents(t *testing.T) {
	...
	// dxrk-orchestrator must NOT be touched by "Set all phases".
	if _, exists := updated[SDDOrchestratorPhase]; exists {
		t.Errorf("dxrk-orchestrator should NOT be assigned by Set all phases effort")
	}
}
