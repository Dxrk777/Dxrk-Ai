...
type UninstallDxrkMemoryScopeOption struct {
	Scope       model.DxrkMemoryUninstallScope
	Label       string
	Description string
}
...
func UninstallModeOptions() []UninstallModeOption {
	return []UninstallModeOption{
		...
		},
		{
			...
			Description: "Remove all dxrk managed configuration from all agents",
		},
		{
			Mode:        model.UninstallModeFullRemove,
			Label:       "Full Uninstall & Remove Binary",
			Description: "Remove all configuration AND delete the dxrk binary itself",
		},
		{
...
func RenderUninstallMode(cursor int) string {
	...
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Choose how you want to uninstall dxrk:"))
	b.WriteString("\n\n")
...
func RenderUninstall(selected []model.AgentID, cursor int) string {
	...
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Select the agents whose dxrk managed configuration should be removed."))
	b.WriteString("\n\n")
	...
	if len(selected) == 0 {
		...
		if relCursor == 0 {
			b.WriteString(styles.SelectedStyle.Render(styles.Cursor+styles.SubtextStyle.Render(actionContinue)+" "+styles.HelpStyle.Render("(select at least one agent)")) + "\n")
		} else {
			b.WriteString(styles.SubtextStyle.Render("  "+actionContinue) + "\n")
		}
		b.WriteString(renderOptions([]string{"Back"}, relCursor-1))
	} else {
		b.WriteString(renderOptions([]string{actionContinue, "Back"}, relCursor))
	}
	b.WriteString("\n")
...
func RenderUninstallComponents(selected []model.ComponentID, cursor int) string {
	...
	b.WriteString(styles.HelpStyle.Render("Use j/k to move, space to toggle, enter to continue."))
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Select which dxrk managed components should be removed from the selected agents."))
	b.WriteString("\n\n")
	...
	if len(selected) == 0 {
		// Render Continue as dimmed with an inline hint when nothing is selected.
		if relCursor == 0 {
			b.WriteString(styles.SelectedStyle.Render(styles.Cursor+styles.SubtextStyle.Render(actionContinue)+" "+styles.HelpStyle.Render("(select at least one component)")) + "\n")
		} else {
			b.WriteString(styles.SubtextStyle.Render("  "+actionContinue) + "\n")
		}
		b.WriteString(renderOptions([]string{"Back"}, relCursor-1))
...
func uninstallDxrkMemoryScopeOptions(projectScopeAvailable bool) []UninstallDxrkMemoryScopeOption {
	options := make([]UninstallDxrkMemoryScopeOption, 0, 2)
	if projectScopeAvailable {
		options = append(options, UninstallDxrkMemoryScopeOption{
			Scope:       model.DxrkMemoryUninstallScopeProject,
			Label:       "Project-only cleanup",
			Description: "Delete only .dxrk-memory/ in the current project",
		})
	}
	options = append(options, UninstallDxrkMemoryScopeOption{
		Scope:       model.DxrkMemoryUninstallScopeGlobal,
		Label:       "Global cleanup",
		Description: "Remove global DxrkMemory MCP/system prompt integration",
	})
	return options
}

func RenderUninstallProfiles(available []string, selected []string, dxrkMemoryProjectScopeAvailable bool, selectedDxrkMemoryScope model.DxrkMemoryUninstallScope, cursor int) string {
	var b strings.Builder
	...
	dxrkMemoryScopeOptions := uninstallDxrkMemoryScopeOptions(dxrkMemoryProjectScopeAvailable)
	dxrkMemoryScopeDisplayed := 0
	if len(dxrkMemoryScopeOptions) > 1 {
		dxrkMemoryScopeDisplayed = len(dxrkMemoryScopeOptions)
		if len(available) > 0 {
			b.WriteString("\n")
		}
		b.WriteString(styles.SubtextStyle.Render("Select Dxrk Memory cleanup scope:"))
		b.WriteString("\n")
		for idx, option := range dxrkMemoryScopeOptions {
			focused := len(available)+idx == cursor
			checked := selectedDxrkMemoryScope == option.Scope
	...
	b.WriteString("\n")
	relCursor := cursor - (len(available) + dxrkMemoryScopeDisplayed)
	b.WriteString(renderOptions([]string{actionContinue, "Back"}, relCursor))
	b.WriteString("\n")
...
func RenderUninstallConfirm(mode model.UninstallMode, selected []model.AgentID, components []model.ComponentID, profilesToRemove []string, dxrkMemoryScope model.DxrkMemoryUninstallScope, dxrkMemoryProjectScopeAvailable bool, cursor int, operationRunning bool, spinnerFrame int) string {
	var b strings.Builder
	...
	case model.UninstallModeFull:
		...
		b.WriteString("\n\n")
		b.WriteString(styles.UnselectedStyle.Render("This will remove all dxrk managed configuration from all supported agents."))
		b.WriteString("\n")
	case model.UninstallModeFullRemove:
		b.WriteString(styles.ErrorStyle.Render("Mode: Full Uninstall & Remove Binary"))
		b.WriteString("\n\n")
		b.WriteString(styles.UnselectedStyle.Render("This will remove all dxrk managed configuration from all agents"))
		b.WriteString("\n")
		b.WriteString(styles.ErrorStyle.Render("AND delete the dxrk binary itself."))
		b.WriteString("\n\n")
	...
	if hasSelectedComponent(components, model.ComponentDxrkMemory) {
		b.WriteString("\n")
		b.WriteString(styles.SubtextStyle.Render("Dxrk Memory cleanup scope:"))
		b.WriteString("\n")
		scopeLabel := "Global"
		detail := "  • Removes global DxrkMemory MCP/system prompt configuration"
		if dxrkMemoryScope == model.DxrkMemoryUninstallScopeProject && dxrkMemoryProjectScopeAvailable {
			scopeLabel = "Project-only"
			detail = "  • Deletes .dxrk-memory/ in the current project only"
		}
	...
	if (mode == model.UninstallModeFull || mode == model.UninstallModeFullRemove) || hasWorkspaceAssets {
		...
		b.WriteString("\n")
		b.WriteString(styles.SubtextStyle.Render("  • .dxrk-memory/ (persistent memory context)"))
		b.WriteString("\n")
	...
	b.WriteString("\n\n")
	b.WriteString(renderOptions([]string{"Uninstall", actionCancel}, cursor))
	b.WriteString("\n")
	b.WriteString(styles.HelpStyle.Render("j/k: navigate • enter: select • esc: back"))
...
func RenderUninstallResult(result componentuninstall.Result, err error, mode model.UninstallMode, selectedProfiles []string, dxrkMemoryScope model.DxrkMemoryUninstallScope, dxrkMemoryProjectScopeAvailable bool, syncFilesChanged int, syncErr error) string {
	var b strings.Builder
	...
	if err != nil {
		...
		if hasDxrkMemoryArtifacts(result) {
			b.WriteString("\n\n")
			if dxrkMemoryScope == model.DxrkMemoryUninstallScopeProject && dxrkMemoryProjectScopeAvailable {
				b.WriteString(styles.UnselectedStyle.Render("Dxrk Memory scope: Project-only (.dxrk-memory/ removed from current workspace)"))
			} else {
				b.WriteString(styles.UnselectedStyle.Render("Dxrk Memory scope: Global (MCP/system prompt integration removed)"))
			}
		}
		...
		if mode == model.UninstallModeCleanInstall {
			...
			if syncErr != nil {
				...
				b.WriteString("\n\n")
				b.WriteString(styles.WarningStyle.Render("You can run 'dxrk sync' manually to retry."))
			} else {
...
func hasDxrkMemoryArtifacts(result componentuninstall.Result) bool {
	for _, path := range result.ChangedFiles {
		if strings.Contains(path, "dxrk-memory") {
			return true
		}
	}
