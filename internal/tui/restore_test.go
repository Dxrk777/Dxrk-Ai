...
func makeTestBackup(t time.Time, source backup.BackupSource) backup.Manifest {
...
func TestBackupSelectionNavigatesToRestoreConfirm(t *testing.T) {
	...
	m.Backups = []backup.Manifest{
		makeTestBackup(time.Now(), backup.BackupSourceInstall),
	}
	m.Cursor = 0
...
func TestRestoreConfirmEnterExecutesAndNavigatesToResult(t *testing.T) {
	...
	m.SelectedBackup = makeTestBackup(time.Now(), backup.BackupSourceInstall)
...
func TestRestoreConfirmCancelNavigatesBackToBackups(t *testing.T) {
	...
	m := NewModel(system.DetectionResult{}, "dev")
	m.Screen = ScreenRestoreConfirm
	m.SelectedBackup = makeTestBackup(time.Now(), backup.BackupSourceSync)
...
func TestRestoreResultSuccessScreenIsShown(t *testing.T) {
	m := NewModel(system.DetectionResult{}, "dev")
	m.Screen = ScreenRestoreConfirm
	m.SelectedBackup = makeTestBackup(time.Now(), backup.BackupSourceUpgrade)
