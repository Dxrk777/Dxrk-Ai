...
func TestResolveEngramBrewBypassesGoValidation(t *testing.T) {
	...
	cmds, err := resolveDxrkMemoryInstall(profile)
	if err != nil {
		t.Fatalf("resolveDxrkMemoryInstall() unexpected error = %v", err)
	}
	if len(cmds) == 0 {
		t.Fatal("resolveDxrkMemoryInstall() returned empty CommandSequence")
	}
}
...
func TestGitBashPathResolvesFromGitOnPath(t *testing.T) {
	...
	if err := os.WriteFile(fakeGit, []byte("fake"), 0o750); err != nil { //nolint:gosec
		t.Fatal(err)
	}
	fakeBash := filepath.Join(binDir, "bash.exe")
	if err := os.WriteFile(fakeBash, []byte("fake"), 0o750); err != nil { //nolint:gosec
		t.Fatal(err)
	}
...
func TestResolveComponentInstall(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:      CommandSequence{{"brew", "tap", "Dxrk777/homebrew-tap"}, {"brew", "install", "dxrk-memory"}},
		},
		...
		{
			name:      "dxrk-memory on ubuntu returns error (uses DownloadLatestBinary instead)",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroUbuntu, PackageManager: "apt"},
			component: model.ComponentDxrkMemory,
			wantErr:   true,
		},
		{
			name:      "dxrk-memory on arch returns error (uses DownloadLatestBinary instead)",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroArch, PackageManager: "pacman"},
			component: model.ComponentDxrkMemory,
			wantErr:   true,
		},
		{
			name:      "dxrk-memory on fedora returns error (uses DownloadLatestBinary instead)",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroFedora, PackageManager: "dnf"},
			component: model.ComponentDxrkMemory,
			wantErr:   true,
		},
		{
			name:      "dxrk-guardian on darwin uses brew tap and reinstall",
			profile:   system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			component: model.ComponentDxrkGuardian,
			want:      CommandSequence{{"brew", "tap", "Dxrk777/homebrew-tap"}, {"brew", "reinstall", "dxrk-guardian"}},
		},
		{
			name:      "dxrk-guardian on ubuntu uses git clone and install.sh",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroUbuntu, PackageManager: "apt"},
			component: model.ComponentDxrkGuardian,
			want: CommandSequence{
				{"rm", "-rf", "/tmp/dxrk-guardian-angel"},
				{"git", "clone", "https://github.com/Dxrk777/dxrk-guardian-angel.git", "/tmp/dxrk-guardian-angel"},
				{"bash", "/tmp/dxrk-guardian-angel/install.sh"},
			},
		},
		{
			name:      "dxrk-guardian on arch uses git clone and install.sh",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroArch, PackageManager: "pacman"},
			component: model.ComponentDxrkGuardian,
			want: CommandSequence{
				{"rm", "-rf", "/tmp/dxrk-guardian-angel"},
		...
		{
			name:      "dxrk-guardian on fedora uses git clone and install.sh",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroFedora, PackageManager: "dnf"},
			component: model.ComponentDxrkGuardian,
			want: CommandSequence{
				{"rm", "-rf", "/tmp/dxrk-guardian-angel"},
		...
		{
			name:      "dxrk-memory on windows returns error (uses DownloadLatestBinary instead)",
			profile:   system.PlatformProfile{OS: "windows", PackageManager: "winget"},
			component: model.ComponentDxrkMemory,
			wantErr:   true,
		},
		{
			name:      "dxrk-guardian on windows cleans temp dir and uses git bash",
			profile:   system.PlatformProfile{OS: "windows", PackageManager: "winget"},
			component: model.ComponentDxrkGuardian,
			want: CommandSequence{
				...
				{"git", "clone", "https://github.com/Dxrk777/dxrk-guardian-angel.git", filepath.Join(os.TempDir(), "dxrk-guardian-angel")},
