...
func (a *Adapter) InstallCommand(system.PlatformProfile) ([][]string, error) {
	return [][]string{
		{"pi", piInstallCmd, "npm:dxrk-pi"},
		{"pi", piInstallCmd, "npm:dxrk-dxrk-memory"},
		{"pi", piInstallCmd, "npm:pi-mcp-adapter"},
		{"npm", "exec", "--yes", "--package", "dxrk-dxrk-memory@0.1.4", "--", "pi-dxrk-memory", "init"},
		{"pi", piInstallCmd, "npm:pi-subagents"},
		{"pi", piInstallCmd, "npm:pi-intercom"},
		{"pi", piInstallCmd, "npm:@juicesharp/rpiv-ask-user-question"},
		{"pi", piInstallCmd, "npm:pi-web-access"},
		{"pi", piInstallCmd, "npm:pi-lens"},
		{"pi", piInstallCmd, "npm:@juicesharp/rpiv-todo"},
		{"pi", piInstallCmd, "npm:pi-btw"},
	}, nil
}
...
func (a *Adapter) MCPConfigPath(homeDir string, _ string) string {
	return filepath.Join(AgentConfigPath(homeDir), piDxrkMemoryMCPConfigFile)
}
...
// ProvisionDxrkMemoryMCP declares pi-mcp-adapter in Pi's settings.json and
// package.json. It is invoked by ComponentDxrkMemory; keeping it here lets Pi
// own the exact config shape without teaching the generic DxrkMemory injector
// about Pi internals.
//
// mcp.json is NOT written here. pi-dxrk-memory init (invoked by InstallCommand)
// is the sole writer of that file and owns its schema.
func (a *Adapter) ProvisionDxrkMemoryMCP(homeDir string) (bool, []string, error) {
	paths := []string{
...
func readPiJSONObject(path string) (map[string]any, error) {
	base, err := os.ReadFile(path) //nolint:gosec
	if err != nil {
