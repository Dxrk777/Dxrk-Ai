...
func TestAdapterIdentityAndStrategies(t *testing.T) {
	...
	if got := a.Agent(); got != model.AgentAmazonQ {
		t.Fatalf("Agent() = %q, want %q", got, model.AgentAmazonQ)
	}
...
func TestAdapterConfigPaths(t *testing.T) {
	a := NewAdapter()
	homeDir := filepath.Join(string(filepath.Separator), "tmp", "home")
	configDir := filepath.Join(homeDir, ".aws", "q")
...
func TestDetect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantConfigPath := filepath.Join(homeDir, ".aws", "q")
...
func TestAgentNotInstallableError(t *testing.T) {
	e := AgentNotInstallableError{Agent: model.AgentAmazonQ}
