...
func TestAdapterIdentityAndStrategies(t *testing.T) {
	...
	if got := a.Agent(); got != model.AgentAider {
		t.Fatalf("Agent() = %q, want %q", got, model.AgentAider)
	}
...
func TestInstallCommand(t *testing.T) {
	...
	if len(commands[0]) != 3 || commands[0][0] != "pip" || commands[0][1] != "install" || commands[0][2] != "aider-chat" {
		t.Fatalf("InstallCommand() = %v, want [[pip install aider-chat]]", commands)
	}
}
...
func TestAdapterConfigPaths(t *testing.T) {
	a := NewAdapter()
	homeDir := filepath.Join(string(filepath.Separator), "tmp", "home")
	configDir := filepath.Join(homeDir, ".aider")
...
func TestDetect(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			lookPathPath:    "/usr/local/bin/aider",
			stat:            statResult{isDir: false},
			wantInstalled:   true,
			wantBinaryPath:  "/usr/local/bin/aider",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantConfigPath := filepath.Join(homeDir, ".aider.conf.yml")
...
func TestAgentNotInstallableError(t *testing.T) {
	e := AgentNotInstallableError{Agent: model.AgentAider}
