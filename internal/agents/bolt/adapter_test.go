...
func TestAdapterIdentity(t *testing.T) {
	a := NewAdapter()
	if got := a.Agent(); got != model.AgentBolt {
		t.Fatalf("Agent() = %q, want %q", got, model.AgentBolt)
	}
}
...
func TestDetect(t *testing.T) {
	...
	if configPath != filepath.Join(homeDir, ".bolt") {
...
func TestConfigPaths(t *testing.T) {
	a := NewAdapter()
	home := "/tmp/home"
	if got := a.GlobalConfigDir(home); got != filepath.Join(home, ".bolt") {
