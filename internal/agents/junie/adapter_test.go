...
func TestAdapterIdentityAndStrategies(t *testing.T) {
	...
	if got := a.Agent(); got != model.AgentJunie {
		t.Fatalf("Agent() = %q, want %q", got, model.AgentJunie)
	}
...
func TestAdapterConfigPaths(t *testing.T) {
	a := NewAdapter()
	homeDir := filepath.Join(string(filepath.Separator), "tmp", "home")
	configDir := filepath.Join(homeDir, ".junie")
	...
	if got := paths["SettingsPath"]; got != filepath.Join(configDir, "settings.json") {
		t.Fatalf("SettingsPath() = %q, want %q", got, filepath.Join(configDir, "settings.json"))
	}

	if got := paths["MCPConfigPath"]; got != filepath.Join(configDir, "settings.json") {
		t.Fatalf("MCPConfigPath() = %q, want %q", got, filepath.Join(configDir, "settings.json"))
	}
...
func TestDetect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantConfigPath := filepath.Join(homeDir, ".junie")
...
func TestAgentNotInstallableError(t *testing.T) {
	e := AgentNotInstallableError{Agent: model.AgentJunie}
