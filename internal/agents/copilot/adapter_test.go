...
func TestAdapterIdentityAndStrategies(t *testing.T) {
	...
	if got := a.Agent(); got != model.AgentCopilot {
		t.Fatalf("Agent() = %q, want %q", got, model.AgentCopilot)
	}
...
func TestAdapterConfigPaths(t *testing.T) {
	a := NewAdapter()
	homeDir := filepath.Join(string(filepath.Separator), "tmp", "home")
	configDir := filepath.Join(homeDir, ".config", "github-copilot")
	...
	wantMCP := filepath.Join(configDir, "mcp.json")
	if got := paths["MCPConfigPath"]; got != wantMCP {
		t.Fatalf("MCPConfigPath() = %q, want %q", got, wantMCP)
	}

	if got := paths["SkillsDir"]; got != filepath.Join(configDir, "skills") {
		t.Fatalf("SkillsDir() = %q, want skills dir", got)
	}
}
...
func TestDetect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantConfigPath := filepath.Join(homeDir, ".config", "github-copilot")
