...
func TestAdapter_Detect_BinaryNotFound(t *testing.T) {
	...
	installed, _, configPath, configFound, err := adapter.Detect(context.TODO(), home)
	if installed {
...
func TestAdapter_Detect_BinaryFoundConfigDirMissing(t *testing.T) {
	...
	installed, binaryPath, configPath, configFound, err := adapter.Detect(context.TODO(), home)
	if !installed {
...
func TestAdapter_Detect_BinaryFoundConfigDirExists(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(home, ".kiro"), 0o750); err != nil {
...
func TestAdapter_SystemPromptFile(t *testing.T) {
	adapter := NewAdapter()
	homeDir := "/home/user"
	expected := filepath.Join(homeDir, ".kiro", "steering", "dxrk.md")
...
func TestAdapter_MCPStrategy(t *testing.T) {
	...
	if got != expected {
		t.Errorf("MCPStrategy() = %v, want %v", got, expected)
	}
}
...
func TestAdapter_Detect_UnexpectedLookPathError(t *testing.T) {
	...
	installed, _, _, _, err := adapter.Detect(context.TODO(), home)
	if installed {
