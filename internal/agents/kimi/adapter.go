...
func (a *Adapter) PostInstallMessage(homeDir string) string {
	dxrkYaml := filepath.Join(homeDir, ".kimi", "agents", "dxrk.yaml")
...
// (SDD, DxrkMemory) can be included even if the Persona component is not installed.
func (a *Adapter) BootstrapTemplate(homeDir string) error {
	kimiDir := a.GlobalConfigDir(homeDir)
	if err := os.MkdirAll(kimiDir, 0o750); err != nil {
	...
	if _, err := filemerge.WriteFileAtomic(skeletonPath, []byte(content), 0o600); err != nil {
