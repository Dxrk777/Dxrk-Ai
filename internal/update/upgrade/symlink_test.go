...
func TestEnumerateFilesInDir_SkipsSymlinks(t *testing.T) {
	...
	if err := os.WriteFile(realFile, []byte(`{"key":"value"}`), 0o600); err != nil {
	...
	if err := os.WriteFile(filepath.Join(targetDir, "skill.md"), []byte("# Skill"), 0o600); err != nil {
...
func TestEnumerateFilesInDir_SymlinkToFileIsIncluded(t *testing.T) {
	...
	if err := os.MkdirAll(dotfilesDir, 0o750); err != nil {
		t.Fatalf("MkdirAll dotfiles: %v", err)
	}
	realConfig := filepath.Join(dotfilesDir, "CLAUDE.md")
	if err := os.WriteFile(realConfig, []byte("# My Claude rules"), 0o600); err != nil {
...
func TestEnumerateFilesInDir_SymlinkInSubdirDoesNotBreakBackup(t *testing.T) {
	...
	if err := os.WriteFile(realConf, []byte("# Claude rules"), 0o600); err != nil {
	...
	if err := os.MkdirAll(skillsTarget, 0o750); err != nil {
		t.Fatalf("MkdirAll skillsTarget: %v", err)
	}
	if err := os.WriteFile(filepath.Join(skillsTarget, "go-testing.md"), []byte("# Go testing"), 0o600); err != nil {
