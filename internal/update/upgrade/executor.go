...
//   - dxrk/internal/pipeline
//   - dxrk/internal/planner
//   - dxrk/internal/cli
package upgrade
...
// configPathsForBackup returns the explicit Dxrk AI-managed file paths that
...
// Upgrade backups are rollback artifacts for files Dxrk AI may create or
...
func managedAgentBackupPaths(homeDir string, adapter agents.Adapter, diagnostics io.Writer) []string {
	...
	if adapter.SupportsMCP() {
		add(adapter.MCPConfigPath(homeDir, "dxrk-memory"), adapter.MCPConfigPath(homeDir, "context7"))
	}

	if adapter.SupportsOutputStyles() {
		add(filepath.Join(adapter.OutputStyleDir(homeDir), "dxrk.md"))
	}
	...
	case model.AgentClaudeCode:
		add(filepath.Join(homeDir, ".claude", "themes", "dxrk.json"))
...
func managedGlobalBackupPaths(homeDir string) []string {
	return []string{
		...
		dxrkguardian.RuntimePRModePath(homeDir),
		dxrkguardian.RuntimePS1Path(homeDir),
	}
}
...
func managedSkillBackupPaths(homeDir string, adapter agents.Adapter, diagnostics io.Writer) []string {
	...
	for _, relPath := range []string{
		...
		"_shared/dxrk-memory-convention.md",
...
func enumerateFilesInDir(dir string, excludeDirNames map[string]bool, diagnostics ...io.Writer) ([]string, error) {
	...
	err := filepath.WalkDir(cleanDir, func(path string, d os.DirEntry, err error) error {
		...
		if d.Type()&os.ModeSymlink != 0 {
			...
			if statErr != nil {
				writeBackupDiagnostic(dw, "backup: skipping unresolvable symlink %s (target may be missing): %v", path, statErr)
				return nil
			}
			if resolved.IsDir() {
...
func ExecuteWithOptions(ctx context.Context, results []update.UpdateResult, profile system.PlatformProfile, homeDir string, dryRun bool, options ExecuteOptions) UpgradeReport {
	...
	for _, r := range devBuilds {
		toolResults = append(toolResults, ToolUpgradeResult{
			...
			ManualHint: fmt.Sprintf("source build — upgrade manually or install a release binary from https://github.com/Dxrk777/%s/releases", r.Tool.Repo),
		})
	}
