...
func TestExecute_NoopWhenNothingIsExecutable(t *testing.T) {
	results := []update.UpdateResult{
		...
		// dxrk-guardian: CheckFailed — should also be omitted from results.
		makeResult("dxrk-guardian", update.CheckFailed, "", "", update.InstallScript),
	}
...
func TestExecute_VersionUnknownIsSurfacedAsSkipped(t *testing.T) {
	results := []update.UpdateResult{
		makeResult("dxrk-memory", update.VersionUnknown, "", "1.2.0", update.InstallBinary),
	}
	results[0].Tool.DetectCmd = []string{"dxrk-memory", "version"}
	...
	if !strings.Contains(report.Results[0].ManualHint, "`dxrk-memory version`") {
...
func TestExecute_RegisteredNotMaterializedIsExecutable(t *testing.T) {
	...
	result := makeResult("opencode-sdd-dxrk-memory-manage", update.RegisteredNotMaterialized, "", "1.2.0", update.InstallOpenCodePlugin)
	result.Tool.NpmPackage = "opencode-sdd-dxrk-memory-manage"
...
func TestRenderUpgradeReport_DryRunManualHintNotCountedAsPending(t *testing.T) {
	report := UpgradeReport{
		...
		Results: []ToolUpgradeResult{
			{ToolName: "dxrk-memory", Status: UpgradeSkipped, ManualHint: "source build — upgrade manually"},
		},
	}
...
func TestExecute_PerToolSuccessAndFailure(t *testing.T) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		// dxrk-memory go install succeeds, dxrk-guardian curl/download attempt fails — we simulate
		// the failure by having execCommand return false for "dxrk-guardian" detection.
		if name == "go" {
...
// DevBuild tools must NOT be auto-executed, and dxrk-memory/dxrk-guardian remain eligible.
...
func TestExecute_ConfigNotMutatedDuringUpgrade(t *testing.T) {
	...
	for relPath, want := range configFiles {
		fullPath := homeDir + "/" + relPath
		got, err := os.ReadFile(fullPath) //nolint:gosec
		if err != nil {
...
// backups include Dxrk AI-managed files for installed agents, without treating
...
func TestConfigPathsForBackup_CoversManagedAgentPaths(t *testing.T) {
	...
	managedFiles := map[string]string{
		...
		".cursor/rules/dxrk.mdc":    "# Cursor rules",
	}
	...
	if err := os.WriteFile(unmanagedFile, []byte("runtime data"), 0o600); err != nil {
...
func TestExecute_UpgradeBackupManifestHasUpgradeMetadata(t *testing.T) {
	...
	if err := os.WriteFile(configFile, []byte("# Claude"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestConfigPathsForBackup_CoversRegistryAgentsNotInOldList(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(codexFile), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(codexFile, []byte("# Codex config"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestConfigPathsForBackup_GGAExtrasAreIncluded(t *testing.T) {
	...
	// Create GGA config file at ~/.config/dxrk-guardian/config
	ggaConfigFile := filepath.Join(homeDir, ".config", "dxrk-guardian", "config")
	if err := os.MkdirAll(filepath.Dir(ggaConfigFile), 0o750); err != nil {
		t.Fatalf("MkdirAll dxrk-guardian config: %v", err)
	}
	if err := os.WriteFile(ggaConfigFile, []byte("dxrk-guardian-config"), 0o600); err != nil {
		t.Fatalf("WriteFile dxrk-guardian config: %v", err)
	}

	// Create GGA runtime lib file at ~/.local/share/dxrk-guardian/lib/pr_mode.sh
	ggaLibFile := filepath.Join(homeDir, ".local", "share", "dxrk-guardian", "lib", "pr_mode.sh")
	if err := os.MkdirAll(filepath.Dir(ggaLibFile), 0o750); err != nil {
		t.Fatalf("MkdirAll dxrk-guardian lib: %v", err)
	}
	if err := os.WriteFile(ggaLibFile, []byte("#!/bin/sh"), 0o750); err != nil { //nolint:gosec
		t.Fatalf("WriteFile dxrk-guardian lib: %v", err)
	}
...
func TestEnumerateFilesInDir_ExcludesSubdirs(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(allowedFile), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(allowedFile, []byte(`{}`), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}

	// Excluded subdir at depth 1 — must be skipped.
	excludedFile := filepath.Join(root, "projects", "data.json")
	if err := os.MkdirAll(filepath.Dir(excludedFile), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(excludedFile, []byte(`big data`), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	...
	if err := os.MkdirAll(filepath.Dir(nestedExcludedFile), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(nestedExcludedFile, []byte(`3.6GB of video`), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	...
	if err := os.WriteFile(nestedConfigFile, []byte(`[settings]`), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestEnumerateFilesInDir_DefaultExclusionDiagnosticsAreSilent(t *testing.T) {
	...
	if err := os.WriteFile(excludedFile, []byte("runtime"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
// must stay limited to Dxrk AI-managed files, not conversations or caches.
...
func TestConfigPathsForBackup_ExcludesRuntimeDirs(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(claudeConfig), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(claudeConfig, []byte("# Claude"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	...
	if err := os.MkdirAll(filepath.Dir(geminiConfig), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(geminiConfig, []byte("# Gemini"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	...
	if err := os.MkdirAll(filepath.Dir(openCodeConfig), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(openCodeConfig, []byte(`{"model":"free"}`), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	...
	for _, agent := range agents {
		for _, dir := range agent.excludes {
			...
			if err := os.WriteFile(f, []byte("runtime data"), 0o600); err != nil {
	...
	if err := os.MkdirAll(filepath.Dir(geminiAntigravityTmpFile), 0o750); err != nil {
		t.Fatalf("MkdirAll gemini antigravity tmp: %v", err)
	}
	if err := os.WriteFile(geminiAntigravityTmpFile, []byte("temp runtime data"), 0o600); err != nil {
...
func TestEnumerateFilesInDir_ExcludesNestedSameNameDir(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(nestedCacheFile), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(nestedCacheFile, []byte(`{"cached":true}`), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}

	// A sibling file under mcp (not in an excluded dir) — must be included.
	mcpConfig := filepath.Join(root, "mcp", "server.json")
	if err := os.WriteFile(mcpConfig, []byte(`{}`), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestEnumerateFilesInDir_CaseInsensitiveExclude(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(upperDir), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(upperDir, []byte("big"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}

	// Mixed case directory.
	mixedDir := filepath.Join(root, "Cache", "temp.dat")
	if err := os.MkdirAll(filepath.Dir(mixedDir), 0o750); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(mixedDir, []byte("cached"), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
