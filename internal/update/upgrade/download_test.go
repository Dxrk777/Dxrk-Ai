...
func TestDownloadAndExtract(t *testing.T) {
	...
	tarContent, err := os.ReadFile(tarPath) //nolint:gosec
	if err != nil {
...
func TestAtomicReplace(t *testing.T) {
	...
	if err := os.WriteFile(src, []byte("new content"), 0o750); err != nil { //nolint:gosec
		t.Fatalf("write src: %v", err)
	}
	// Write destination (old binary)
	if err := os.WriteFile(dst, []byte("old content"), 0o750); err != nil { //nolint:gosec
...
func TestFindBinaryInTar(t *testing.T) {
	...
	f, _ := os.Create(tarPath) //nolint:gosec
	gw := gzip.NewWriter(f)
	tw := tar.NewWriter(gw)
	...
	for _, e := range entries {
		_ = tw.WriteHeader(&tar.Header{Name: e.name, Mode: 0o600, Size: int64(len(e.content))})
	...
	tarContent, _ := os.ReadFile(tarPath) //nolint:gosec
	...
	got, _ := os.ReadFile(outPath) //nolint:gosec
...
func TestExpectedChecksumFor(t *testing.T) {
	content := "abc123  dxrk_1.0.0_darwin_arm64.tar.gz\ndef456  dxrk_1.0.0_linux_amd64.tar.gz\n"
	...
	tests := []struct {
	...
	}{
		{
			...
			content:  content,
			filename: "dxrk_1.0.0_darwin_arm64.tar.gz",
			want:     "abc123",
		},
		{
			name:     "found second entry",
			content:  content,
			filename: "dxrk_1.0.0_linux_amd64.tar.gz",
			want:     "def456",
		},
		{
			name:     "not found returns error",
			content:  content,
			filename: "dxrk_1.0.0_windows_amd64.zip",
			wantErr:  true,
		},
		{
...
func TestFetchChecksums(t *testing.T) {
	const fakeContent = "abc123  dxrk_1.0.0_darwin_arm64.tar.gz\n"

	t.Run("success returns content", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
			_, _ = fmt.Fprint(w, fakeContent)
		}))
		defer server.Close()
		...
		t.Run(tc.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if strings.HasSuffix(r.URL.Path, "checksums.txt") {
					...
					_, _ = fmt.Fprint(w, tc.checksumsBody)
				} else {
					w.WriteHeader(http.StatusOK)
					_, _ = w.Write(tarContent) //nolint:errcheck
				}
			}))
			defer server.Close()
			...
			if err := os.WriteFile(tmpBinary, []byte("old binary"), 0o750); err != nil { //nolint:gosec
