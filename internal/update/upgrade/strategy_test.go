...
func TestEffectiveMethod(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tool:    update.ToolInfo{Name: "dxrk-guardian", InstallMethod: update.InstallBinary},
			profile: system.PlatformProfile{PackageManager: "brew"},
			want:    update.InstallBrew,
		},
		{
			name:    "brew profile overrides script",
			tool:    update.ToolInfo{Name: "dxrk-guardian", InstallMethod: update.InstallScript},
			profile: system.PlatformProfile{PackageManager: "brew"},
			want:    update.InstallBrew,
		},
		{
...
func TestRunStrategyOpenCodePluginUpgradesMaterializedPackage(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(pkgDir, "package.json"), []byte(`{"version":"0.1.0"}`), 0o600); err != nil {
		t.Fatal(err)
	}
	cacheRoot := filepath.Join(home, ".cache", "opencode", "packages")
	targetCache := filepath.Join(cacheRoot, pkg+"@latest")
	otherPluginCache := filepath.Join(cacheRoot, "opencode-sdd-dxrk-memory-manage@latest")
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		cmd := exec.Command(os.Args[0], "-test.run=TestOpenCodePluginUpgradeHelperProcess", "--") //nolint:gosec
		cmd.Env = append(os.Environ(),
			"DXRK_UPGRADE_HELPER=1",
			"DXRK_UPGRADE_HELPER_CWD_FILE="+cwdFile,
		)
		return cmd
	}
	...
	cwd, err := os.ReadFile(cwdFile) //nolint:gosec
	if err != nil {
...
func TestRunStrategyOpenCodePluginRegisteredPendingRunsPackageManager(t *testing.T) {
	...
	home := t.TempDir()
	opencodeDir := filepath.Join(home, ".config", "opencode")
	pkg := "opencode-sdd-dxrk-memory-manage"
...
func TestRunStrategyOpenCodePluginFallsBackWithoutPackageManager(t *testing.T) {
	...
	opencodeDir := filepath.Join(home, ".config", "opencode")
	pkg := "opencode-sdd-dxrk-memory-manage"
	if err := os.MkdirAll(filepath.Join(opencodeDir, "node_modules", pkg), 0o750); err != nil {
		t.Fatal(err)
	}
...
func TestSelectOpenCodePackageManagerPrefersPackageMetadata(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(opencodeDir, "package.json"), []byte(`{"packageManager":"npm@10.8.0"}`), 0o600); err != nil {
		t.Fatal(err)
	}
	lookPathCommand = func(file string) (string, error) {
		switch file {
...
func openCodePluginUpgradeHelperProcess() {
	cwd, err := os.Getwd()
	if err != nil {
		_, _ = os.Stderr.WriteString(err.Error())
		os.Exit(2) //nolint:revive
	}
	if err := os.WriteFile(os.Getenv("DXRK_UPGRADE_HELPER_CWD_FILE"), []byte(cwd), 0o600); err != nil { //nolint:gosec
		_, _ = os.Stderr.WriteString(err.Error())
		os.Exit(2) //nolint:revive
	}
...
func TestOpenCodePluginUpgradeHelperProcess(t *testing.T) {
	if os.Getenv("DXRK_UPGRADE_HELPER") != "1" {
		return
	}
	openCodePluginUpgradeHelperProcess()
}
...
// `brew tap Dxrk777/homebrew-tap` BEFORE `brew update` and
...
func TestBrewUpgrade_TapsBeforeUpdateAndUpgrade(t *testing.T) {
	...
	if err := brewUpgrade(context.Background(), "dxrk-memory"); err != nil {
		t.Fatalf("brewUpgrade: unexpected error: %v", err)
	}
	...
	if calls[0].arg != "Dxrk777/homebrew-tap" {
		t.Errorf("first brew call arg = %q, want %q", calls[0].arg, "Dxrk777/homebrew-tap")
	}
...
// TestGGAScriptUpgradeUsesGitClone verifies that dxrkGuardianScriptUpgrade:
// 1. First calls `git clone <repo-url> /tmp/dxrk-guardian-angel`
...
func TestGGAScriptUpgradeUsesGitClone(t *testing.T) {
	...
	err := dxrkGuardianScriptUpgrade(context.Background(), r)
	if err != nil {
		t.Fatalf("dxrkGuardianScriptUpgrade: unexpected error: %v", err)
	}
...
// dxrkGuardianScriptUpgrade returns a ManualFallbackError without calling exec.
func TestGGAScriptUpgradeWindowsManualFallback(t *testing.T) {
	...
	err := dxrkGuardianScriptUpgradeForOS(context.Background(), r, "windows")
	if err == nil {
	...
	if execCalled {
		t.Errorf("exec should NOT be called on Windows for dxrkGuardianScriptUpgrade")
	}
}
...
// a GGA tool (InstallScript), it routes to dxrkGuardianScriptUpgrade (git clone approach)
...
func TestInstallScriptURL(t *testing.T) {
	url := installScriptURL("Dxrk777", "dxrk-guardian-angel")
	if url != "https://raw.githubusercontent.com/Dxrk777/dxrk-guardian-angel/main/install.sh" {
...
func TestEngramUpgradeUsesDownloadNotGoInstall(t *testing.T) {
	origExecCommand := execCommand
	origEngramDownloadFn := dxrkMemoryDownloadFn
	t.Cleanup(func() {
		execCommand = origExecCommand
		dxrkMemoryDownloadFn = origEngramDownloadFn
	})
	...
	if !downloadCalled {
		t.Errorf("expected dxrkMemoryDownloadFn to be called, but it was not")
	}
	if execCalled {
