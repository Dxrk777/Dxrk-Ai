...
var dxrkMemoryDownloadFn = dxrkmemory.DownloadLatestBinary
...
//   - script method + linux/darwin + dxrk-guardian → dxrkGuardianScriptUpgrade (git clone approach)
...
func runStrategy(ctx context.Context, r update.UpdateResult, profile system.PlatformProfile) error {
	...
	case update.InstallScript:
		// Dxrk Guardian's install.sh expects to run from within a cloned repo — it references
		// $SCRIPT_DIR/bin/dxrk-guardian and $SCRIPT_DIR/lib/*.sh. The generic scriptUpgrade
		...
		// (same as the initial install resolver) for Dxrk Guardian specifically.
		if r.Tool.Name == "dxrk-guardian" {
			return dxrkGuardianScriptUpgrade(ctx, r)
		}
	...
	default:
		return &ManualFallbackError{
			Hint: fmt.Sprintf("upgrade %q: unsupported install method %q — please update manually. See: https://github.com/Dxrk777/%s",
...
func openCodePluginRegisteredOrMaterialized(opencodeDir, pkg string) (bool, bool, error) {
	...
	data, err := os.ReadFile(filepath.Join(opencodeDir, "tui.json")) //nolint:gosec
	if err != nil {
...
func openCodePackageJSONDependencyVersion(opencodeDir, pkg string) (string, bool) {
	data, err := os.ReadFile(filepath.Join(opencodeDir, "package.json")) //nolint:gosec
...
func openCodePackageManagerFromMetadata(opencodeDir string) string {
	...
	if data, err := os.ReadFile(packageJSON); err == nil { //nolint:gosec
		var manifest struct {
...
func brewUpgrade(_ context.Context, toolName string) error {
	// Ensure the Dxrk777 homebrew tap is present before upgrading.
	...
	// with "No available formula" for dxrk-memory/dxrk-guardian/dxrk.
	tapCmd := execCommand("brew", "tap", "Dxrk777/homebrew-tap")
	tapCmd.Stdin = nil
	_ = tapCmd.Run()
...
func goInstallUpgrade(_ context.Context, tool update.ToolInfo, latestVersion string) error {
...
func binaryUpgrade(ctx context.Context, r update.UpdateResult, profile system.PlatformProfile) error {
	...
	if r.Tool.Name == "dxrk-memory" {
		return dxrkMemoryBinaryUpgrade(profile)
	}
	...
	if profile.OS == osWindows {
		...
		hint := r.UpdateHint
		if hint == "" {
			hint = fmt.Sprintf("Download manually from https://github.com/Dxrk777/%s/releases", r.Tool.Repo)
		}
		return &ManualFallbackError{
...
// dxrkMemoryBinaryUpgrade downloads the latest dxrk-memory binary using its dedicated
...
func dxrkMemoryBinaryUpgrade(profile system.PlatformProfile) error {
	binaryPath, err := dxrkMemoryDownloadFn(profile)
	if err != nil {
...
// This is used for tools that distribute via shell scripts (e.g., Dxrk Guardian) rather than
...
// dxrkGuardianMkdirTemp is the function used to create a temporary directory for GGA git clone.
// Package-level var for testability — swapped in tests to control the temp dir path.
var dxrkGuardianMkdirTemp = func() (string, error) {
	return os.MkdirTemp("", "dxrk-dxrk-guardian-*")
}

// dxrkGuardianScriptUpgrade upgrades GGA by cloning its repository and running install.sh
// from within the cloned repo — the same approach used by the initial install resolver.
//
// This is required because Dxrk Guardian's install.sh references $SCRIPT_DIR/bin/dxrk-guardian and
...
func dxrkGuardianScriptUpgrade(ctx context.Context, r update.UpdateResult) error {
	return dxrkGuardianScriptUpgradeForOS(ctx, r, detectOS())
}
...
// dxrkGuardianScriptUpgradeForOS is the testable version of dxrkGuardianScriptUpgrade that accepts
// an explicit OS string so tests can simulate Windows without actually running on it.
func dxrkGuardianScriptUpgradeForOS(_ context.Context, r update.UpdateResult, osName string) error {
	if osName == osWindows {
		hint := r.UpdateHint
		if hint == "" {
			hint = fmt.Sprintf("Download manually from https://github.com/%s/%s/releases", r.Tool.Owner, r.Tool.Repo)
	...
	tmpDir, err := dxrkGuardianMkdirTemp()
	if err != nil {
		return fmt.Errorf("create temp dir for dxrk-guardian clone: %w", err)
	}
