...
func detectInstalledVersion(ctx context.Context, tool ToolInfo, currentBuildVersion string) string {
	...
	// Apply a bounded timeout so a hanging binary (e.g. dxrk-memory stuck on DB
...
func detectOpenCodePluginPackage(pkg string) (string, bool) {
	...
	data, err := os.ReadFile(filepath.Join(opencodeDir, "node_modules", pkg, "package.json")) //nolint:gosec
	if err != nil {
