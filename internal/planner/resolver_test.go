...
func TestResolverAddsMissingDependenciesInOrder(t *testing.T) {
	...
	if !reflect.DeepEqual(plan.OrderedComponents, []model.ComponentID{model.ComponentDxrkMemory, model.ComponentSDD, model.ComponentSkills}) {
		t.Fatalf("Resolve() ordered components = %v", plan.OrderedComponents)
	}

	if !reflect.DeepEqual(plan.AddedDependencies, []model.ComponentID{model.ComponentDxrkMemory, model.ComponentSDD}) {
...
func TestResolverPersonaOrderedBeforeEngramAndSDDWhenSelected(t *testing.T) {
	...
	if !reflect.DeepEqual(plan.OrderedComponents, []model.ComponentID{model.ComponentPersona, model.ComponentDxrkMemory, model.ComponentSDD}) {
		t.Fatalf("Resolve() ordered components = %v", plan.OrderedComponents)
	}

	if !reflect.DeepEqual(plan.AddedDependencies, []model.ComponentID{model.ComponentDxrkMemory}) {
		t.Fatalf("Resolve() added dependencies = %v", plan.AddedDependencies)
	}
}
...
func TestResolverEngramOnlyDoesNotForcePersona(t *testing.T) {
	...
	if !reflect.DeepEqual(plan.OrderedComponents, []model.ComponentID{model.ComponentDxrkMemory}) {
		t.Fatalf("Resolve() ordered components = %v", plan.OrderedComponents)
	}
...
func TestResolverPersonaAndEngramWithoutSDD(t *testing.T) {
	...
	selection := model.Selection{
		Components: []model.ComponentID{model.ComponentPersona, model.ComponentDxrkMemory},
	}
	...
	if !reflect.DeepEqual(plan.OrderedComponents, []model.ComponentID{model.ComponentPersona, model.ComponentDxrkMemory}) {
		t.Fatalf("Resolve() ordered components = %v, want [persona, dxrk-memory]", plan.OrderedComponents)
	}
