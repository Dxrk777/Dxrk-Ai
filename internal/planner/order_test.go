...
func TestApplySoftOrderingReordersWithoutAddingDependencies(t *testing.T) {
	...
	result := applySoftOrdering(ordered, [][2]model.ComponentID{{model.ComponentPersona, model.ComponentDxrkMemory}})
	...
	result = applySoftOrdering([]model.ComponentID{model.ComponentDxrkMemory}, [][2]model.ComponentID{{model.ComponentPersona, model.ComponentDxrkMemory}})
	if !reflect.DeepEqual(result, []model.ComponentID{model.ComponentDxrkMemory}) {
...
func TestApplySoftOrderingEdgeCases(t *testing.T) {
	pair := [][2]model.ComponentID{{model.ComponentPersona, model.ComponentDxrkMemory}}
	...
	already := []model.ComponentID{model.ComponentPersona, model.ComponentDxrkMemory}
	result = applySoftOrdering(already, pair)
	if !reflect.DeepEqual(result, []model.ComponentID{model.ComponentPersona, model.ComponentDxrkMemory}) {
		t.Fatalf("already correct: expected [persona, dxrk-memory], got %v", result)
	}

	// Input slice must NOT be mutated
	input := []model.ComponentID{model.ComponentDxrkMemory, model.ComponentPersona}
	_ = applySoftOrdering(input, pair)
	if !reflect.DeepEqual(input, []model.ComponentID{model.ComponentDxrkMemory, model.ComponentPersona}) {
...
func TestApplySoftOrderingBothMVPPairsWithFullSelection(t *testing.T) {
	...
	personaIdx, dxrkMemoryIdx, sddIdx := -1, -1, -1
	...
	if personaIdx < 0 || dxrkMemoryIdx < 0 || sddIdx < 0 {
		t.Fatalf("missing components in result: %v", result)
	}
	if personaIdx > dxrkMemoryIdx {
		t.Fatalf("Persona (%d) must be before Engram (%d), got %v", personaIdx, dxrkMemoryIdx, result)
	}
	...
	if dxrkMemoryIdx > sddIdx {
		t.Fatalf("Engram (%d) must remain before SDD (%d) after soft reorder, got %v", dxrkMemoryIdx, sddIdx, result)
	}
