...
func TestComponentApplyStepOpenClawWorkspaceScopedInjections(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "dxrk-memory writes protocol to workspace AGENTS",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.component == model.ComponentDxrkMemory {
			...
			body, err := os.ReadFile(workspaceFile) //nolint:gosec
			if err != nil {
...
func TestComponentSyncStepOpenClawWorkspaceScopedInjections(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "dxrk-memory sync writes protocol to workspace AGENTS",
			component: model.ComponentDxrkMemory,
			fileName:  "AGENTS.md",
			marker:    "<!-- dxrk:dxrk-memory-protocol -->",
		},
...
func TestInstallRuntimeOpenClawUsesConfiguredActiveWorkspace(t *testing.T) {
	...
	selection := model.Selection{
		Agents:     []model.AgentID{model.AgentOpenClaw},
		Components: []model.ComponentID{model.ComponentPersona, model.ComponentSDD, model.ComponentDxrkMemory},
...
func writeOpenClawConfigWithWorkspace(t *testing.T, home, workspace string) {
	...
	if err := os.WriteFile(filepath.Join(configDir, "openclaw.json"), []byte(config), 0o600); err != nil {
...
func assertOpenClawInstructionsInWorkspace(t *testing.T, workspace string) {
	...
	for _, want := range []string{"dxrk:dxrk-memory-protocol", "dxrk:sdd-orchestrator", "dxrk:strict-tdd-mode"} {
	...
	if !strings.Contains(soulText, "dxrk:persona") || !strings.Contains(soulText, "Senior Architect") {
		t.Fatalf("active workspace SOUL.md missing Dxrk AI persona; got:\n%s", soulText)
	}
}
...
func assertOpenClawEngramMCPInGlobalConfig(t *testing.T, home string) {
	...
	dxrkMemoryServer := objectAtOpenClawTest(t, servers, "dxrk-memory")
	command, ok := dxrkMemoryServer["command"].(string)
	if !ok || filepath.Base(command) != "dxrk-memory" {
		t.Fatalf("global OpenClaw Engram command = %#v, want dxrk-memory executable", dxrkMemoryServer["command"])
	}
}
...
func assertNoOpenClawEngramMCPInWorkspaceConfig(t *testing.T, workspace string) {
	...
	if _, ok := servers["dxrk-memory"]; ok {
		t.Fatalf("workspace OpenClaw config %q must not contain mcp.servers.dxrk-memory", configPath)
	}
}
