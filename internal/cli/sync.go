...
import (
	...
	"github.com/Dxrk777/Dxrk-Ai/internal/components/internalmcp"
...
//
// Default sync scope: SDD, DxrkMemory, Context7, GGA, Skills, Persona.
...
// <!-- dxrk:persona --> markers — that block is harness-managed and
...
func BuildSyncSelection(flags SyncFlags, agentIDs []model.AgentID) model.Selection {
	// Order matters: Persona must run BEFORE SDD/DxrkMemory/MCP because those
	...
	skillIDs := make([]model.SkillID, 0, len(flags.Skills))
	...
	return model.Selection{
		...
		// PersonaDxrk. This avoids regenerating a Dxrk persona on
...
func (r *syncRuntime) stagePlan() pipeline.StagePlan {
	...
	apply := make([]pipeline.Step, 0, 1+len(r.selection.Components))
...
//   - Step 3: Dxrk output-style overlay (only when the agent supports it).
//
...
func syncPersonaPathsWithWorkspace(homeDir, workspaceDir string, selection model.Selection, adapters []agents.Adapter) []string {
	...
	for _, adapter := range adapters {
		...
		if selection.Persona == model.PersonaDxrk && adapter.SupportsOutputStyles() {
...
func (s componentSyncStep) Run() error {
	...
	case model.ComponentDxrkMemory:
		...
		for _, adapter := range adapters {
			var res dxrkmemory.InjectionResult
			var err error
			if adapter.Agent() == model.AgentOpenClaw {
				res, err = dxrkmemory.InjectWithPromptDir(s.homeDir, s.workspaceDir, adapter)
			} else {
				targetDir := componentInjectionDir(s.homeDir, s.workspaceDir, adapter)
				res, err = dxrkmemory.Inject(targetDir, adapter)
			}
			if err != nil {
	...
	case model.ComponentDxrkGuardian:
		...
		if err := dxrkguardian.EnsureRuntimeAssets(s.homeDir); err != nil {
			return fmt.Errorf("sync dxrk-guardian runtime assets: %w", err)
		}
		...
		res, err := dxrkguardian.Inject(s.homeDir, s.agents)
		if err != nil {
			return fmt.Errorf("sync dxrk-guardian config: %w", err)
		}
		// Count Dxrk Guardian files changed based on individual Changed flags.
	...
	case model.ComponentPersona:
		...
		// <!-- dxrk:persona --> markers and (when supported) refreshes
		// the Dxrk output-style overlay. We deliberately skip the
	...
	case model.ComponentChecker:
		res, err := checker.Inject(s.homeDir)
		if err != nil {
			return fmt.Errorf("sync checker: %w", err)
		}
		s.countChanged(boolToInt(res.Changed))
		return nil
	...
	case model.ComponentInternalMCPServer:
		for _, adapter := range adapters {
			res, err := internalmcp.Inject(s.homeDir, adapter)
			if err != nil {
				return fmt.Errorf("sync internal MCP server for %q: %w", adapter.Agent(), err)
			}
			s.countChanged(boolToInt(res.Changed))
		}
		return nil
...
func RunSync(args []string) (SyncResult, error) {
	...
	// have no Persona field. Default to Dxrk so sync still has a target.
	if selection.Persona == "" {
		selection.Persona = model.PersonaDxrk
	}
