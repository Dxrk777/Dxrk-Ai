...
func TestComponentPathsSDDKimiIncludesAgentFilesAndGlobalSkills(t *testing.T) {
	...
	for _, want := range []string{
		...
		filepath.Join(home, ".kimi", "agents", "dxrk.yaml"),
		...
		filepath.Join(home, ".config", "agents", "skills", "_shared", "dxrk-memory-convention.md"),
	} {
		if !containsPath(paths, want) {
...
// for ComponentDxrkMemory + Codex reports ~/.codex/config.toml as a backup target.
func TestComponentPathsEngramCodexIncludesConfigTOML(t *testing.T) {
	home := t.TempDir()
	adapters := resolveAdapters([]model.AgentID{model.AgentCodex})

	paths := componentPaths(home, model.Selection{}, adapters, model.ComponentDxrkMemory)
...
// dxrk-memory component path for OpenClaw always resolves to the canonical
...
func TestComponentPathsEngramOpenClawUsesCanonicalSettingsPath(t *testing.T) {
	...
	paths := componentPathsWithWorkspace(home, workspace, model.Selection{}, adapters, model.ComponentDxrkMemory)

	canonical := filepath.Join(home, ".openclaw", "openclaw.json")
	if !containsPath(paths, canonical) {
		t.Fatalf("componentPathsWithWorkspace(dxrk-memory,openclaw) missing canonical path %q\npaths=%v", canonical, paths)
	}

	wrongPath := filepath.Join(workspace, ".openclaw", "openclaw.json")
	if containsPath(paths, wrongPath) {
		t.Fatalf("componentPathsWithWorkspace(dxrk-memory,openclaw) must not include workspace-scoped path %q\npaths=%v", wrongPath, paths)
	}
}
