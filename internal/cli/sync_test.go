...
func TestBuildSyncSelectionDefaultScopeIncludesManagedComponents(t *testing.T) {
	...
	// Default sync must include: SDD, DxrkMemory, Context7, GGA, Skills, Persona.
	// Persona is included because the content between <!-- dxrk:persona -->
...
func TestBuildSyncSelectionAgentsForwarded(t *testing.T) {
	agentIDs := []model.AgentID{model.AgentClaudeCode, model.AgentOpenCode}
	flags := SyncFlags{}

	sel := BuildSyncSelection(flags, agentIDs)

	if !reflect.DeepEqual(sel.Agents, agentIDs) {
		t.Errorf("Agents = %v, want %v", sel.Agents, agentIDs)
	}
}
...
func TestComponentSyncStepSkipsEngramBinaryInstall(t *testing.T) {
	...
	step := componentSyncStep{
		id:        "sync:dxrk-memory",
		component: model.ComponentDxrkMemory,
		homeDir:   home,
...
func TestComponentSyncStepRunsPersonaInjectForSync(t *testing.T) {
	...
	step := componentSyncStep{
		...
		homeDir:   home,
		agents:    []model.AgentID{model.AgentOpenCode},
		selection: model.Selection{Persona: model.PersonaDxrk},
	}
	...
	body, err := os.ReadFile(agentsMD) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile AGENTS.md: %v", err)
	}
	if !strings.Contains(string(body), "<!-- dxrk:persona -->") {
	...
	if _, err := os.Stat(settings); err == nil {
		raw, _ := os.ReadFile(settings) //nolint:gosec
		if strings.Contains(string(raw), "dxrk") {
			t.Errorf("opencode.json should NOT contain dxrk agent after sync; got:\n%s", string(raw))
		}
	}
}
...
func TestComponentSyncStepRunsGGAInjectWithoutBinaryInstall(t *testing.T) {
	...
	step := componentSyncStep{
		id:        "sync:dxrk-guardian",
		component: model.ComponentDxrkGuardian,
		homeDir:   home,
		agents:    []model.AgentID{model.AgentOpenCode},
	...
	// Dxrk Guardian runtime asset should be written.
...
func TestRunSyncPreservesUnmanagedAdjacentFiles(t *testing.T) {
	...
	if err := os.WriteFile(userConfigPath, []byte(userContent), 0o600); err != nil {
	...
	after, err := os.ReadFile(userConfigPath) //nolint:gosec
	if err != nil {
...
func TestRunSyncIsIdempotent(t *testing.T) {
	...
	contentAfterRun1, err := os.ReadFile(claudeMDPath) //nolint:gosec
	if err != nil {
	...
	contentAfterRun2, err := os.ReadFile(claudeMDPath) //nolint:gosec
	if err != nil {
...
func TestRunSyncExcludesUnmanagedLookalikeFile(t *testing.T) {
	...
	if err := os.MkdirAll(lookalikeDir, 0o750); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	lookalikePath := filepath.Join(lookalikeDir, "AGENTS.md")
	const lookalikeContent = "# My project AGENTS.md — NOT managed by dxrk"
	if err := os.WriteFile(lookalikePath, []byte(lookalikeContent), 0o600); err != nil {
	...
	after, err := os.ReadFile(lookalikePath) //nolint:gosec
	if err != nil {
...
func TestRunSyncWithProfilesIntegration(t *testing.T) {
	...
	settingsPath := filepath.Join(home, ".config", "opencode", "opencode.json")
	settingsData, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
...
func TestRunSyncDetectsExistingProfilesOnRegularSync(t *testing.T) {
	...
	settingsData2, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
...
func TestRunSyncExternalSingleActiveSkipsDetectAndPreservesOrchestratorPrompt(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(home, ".config", "opencode", "node_modules", "unique-names-generator"), 0o750); err != nil {
	...
	if err := os.WriteFile(filepath.Join(profilesDir, "active.json"), []byte(`{"name":"cheap"}`), 0o600); err != nil {
  ...
  "agent": {
    ...
    "dxrk": {"mode": "primary", "description": "revoked OpenCode persona", "prompt": "REVOKED_DXRK_PROMPT_SHOULD_NOT_SURVIVE"},
	...
	if !strings.Contains(settingsText, "Bind this to the dedicated `dxrk-orchestrator` agent only.") {
		t.Fatalf("external-single-active sync did not migrate binding text to dxrk-orchestrator")
	}
	if !strings.Contains(settingsText, "agent.dxrk-orchestrator.model") {
		t.Fatalf("external-single-active sync did not migrate model assignment key to dxrk-orchestrator")
	}
	...
	if strings.Contains(settingsText, "\"dxrk\"") {
		t.Fatalf("external-single-active sync should delete revoked dxrk agent")
	}
	if strings.Contains(settingsText, "REVOKED_DXRK_PROMPT_SHOULD_NOT_SURVIVE") {
		t.Fatalf("external-single-active sync preserved revoked dxrk prompt")
	}
...
func TestRunSyncWithSelection_NoAgentsIsNoOp(t *testing.T) {
	...
	sel := model.Selection{
		Agents:     nil,
		Components: []model.ComponentID{model.ComponentSDD, model.ComponentDxrkMemory},
	}
...
func TestRunSyncWithSelection_WritesExpectedFiles(t *testing.T) {
	...
	sel := model.Selection{
		Agents:     []model.AgentID{model.AgentOpenCode},
		Components: []model.ComponentID{model.ComponentSDD, model.ComponentDxrkMemory, model.ComponentContext7, model.ComponentDxrkGuardian, model.ComponentSkills},
		SDDMode:    model.SDDModeSingle,
	}
...
func TestDiscoverAgentsStateMultipleAgents(t *testing.T) {
	...
	agentIDs := []string{"claude-code", "opencode", "gemini-cli"}
	if err := state.Write(home, state.InstallState{InstalledAgents: agentIDs}); err != nil {
		t.Fatalf("state.Write() error = %v", err)
	}
...
func TestRunSyncRollsBackOnFailure(t *testing.T) {
	...
	// Inject a forced failure by injecting a bad dxrk-guardian step — we use a test
...
func TestSyncPersonaPathsExcludeOpenCodeAgentJson(t *testing.T) {
	...
	paths := syncPersonaPaths(home, model.Selection{Persona: model.PersonaDxrk}, []agents.Adapter{a})
...
func TestRunSyncRegeneratesPersonaBlockBetweenMarkers(t *testing.T) {
	...
	// Write a stale managed persona block — what an older version of dxrk
	...
	if err := os.WriteFile(claudeMD, []byte(stalePersona), 0o600); err != nil {
	...
	body, err := os.ReadFile(claudeMD) //nolint:gosec
	if err != nil {
...
// user installed (from state.json) rather than always defaulting to Dxrk.
...
// TestRunSyncFallsBackToDxrkWhenStateLacksPersona verifies the backward
// compatibility path: state files written before persona persistence still
// produce a working sync.
func TestRunSyncFallsBackToDxrkWhenStateLacksPersona(t *testing.T) {
	home := t.TempDir()
	setSyncTestHome(t, home)
	...
	if got, want := res.Selection.Persona, model.PersonaDxrk; got != want {
