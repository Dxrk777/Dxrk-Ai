...
// TestGGAFixInstallErrorWhenAlreadyAvailable tests that when Dxrk Guardian install
...
func TestGGAFixInstallErrorWhenAlreadyAvailable(t *testing.T) {
	...
	origGGAAvailableCheck := dxrkGuardianAvailableCheck

	t.Cleanup(func() {
		osUserHomeDir = origHome
		cmdLookPath = origCmdLookPath
		runCommand = origRunCmd
		dxrkGuardianAvailableCheck = origGGAAvailableCheck
	})
	...
	// Make dxrkGuardianAvailable return false initially (simulating install needed),
	// then return true after the "install" (simulating GGA was already there)
	dxrkGuardianAvailableCheck = func(profile system.PlatformProfile) bool {
...
// TestGGAFixInstallErrorWhenNotAvailable tests that when Dxrk Guardian install
...
func TestGGAFixInstallErrorWhenNotAvailable(t *testing.T) {
	...
	// Make dxrkGuardianAvailable ALWAYS return false (GGA is not available)
	dxrkGuardianAvailableCheck = func(profile system.PlatformProfile) bool {
		return false
	}
