...
func TestRunInstallEngramForPiAndOpenCodeProvisionsBothMCPTargets(t *testing.T) {
	...
	runCommand = func(name string, args ...string) error {
		...
		// Simulate pi-dxrk-memory init writing mcp.json with the new schema.
		if name == "npm" && len(args) >= 7 && args[5] == "pi-dxrk-memory" && args[6] == "init" {
			mcpPath := filepath.Join(home, ".pi", "agent", "mcp.json")
			if err := os.MkdirAll(filepath.Dir(mcpPath), 0o750); err != nil {
				return err
			}
			if err := os.WriteFile(mcpPath, []byte(`{"activeMCP":"dxrk-memory","mcpServers":{"dxrk-memory":{"command":"node","args":["--eval","require('child_process').spawn('dxrk-memory',['mcp','--tools=agent'],{stdio:'inherit'})"]}}}`+"\n"), 0o600); err != nil {
				return err
			}
		}
		return nil
	...
	assertFileContains(t, filepath.Join(home, ".config", "opencode", "opencode.json"), "dxrk-memory")

	for _, want := range []string{"pi install npm:pi-mcp-adapter", fmt.Sprintf("npm exec --yes --package dxrk-dxrk-memory@%s -- pi-dxrk-memory init", versions.DxrkEngram)} {
...
func TestRunInstallRollsBackOnComponentFailure(t *testing.T) {
	...
	// Use only dxrk-memory (not context7) — context7 injects MCP config into
...
func TestRunInstallLinuxUbuntuWithEngramUsesDirectDownload(t *testing.T) {
	...
	// Override dxrkMemoryDownloadFn to avoid real HTTP calls.
...
func TestRunInstallFedoraQwenEngramSkipsUnsupportedSetupAndWritesSettings(t *testing.T) {
	...
	origDownloadFn := dxrkMemoryDownloadFn
	dxrkMemoryDownloadFn = func(profile system.PlatformProfile) (string, error) {
		return filepath.Join(home, "bin", "dxrk-memory"), nil
	}
	t.Cleanup(func() { dxrkMemoryDownloadFn = origDownloadFn })

	detection := linuxDetectionResult(system.LinuxDistroFedora, "dnf")
	result, err := RunInstall(
		[]string{"--agent", "qwen-code", "--component", "dxrk-memory"},
		detection,
	)
	if err != nil {
		t.Fatalf("RunInstall() error = %v", err)
	...
	for _, cmd := range recorder.get() {
		if strings.Contains(cmd, "dxrk-memory setup qwen-code") {
...
func TestRunInstallMacOSStillResolvesBrewCommands(t *testing.T) {
	...
	if !foundBrew {
		t.Fatalf("expected brew install for macOS dxrk-memory, got commands: %v", commands)
	}
}
...
func TestRunInstallEngramSetupStrictFailsWhenSetupFails(t *testing.T) {
	t.Setenv("DXRK_ENGRAM_SETUP_STRICT", "1")
...
func TestRunInstallAntigravityCopiesGeminiSettingsAfterEngramSetup(t *testing.T) {
	...
	runCommand = func(name string, args ...string) error {
		if name == "dxrk-memory" && len(args) == 2 && args[0] == "setup" && args[1] == "gemini-cli" {
	...
	result, err := RunInstall(
		[]string{"--agent", "antigravity", "--component", "dxrk-memory", "--component", "context7", "--component", "permissions"},
		macOSDetectionResult(),
	)
	if err != nil {
		t.Fatalf("RunInstall() error = %v", err)
...
func TestRunInstallDeduplicatesSharedEngramSetupSlugs(t *testing.T) {
	...
	result, err := RunInstall(
		[]string{"--agent", "gemini-cli", "--agent", "antigravity", "--component", "dxrk-memory", "--component", "context7", "--component", "permissions"},
		macOSDetectionResult(),
	)
	if err != nil {
		t.Fatalf("RunInstall() error = %v", err)
	...
	var setupCount int
	for _, cmd := range recorder.get() {
		if strings.Contains(cmd, "dxrk-memory setup gemini-cli") {
			setupCount++
		}
	}
	if setupCount != 1 {
		t.Fatalf("dxrk-memory setup gemini-cli count = %d, want 1", setupCount)
	}
}
...
func TestRunInstallGGASkipsInstallWhenAlreadyOnPath(t *testing.T) {
	...
	detection := macOSDetectionResult()
	result, err := RunInstall(
		[]string{"--agent", "opencode", "--component", "dxrk-guardian"},
		detection,
	)
	if err != nil {
		t.Fatalf("RunInstall() error = %v", err)
	...
	for _, cmd := range recorder.get() {
		if strings.Contains(cmd, "dxrk-guardian") || strings.Contains(cmd, "dxrk-guardian-angel") {
			t.Fatalf("expected dxrk-guardian install to be skipped, but got command: %s", cmd)
		}
	}

	prModePath := filepath.Join(home, ".local", "share", "dxrk-guardian", "lib", "pr_mode.sh")
	content, err := os.ReadFile(prModePath) //nolint:gosec
	if err != nil {
		t.Fatalf("expected dxrk-guardian runtime asset at %q: %v", prModePath, err)
	}
...
func TestRunInstallGGALinuxIncludesTempCleanupBeforeClone(t *testing.T) {
	...
	for i, cmd := range commands {
		...
		if strings.Contains(cmd, "git clone https://github.com/Dxrk777/dxrk-guardian-angel.git /tmp/dxrk-guardian-angel") {
			cloneIdx = i
		}
	}

	for _, cmd := range commands {
		if strings.Contains(cmd, "dxrk-guardian install") || strings.Contains(cmd, "dxrk-guardian init") {
			t.Fatalf("expected global dxrk-guardian provisioning only, got repo-level command: %s", cmd)
		}
	}
...
func TestRunInstallDryRunMatchesActualInstall(t *testing.T) {
	...
	expectedPaths := make([]string, 0, len(dryResult.Resolved.OrderedComponents)*3)
...
func TestRunInstallUpgradeIdempotency(t *testing.T) {
	...
	dxrkMemoryMCPPath := filepath.Join(home, ".claude", "mcp", "dxrk-memory.json")

	claudeMDAfterRun1, err := os.ReadFile(claudeMDPath) //nolint:gosec
	if err != nil {
		t.Fatalf("run 1: ReadFile(%q) error = %v", claudeMDPath, err)
	}
	dxrkMemoryMCPAfterRun1, err := os.ReadFile(dxrkMemoryMCPPath) //nolint:gosec
	if err != nil {
		t.Fatalf("run 1: ReadFile(%q) error = %v", dxrkMemoryMCPPath, err)
	}
	...
	claudeMDAfterRun2, err := os.ReadFile(claudeMDPath) //nolint:gosec
	if err != nil {
		t.Fatalf("run 2: ReadFile(%q) error = %v", claudeMDPath, err)
	}
	dxrkMemoryMCPAfterRun2, err := os.ReadFile(dxrkMemoryMCPPath) //nolint:gosec
	if err != nil {
		t.Fatalf("run 2: ReadFile(%q) error = %v", dxrkMemoryMCPPath, err)
	}
	...
	if string(dxrkMemoryMCPAfterRun1) != string(dxrkMemoryMCPAfterRun2) {
		t.Errorf("dxrk-memory MCP config changed between run 1 and run 2 (idempotency violation):\n--- run1 ---\n%s\n--- run2 ---\n%s",
			dxrkMemoryMCPAfterRun1, dxrkMemoryMCPAfterRun2)
	}
	...
	dxrkMemoryJSON := string(dxrkMemoryMCPAfterRun2)
	commandCount := strings.Count(dxrkMemoryJSON, `"command"`)
	if commandCount != 1 {
		t.Errorf("dxrk-memory MCP JSON contains %d occurrences of \"command\", want exactly 1:\n%s",
			commandCount, dxrkMemoryJSON)
	}
}
...
func TestRunInstallCustomPresetExplicitComponentsResolveCorrectly(t *testing.T) {
	...
	// Verify persona, skills, context7, dxrk-guardian are NOT in the plan.
	for _, c := range result.Resolved.OrderedComponents {
		switch c {
		case model.ComponentPersona, model.ComponentSkills, model.ComponentContext7, model.ComponentDxrkGuardian:
...
func TestOpenCodePersonaBeforeSDDPreservesAllSections(t *testing.T) {
	...
	content, err := os.ReadFile(agentsMD) //nolint:gosec
	if err != nil {
	...
	// DxrkMemory protocol section must be present
	...
	marker := "<!-- dxrk:dxrk-memory-protocol -->"
	...
	// the canonical dxrk-orchestrator key. Legacy sdd-orchestrator should be
	// migrated away during injection.
	opencodeJSON := filepath.Join(home, ".config", "opencode", "opencode.json")
	jsonContent, err := os.ReadFile(opencodeJSON) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(opencode.json) error = %v", err)
	}
	jsonText := string(jsonContent)
	if !strings.Contains(jsonText, "dxrk-orchestrator") {
		t.Error("opencode.json missing dxrk-orchestrator agent entry (SDD not injected)")
	}
...
func TestRunInstallKimiBootstrapsHub(t *testing.T) {
	...
	content, err := os.ReadFile(hubPath) //nolint:gosec
	if err != nil {
...
func TestRunInstallKimiMissingUVFailsBeforeExecutingInstallCommands(t *testing.T) {
	t.Skip("Skipping due to test environment issues - needs investigation")
	home := t.TempDir()
	restoreHome := osUserHomeDir
	restoreCommand := runCommand
	restoreLookPath := cmdLookPath
