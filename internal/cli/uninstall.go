...
func RunUninstallWithSelectionAndProfiles(homeDir, workspaceDir string, agentIDs []model.AgentID, componentIDs []model.ComponentID, profileNames []string, dxrkMemoryScope model.DxrkMemoryUninstallScope) (componentuninstall.Result, error) {
	agents := make([]string, 0, len(agentIDs))
	for _, agentID := range agentIDs {
		agents = append(agents, string(agentID))
	}
	...
	return componentuninstall.PartialUninstallWithProfileSelection(homeDir, workspaceDir, AppVersion, agents, components, profileNames, dxrkMemoryScope)
}
...
func promptUninstallConfirm(flags UninstallFlags, stdout io.Writer, stdin io.Reader) (bool, error) {
	if flags.All {
		_, _ = fmt.Fprintln(stdout, "This will remove dxrk managed configuration from all supported agents.")
	} else {
		_, _ = fmt.Fprintf(stdout, "This will remove dxrk managed configuration from: %s\n", strings.Join(agentLabelsFromStrings(flags.Agents), ", "))
	}
