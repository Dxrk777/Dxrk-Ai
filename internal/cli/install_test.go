...
func TestNormalizeInstallFlagsCustomAcceptsOptionalDxrkInstallables(t *testing.T) {
	input, err := NormalizeInstallFlags(InstallFlags{
		Preset:     string(model.PresetCustom),
		Components: []string{string(model.ComponentClaudeTheme), string(model.ComponentOpenCodeDxrkLogo)},
	}, system.DetectionResult{})
	if err != nil {
		t.Fatalf("NormalizeInstallFlags() error = %v", err)
	}

	want := []model.ComponentID{model.ComponentClaudeTheme, model.ComponentOpenCodeDxrkLogo}
...
func TestNormalizeInstallFlagsPiOnlyRespectsExplicitComponents(t *testing.T) {
	input, err := NormalizeInstallFlags(InstallFlags{
		Agents:     []string{string(model.AgentPi)},
		Components: []string{string(model.ComponentDxrkMemory)},
	}, system.DetectionResult{})
	if err != nil {
		t.Fatalf("NormalizeInstallFlags() error = %v", err)
	}

	want := []model.ComponentID{model.ComponentDxrkMemory}
	if !reflect.DeepEqual(input.Selection.Components, want) {
		t.Fatalf("components = %#v, want %#v", input.Selection.Components, want)
	}
}
