...
description: >
  ...
  "iniciar sdd", or wants to bootstrap SDD persistence (dxrk-memory, openspec, or hybrid) for the
  first time in a project. Detects tech stack and writes the skill registry.
tools: ["@builtin", "@dxrk-memory"]
model: {{KIRO_MODEL}}
includeMcpJson: true
---
...
2. Initialize the persistence backend (dxrk-memory, openspec, or hybrid — per user preference)
