...
The Claude Code session model is controlled by Claude Code; Dxrk AI only configures models for Agent tool calls to phase sub-agents.
...
DXRK_MEMORY NOTE:
Sub-agents handle persistence automatically. Each phase saves its artifact to dxrk-memory with topic_key "sdd/$ARGUMENTS/{type}" where type is: proposal, spec, design, tasks.
