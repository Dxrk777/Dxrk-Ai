...
- **`dxrk-memory`**: Fast, no files created. Artifacts live in dxrk-memory only.
...
2. Fallback: read `.atl/skill-registry.md` if dxrk-memory is not available
...
- Write context: sub-agent MUST save significant discoveries, decisions, or bug fixes to dxrk-memory via `mem_save` before returning.
- Always add to the sub-agent prompt: `"If you make important discoveries, decisions, or fix bugs, save them to dxrk-memory via mem_save with project: '{project}'."`
