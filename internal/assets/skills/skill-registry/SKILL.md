...
1. Glob for `*/SKILL.md` files across ALL known skill directories. Check every path below — scan ALL that exist, not just the first match:
   ...
   **User-level (global skills):**
   - `~/.pi/agent/skills/` — Pi/Dxrk AI user skills
   ...
   **Project-level (workspace skills):**
   ...
   - `{project-root}/.pi/skills/`, `{project-root}/.agent/skills/`, `{project-root}/.agents/skills/`, `{project-root}/.atl/skills/` — Dxrk AI/Pi and generic workspace skills
...
#### B. If dxrk-memory is available, also save to dxrk-memory (cross-session bonus):
...
`capture_prompt: false` is required because the skill registry is an automated artifact, not a human/proactive memory save. Set it when the Dxrk-Memory tool schema supports it; if an older schema rejects or does not expose the field, omit it rather than failing.
...
**Dxrk-Memory**: {saved / not available}
...
- ALWAYS save to dxrk-memory if the `mem_save` tool is available
