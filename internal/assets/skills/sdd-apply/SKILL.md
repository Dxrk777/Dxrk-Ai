...
- **dxrk-memory**: Read `sdd/{change-name}/proposal`, `sdd/{change-name}/spec`, `sdd/{change-name}/design`, `sdd/{change-name}/tasks` (all required — keep tasks ID for updates). Mark tasks complete via `mem_update(id: {tasks-observation-id}, content: "...")`. Save progress as `sdd/{change-name}/apply-progress`.
- **openspec**: Read and follow `skills/_shared/openspec-convention.md`. Update `tasks.md` with `[x]` marks.
- **hybrid**: Follow BOTH conventions — persist progress to Dxrk-Memory (`mem_update` for tasks) AND update `tasks.md` with `[x]` marks on filesystem.
...
├── dxrk-memory: mem_search("sdd/{project}/testing-capabilities") → mem_get_observation(id)
...
- type: `architecture`
- Also update the tasks artifact with `[x]` marks via `mem_update` (dxrk-memory) or file edit (openspec/hybrid).
