...
## B. Artifact Retrieval (Dxrk-Memory Mode)
...
`capture_prompt: false` is mandatory for SDD artifacts because they are automated pipeline outputs, not human/proactive memory saves. Set it when the Dxrk-Memory tool schema supports it; if an older schema rejects or does not expose the field, omit it rather than failing.
...
**Artifacts**: Dxrk-Memory `sdd/{change-name}/proposal` | `openspec/changes/{change-name}/proposal.md`
