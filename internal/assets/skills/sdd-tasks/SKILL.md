...
- **dxrk-memory**: Read `sdd/{change-name}/proposal` (required), `sdd/{change-name}/spec` (required), `sdd/{change-name}/design` (required). Save as `sdd/{change-name}/tasks`.
- **openspec**: Read and follow `skills/_shared/openspec-convention.md`.
- **hybrid**: Follow BOTH conventions — persist to Dxrk-Memory AND write `tasks.md` to filesystem. Retrieve dependencies from Dxrk-Memory (primary) with filesystem fallback.
...
**IF mode is `dxrk-memory` or `none`:** Do NOT create any `openspec/` directories or files. Compose the tasks content in memory — you will persist it in Step 4.
...
**Change**: {change-name}
**Location**: `openspec/changes/{change-name}/tasks.md` (openspec/hybrid) | Dxrk-Memory `sdd/{change-name}/tasks` (dxrk-memory) | inline (none)
