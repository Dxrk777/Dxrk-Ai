...
- **dxrk-memory**: Read `sdd/{change-name}/proposal` (required). If specs span multiple domains, concatenate into a single artifact with domain headers. Save as `sdd/{change-name}/spec`.
- **openspec**: Read and follow `skills/_shared/openspec-convention.md`.
- **hybrid**: Follow BOTH conventions — persist to Dxrk-Memory (single concatenated artifact) AND write domain files to filesystem.
...
**IF mode is `dxrk-memory`:** Existing specs were already retrieved from Dxrk-Memory in the Persistence Contract. Skip filesystem reads.
...
**IF mode is `dxrk-memory` or `none`:** Do NOT create any `openspec/` directories or files. Compose the spec content in memory — you will persist it in Step 5.
