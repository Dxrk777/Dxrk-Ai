...
- **dxrk-memory**: Read `sdd/{change-name}/explore` (optional) and `sdd-init/{project}` (optional). Save artifact as `sdd/{change-name}/proposal`.
- **openspec**: Read and follow `skills/_shared/openspec-convention.md`.
- **hybrid**: Follow BOTH conventions — persist to Dxrk-Memory AND write to filesystem. Retrieve dependencies from Dxrk-Memory (primary) with filesystem fallback.
...
**IF mode is `dxrk-memory` or `none`:** Do NOT create any `openspec/` directories. Skip this step.
...
**IF mode is `dxrk-memory`:** Existing context was already retrieved from Dxrk-Memory in the Persistence Contract. Skip filesystem reads.
...
**Change**: {change-name}
**Location**: `openspec/changes/{change-name}/proposal.md` (openspec/hybrid) | Dxrk-Memory `sdd/{change-name}/proposal` (dxrk-memory) | inline (none)
