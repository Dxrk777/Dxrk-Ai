...
- Read context: orchestrator searches dxrk-memory (`mem_search`) for relevant prior context and passes it in the subagent invocation message. Sub-agent does NOT search dxrk-memory itself.
...
- Always include in invocation message: `"If you make important discoveries, decisions, or fix bugs, save them to dxrk-memory via mem_save with project: '{project}'."`
...
Convention files under `~/.cursor/skills/_shared/` (global) or `.agent/skills/_shared/` (workspace): `dxrk-memory-convention.md`, `persistence-contract.md`, `openspec-convention.md`.
