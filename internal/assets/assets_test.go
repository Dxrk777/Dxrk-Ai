...
func TestAllEmbeddedAssetsAreReadable(t *testing.T) {
	expectedFiles := []string{
		...
		// Kimi agent files
		"kimi/persona-dxrk.md",
		"kimi/output-style-dxrk.md",
		"kimi/sdd-orchestrator.md",
		"kimi/KIMI.md",
		"kimi/agents/dxrk.yaml",
...
func TestOpenCodeEmbeddedAssetLayout(t *testing.T) {
	...
	for _, name := range []string{"commands", "plugins", "persona-dxrk.md", "sdd-orchestrator.md", "sdd-overlay-single.json", "sdd-overlay-multi.json"} {
		if !seen[name] {
...
func TestClaudeEmbeddedAssetLayout(t *testing.T) {
	...
	for _, name := range []string{"commands", "dxrk-memory-protocol.md", "persona-dxrk.md", "sdd-orchestrator.md"} {
		if !seen[name] {
...
func TestDxrkLanguageInstructionsDoNotBiasEnglishSessions(t *testing.T) {
	personaPaths := []string{
		"claude/persona-dxrk.md",
		"generic/persona-dxrk.md",
		"kiro/persona-dxrk.md",
		"kimi/persona-dxrk.md",
		"opencode/persona-dxrk.md",
	}
	...
	// dxrk-memory-protocol assets must not ship Spanish trigger examples that bias
	// English sessions into Spanish replies (same mechanism as #341 / #350).
	// Covers all agent families that ship a dedicated dxrk-memory instruction asset.
	for _, path := range []string{
		"claude/dxrk-memory-protocol.md",
		"codex/dxrk-memory-instructions.md",
	} {
		t.Run(path, func(t *testing.T) {
			content := MustRead(path)
...
func TestPersonasContainContextualSkillLoadingDirective(t *testing.T) {
	tests := []struct {
		...
		{path: "claude/persona-dxrk.md", isClaude: true, invokeMsg: "invoke it via the built-in `Skill` tool"},
		{path: "opencode/persona-dxrk.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
		{path: "generic/persona-dxrk.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
		{path: "generic/persona-neutral.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
		{path: "kiro/persona-dxrk.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
		{path: "kimi/persona-dxrk.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
	}
	...
	for _, assetPath := range []string{
		...
		t.Run(assetPath, func(t *testing.T) {
			...
			if assetPath == "claude/sdd-orchestrator.md" {
				if !strings.Contains(content, "dedicated") || !strings.Contains(content, "sdd-orchestrator") {
					t.Fatalf("%q missing scoping note", assetPath)
				}
