...
func TestInstallPlannerParityLinuxPreservesComponentOrder(t *testing.T) {
	...
	dxrkMemoryIdx, sddIdx, skillsIdx := -1, -1, -1
	for i, c := range order {
		switch c {
		case "dxrk-memory":
			dxrkMemoryIdx = i
	...
	if dxrkMemoryIdx < 0 || sddIdx < 0 || skillsIdx < 0 {
		t.Fatalf("missing expected components in order: %v", order)
	}
	if dxrkMemoryIdx >= sddIdx || sddIdx >= skillsIdx {
		t.Fatalf("dependency order violated: dxrk-memory@%d sdd@%d skills@%d", dxrkMemoryIdx, sddIdx, skillsIdx)
	}
}
