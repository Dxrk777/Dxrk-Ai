...
func RunArgs(args []string, stdout io.Writer) error {
	...
	if len(args) > 0 {
		...
		case "uninstall":
			...
			return err
		case "query":
			flags, err := cli.ParseQueryFlags(args[1:])
			if err != nil {
				return err
			}
			return cli.RunQuery(flags)
		case "chat":
			apiKey := os.Getenv("ANTHROPIC_API_KEY")
			aiModel := "claude-sonnet-4-20250514"
			return cli.RunChat(apiKey, aiModel)
		case "skill-registry":
			return runSkillRegistry(args[1:], stdout)
		case "completion":
			shell, err := cli.ParseCompletionFlags(args[1:])
			if err != nil {
				return err
			}
			script, err := cli.RunCompletion(shell)
			if err != nil {
				return err
			}
	...
	if len(args) == 0 {
		...
		m.DeleteBackupFn = backup.DeleteBackup
		m.RenameBackupFn = backup.RenameBackup
		m.TogglePinFn = backup.TogglePin
	...
	case "mcp":
		sub, err := cli.ParseMCPFlags(args[1:])
		if err != nil {
			return err
		}
		switch sub {
		case "serve":
			return cli.RunMCPServe(args[2:])
		case "generate-config":
			return nil // handled in ParseMCPFlags
		default:
			return fmt.Errorf("unknown mcp subcommand %q", sub)
		}
	case "check-permission":
		return cli.RunCheckPermission(args[1:])
	default:
		return fmt.Errorf("unknown command %q — run 'dxrk help' for available commands", args[0])
	}
}

func runSkillRegistry(args []string, stdout io.Writer) error {
	if len(args) == 0 || args[0] != "refresh" {
		return fmt.Errorf("usage: dxrk skill-registry refresh [--cwd <dir>] [--force] [--quiet] [--no-gitignore]")
	}
...
func tuiUninstallWithProfiles(homeDir string) tui.UninstallWithProfilesFunc {
	return func(agentIDs []model.AgentID, componentIDs []model.ComponentID, profileNames []string, dxrkMemoryScope model.DxrkMemoryUninstallScope) (componentuninstall.Result, error) {
		workspaceDir, err := os.Getwd()
		if err != nil {
			return componentuninstall.Result{}, fmt.Errorf("resolve workspace directory: %w", err)
		}
		return cli.RunUninstallWithSelectionAndProfiles(homeDir, workspaceDir, agentIDs, componentIDs, profileNames, dxrkMemoryScope)
	}
}
