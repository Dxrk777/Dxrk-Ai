...
//  1. DXRK_SELF_UPDATE_DONE=1 → skip (loop guard)
//  2. DXRK_NO_SELF_UPDATE=1 → skip (opt-out)
...
func selfUpdate(ctx context.Context, version string, profile system.PlatformProfile, stdout io.Writer) error {
	...
	results := updateCheckFiltered(ctx, version, profile, []string{dxrkTool})

	// Find the dxrk result.
	var target *update.UpdateResult
	for i := range results {
		if results[i].Tool.Name == dxrkTool {
			target = &results[i]
			break
		}
	}
	...
	for _, r := range report.Results {
		if r.ToolName == dxrkTool && r.Status == upgrade.UpgradeSucceeded {
			succeeded = true
			break
		}
	}
	...
	executable, err := lookPathFn(dxrkTool)
	if err != nil {
	...
	_ = os.Setenv(envSelfUpdateDone, "1")
