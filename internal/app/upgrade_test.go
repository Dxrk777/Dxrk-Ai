...
func TestRunArgs_UpgradeToolFilter(t *testing.T) {
	...
	out := buf.String()
	// Output should only mention dxrk-memory or no-upgrades, not dxrk or dxrk-guardian.
			...
			results: []upgrade.ToolUpgradeResult{
				{
					...
					Err:        errors.New("brew upgrade dxrk-guardian: exit status 1"),
				},
			},
		...
		},
		{
			...
			results: []upgrade.ToolUpgradeResult{
				...
				},
				{
					...
					NewVersion: "2.0.0",
					Status:     upgrade.UpgradeSkipped,
					ManualHint: "Download from https://github.com/Dxrk777/dxrk-guardian/releases",
				},
			},
