...
// setEnv is a test helper that sets an env var to "1" and registers cleanup to restore it.
func setEnv(t *testing.T, key string) {
	t.Helper()
...
func TestSelfUpdate_UpdateAvailable_CallsUpgradeAndReExec(t *testing.T) {
	...
	// Verify DXRK_SELF_UPDATE_DONE=1 is in the re-exec env.
	found := false
