...
func TestInstallPreservesExistingTUIPluginsAndIsIdempotent(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(configDir, "tui.json"), initial, 0o600); err != nil {
		t.Fatal(err)
	}

	first, err := Install(home, model.OpenCodePluginSDDDxrkMemoryManage)
	if err != nil {
		t.Fatalf("first Install() error = %v", err)
	}
	second, err := Install(home, model.OpenCodePluginSDDDxrkMemoryManage)
	if err != nil {
	...
	data, err := os.ReadFile(filepath.Join(configDir, "tui.json")) //nolint:gosec
	if err != nil {
		t.Fatal(err)
	}
	var root struct {
	...
	want := []string{"existing-plugin", "opencode-sdd-dxrk-memory-manage"}
...
func TestInstallDxrkLogoWritesLocalTUIPluginAndRegistersAbsolutePath(t *testing.T) {
	home := t.TempDir()

	result, err := Install(home, model.OpenCodePluginDxrkLogo)
	if err != nil {
		t.Fatalf("Install() error = %v", err)
	}
	if !result.Changed {
	...
	pluginPath := filepath.Join(home, ".config", "opencode", "tui-plugins", "dxrk-logo.tsx")
	configPath := filepath.Join(home, ".config", "opencode", "tui.json")
	...
	pluginData, err := os.ReadFile(pluginPath) //nolint:gosec
	if err != nil {
