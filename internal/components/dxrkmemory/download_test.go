...
func buildFakeTarGz(t *testing.T, binaryName string) []byte {
	...
	_ = tw.WriteHeader(&tar.Header{Name: binaryName, Mode: 0o750, Size: int64(len(content))})
	_, _ = tw.Write(content)
	_ = tw.Close()
	_ = gw.Close()
	_ = f.Close()

	data, err := os.ReadFile(tarPath) //nolint:gosec
	if err != nil {
...
func buildFakeZip(t *testing.T, binaryName string) []byte {
	...
	data, err := os.ReadFile(zipPath) //nolint:gosec
	if err != nil {
...
func TestEngramAssetURL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			url := dxrkMemoryAssetURL("https://github.com", tt.version, tt.goos, tt.goarch)
			if !strings.Contains(url, tt.wantSubstr) {
				t.Errorf("dxrkMemoryAssetURL(%s, %s) = %q, want it to contain %q", tt.goos, tt.goarch, url, tt.wantSubstr)
			}
			if !strings.HasSuffix(url, tt.wantExt) {
				t.Errorf("dxrkMemoryAssetURL(%s) = %q, want suffix %q", tt.goos, url, tt.wantExt)
			}
		})
	}
}
...
func TestEngramInstallDir(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dir := dxrkMemoryInstallDir(tt.goos)
			if !strings.Contains(dir, tt.wantSubstr) {
				t.Errorf("dxrkMemoryInstallDir(%s) = %q, want it to contain %q", tt.goos, dir, tt.wantSubstr)
			}
		})
	}
}
...
func TestDownloadLatestBinaryLinux(t *testing.T) {
	...
	origClient := dxrkMemoryHTTPClient
	origBaseURL := dxrkMemoryGitHubBaseURL
	dxrkMemoryHTTPClient = server.Client()
	dxrkMemoryGitHubBaseURL = server.URL
	t.Cleanup(func() {
		dxrkMemoryHTTPClient = origClient
		dxrkMemoryGitHubBaseURL = origBaseURL
	})

	// Override install dir to a temp directory (avoids needing root).
	tmpDir := t.TempDir()
	origInstallDirFn := dxrkMemoryInstallDirFn
	dxrkMemoryInstallDirFn = func(goos string) string { return tmpDir }
	t.Cleanup(func() { dxrkMemoryInstallDirFn = origInstallDirFn })
...
func TestDownloadLatestBinaryFallsBackToAnonymousWhenTokenGets403(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/octet-stream")
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write(buildFakeTarGz(t, "dxrk-memory"))
	}))
	defer server.Close()
