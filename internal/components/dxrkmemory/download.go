...
const (
	dxrkMemoryOwner = "Dxrk777"
	dxrkMemoryRepo  = "dxrk-memory"
...
const (
	...
	dxrkMemoryServer   = "dxrk-memory"
)

// Package-level vars for testability.
var (
	dxrkMemoryHTTPClient    = &http.Client{Timeout: 5 * time.Minute}
	dxrkMemoryGitHubBaseURL = "https://github.com"
	dxrkMemoryInstallDirFn  = dxrkMemoryInstallDir
)
...
func DownloadLatestBinary(profile system.PlatformProfile) (string, error) {
	...
	assetURL := dxrkMemoryAssetURL(dxrkMemoryGitHubBaseURL, version, goos, goarch)

	// 3. Determine install directory.
	installDir := dxrkMemoryInstallDirFn(goos)
	...
	binaryName := dxrkMemoryName
	if goos == goosWindows {
		binaryName = dxrkMemoryName + ".exe"
	}
	...
	if strings.HasSuffix(assetURL, ".zip") {
		...
		if err := downloadAndExtractTarGz(assetURL, dxrkMemoryName, outPath); err != nil {
...
func fetchLatestEngramVersion() (string, error) {
	...
	// token is forwarded into our Linux E2E containers, the public dxrk-memory releases
...
func fetchLatestEngramVersionRequest(token string) (string, int, error) {
	apiURL := fmt.Sprintf("%s/repos/%s/%s/releases/latest",
		dxrkMemoryAPIBaseURL(), dxrkMemoryOwner, dxrkMemoryRepo)
	...
	resp, err := dxrkMemoryHTTPClient.Do(req)
	if err != nil {
...
// dxrkMemoryAPIBaseURL returns the GitHub API base URL for fetching release info.
// In tests, the mock server handles both API and download under the same URL,
// so we derive the API base from dxrkMemoryGitHubBaseURL.
func dxrkMemoryAPIBaseURL() string {
	base := dxrkMemoryGitHubBaseURL
...
// dxrkMemoryAssetURL constructs the download URL for the dxrk-memory release asset.
func dxrkMemoryAssetURL(baseURL, version, goos, goarch string) string {
	ext := ".tar.gz"
	if goos == goosWindows {
		ext = ".zip"
	}
	filename := fmt.Sprintf("%s_%s_%s_%s%s", dxrkMemoryRepo, version, goos, goarch, ext)
	return fmt.Sprintf("%s/%s/%s/releases/download/v%s/%s",
		baseURL, dxrkMemoryOwner, dxrkMemoryRepo, version, filename)
}

// dxrkMemoryInstallDir returns the directory where the dxrk-memory binary should be installed
// for the given OS.
//   - Linux/macOS: /usr/local/bin (fallback: ~/.local/bin if not writable)
//   - Windows: %LOCALAPPDATA%\dxrk-memory\bin
func dxrkMemoryInstallDir(goos string) string {
	if goos == goosWindows {
...
func downloadAndExtractZip(url, binaryName, outPath string) error {
	...
	for _, f := range zr.File {
		if filepath.Base(f.Name) == binaryName && !f.FileInfo().IsDir() {
			...
			err = writeExecutable(rc, outPath)
			_ = rc.Close()
			if err != nil {
...
// currently running (e.g. dxrk-memory as an MCP server). The rename trick works
...
func writeExecutable(r io.Reader, outPath string) error {
	...
	tmp, err := os.CreateTemp(dir, ".dxrk-memory-upgrade-*.tmp")
	if err != nil {
	...
	if err := os.Chmod(tmpPath, 0o750); err != nil { //nolint:gosec
