...
type Service struct {
	...
	// dxrkMemoryUninstallScope controls whether Dxrk Memory cleanup removes global
	// integration files/config (global) or project-local .dxrk-memory data only.
	dxrkMemoryUninstallScope model.DxrkMemoryUninstallScope
}
...
var (
	allManagedComponents = []model.ComponentID{
		...
		model.ComponentOpenCodeDxrkLogo,
		model.ComponentDxrkGuardian,
	}
...
func PartialUninstallWithProfileSelection(homeDir, workspaceDir, appVersion string, agentIDs []string, componentIDs []string, profileNames []string, dxrkMemoryScope model.DxrkMemoryUninstallScope) (Result, error) {
	svc, err := NewService(homeDir, workspaceDir, appVersion)
	if err != nil {
		return Result{}, err
	}
	...
	return svc.PartialUninstallWithProfiles(agentsTyped, componentsTyped, profileNames, dxrkMemoryScope)
}
...
func (s *Service) PartialUninstall(agentIDs []model.AgentID, componentIDs []model.ComponentID) (Result, error) {
	...
	s.dxrkMemoryUninstallScope = model.DxrkMemoryUninstallScopeGlobal
...
func (s *Service) PartialUninstallWithProfiles(agentIDs []model.AgentID, componentIDs []model.ComponentID, profileNames []string, dxrkMemoryScope model.DxrkMemoryUninstallScope) (Result, error) {
	s.SetProfileNamesToRemove(profileNames)
	s.SetDxrkMemoryUninstallScope(dxrkMemoryScope)
	defer func() {
		s.profileNamesToRemove = nil
		s.profileSelectionScoped = false
		s.dxrkMemoryUninstallScope = model.DxrkMemoryUninstallScopeGlobal
...
func (s *Service) SetDxrkMemoryUninstallScope(scope model.DxrkMemoryUninstallScope) {
	if scope == model.DxrkMemoryUninstallScopeProject {
		s.dxrkMemoryUninstallScope = model.DxrkMemoryUninstallScopeProject
		return
	}
	s.dxrkMemoryUninstallScope = model.DxrkMemoryUninstallScopeGlobal
}
...
func (s *Service) CompleteUninstall() (Result, error) {
	...
	result.ManualActions = append(result.ManualActions, "To completely remove dxrk from your system, delete the executable (e.g., rm -f $(which dxrk))")
	return result, nil
}
		...
		for _, componentID := range componentIDs {
			...
			for _, op := range ops {
				...
				if existing, ok := operationsByKey[key]; ok && op.typeID == opRewriteFile {
					...
					// mutations apply (e.g. persona + dxrk-memory on system prompt).
...
func (s *Service) componentOperations(adapter agents.Adapter, componentID model.ComponentID) ([]operation, []string, error) {
	...
	case model.ComponentPersona:
		...
		if adapter.SupportsOutputStyles() {
			path := filepath.Join(adapter.OutputStyleDir(homeDir), "dxrk.md")
			targets = append(targets, path)
		...
		if path := adapter.SettingsPath(homeDir); path != "" {
			...
			if adapter.Agent() == model.AgentOpenCode {
				jsonPaths = append(jsonPaths, jsonPath{uninstallAgentKey, "dxrk"})
			}
	...
	case model.ComponentDxrkMemory:
		if s.dxrkMemoryUninstallScope == model.DxrkMemoryUninstallScopeProject {
			projectDataPath := filepath.Join(s.workspaceDir, ".dxrk-memory")
		...
		targets = append(targets, dxrkMemoryTargets(adapter, homeDir)...)
		ops = append(ops, dxrkMemoryOperations(adapter, homeDir)...)
		if adapter.SupportsSystemPrompt() {
			path := adapter.SystemPromptFile(homeDir)
			targets = append(targets, path)
			ops = append(ops, rewriteMarkdownFile(path, func(content string) (string, bool) {
				return removeMarkdownSections(content, "dxrk-memory-protocol")
			}))
		}
	...
	case model.ComponentClaudeTheme:
		if adapter.Agent() == model.AgentClaudeCode {
			path := filepath.Join(homeDir, ".claude", "themes", "dxrk.json")
			targets = append(targets, path)
			ops = append(ops, removeFile(path), removeDirIfEmpty(filepath.Dir(path)))
		}
	case model.ComponentOpenCodeDxrkLogo:
		pluginPath := filepath.Join(homeDir, ".config", "opencode", "tui-plugins", "dxrk-logo.tsx")
	...
	case model.ComponentSDD:
		...
		if path := adapter.SettingsPath(homeDir); path != "" && adapter.Agent() == model.AgentOpenCode {
			...
			for _, agentKey := range sddPhaseAgents {
				paths = append(paths, jsonPath{uninstallAgentKey, agentKey})
			}
			...
			modelVariantsCacheDir := filepath.Join(homeDir, ".dxrk", "cache")
		...
		if wfCap, ok := adapter.(workflowCapability); ok && wfCap.SupportsWorkflows() && s.workspaceDir != "" {
			workflowsDir := wfCap.WorkflowsDir(s.workspaceDir)
			entries, err := fs.ReadDir(assets.FS, wfCap.EmbeddedWorkflowsDir())
			if err != nil {
	...
	case model.ComponentDxrkGuardian:
		...
		ops = append(ops, removeDirIfEmpty(filepath.Dir(dxrkguardian.ConfigPath(homeDir))))
	default:
...
func context7Targets(adapter agents.Adapter, homeDir string) []string {
	...
	case model.StrategySeparateMCPFiles:
		return []string{adapter.MCPConfigPath(homeDir, uninstallContext7Key)}
...
func context7Operations(adapter agents.Adapter, homeDir string) []operation {
	switch adapter.MCPStrategy() {
	case model.StrategySeparateMCPFiles:
		path := adapter.MCPConfigPath(homeDir, uninstallContext7Key)
		return []operation{removeFile(path), removeDirIfEmpty(filepath.Dir(path))}
	case model.StrategyMergeIntoSettings:
		path := adapter.SettingsPath(homeDir)
		if adapter.Agent() == model.AgentOpenCode {
			return []operation{rewriteJSONFile(path, jsonPath{"mcp", uninstallContext7Key})}
		}
		return []operation{rewriteJSONFile(path, jsonPath{uninstallMCPServersKey, uninstallContext7Key})}
	case model.StrategyMCPConfigFile:
		path := adapter.MCPConfigPath(homeDir, uninstallContext7Key)
		switch adapter.Agent() {
		case model.AgentVSCodeCopilot:
			return []operation{rewriteJSONFile(path, jsonPath{"servers", uninstallContext7Key})}
...
func dxrkMemoryTargets(adapter agents.Adapter, homeDir string) []string {
	targets := make([]string, 0, 3)
	switch adapter.MCPStrategy() {
	case model.StrategySeparateMCPFiles:
		targets = append(targets, adapter.MCPConfigPath(homeDir, uninstallDxrkMemoryKey))
	case model.StrategyMergeIntoSettings:
	...
	case model.StrategyTOMLFile:
		targets = append(targets,
			adapter.MCPConfigPath(homeDir, uninstallDxrkMemoryKey),
			filepath.Join(homeDir, ".codex", uninstallDxrkMemoryKey+"-instructions.md"),
			filepath.Join(homeDir, ".codex", uninstallDxrkMemoryKey+"-compact-prompt.md"),
		)
	}
	return targets
}

func dxrkMemoryOperations(adapter agents.Adapter, homeDir string) []operation {
	switch adapter.MCPStrategy() {
	case model.StrategySeparateMCPFiles:
		path := adapter.MCPConfigPath(homeDir, uninstallDxrkMemoryKey)
		return []operation{removeFile(path), removeDirIfEmpty(filepath.Dir(path))}
	case model.StrategyMergeIntoSettings:
		path := adapter.SettingsPath(homeDir)
		if adapter.Agent() == model.AgentOpenCode {
			return []operation{rewriteJSONFile(path, jsonPath{"mcp", uninstallDxrkMemoryKey})}
		}
		return []operation{rewriteJSONFile(path, jsonPath{uninstallMCPServersKey, uninstallDxrkMemoryKey})}
	case model.StrategyMCPConfigFile:
		path := adapter.MCPConfigPath(homeDir, uninstallDxrkMemoryKey)
		if adapter.Agent() == model.AgentVSCodeCopilot {
			return []operation{rewriteJSONFile(path, jsonPath{"servers", uninstallDxrkMemoryKey})}
		}
		return []operation{rewriteJSONFile(path, jsonPath{uninstallMCPServersKey, uninstallDxrkMemoryKey})}
	case model.StrategyTOMLFile:
		configPath := adapter.MCPConfigPath(homeDir, uninstallDxrkMemoryKey)
		instructionsPath := filepath.Join(homeDir, ".codex", uninstallDxrkMemoryKey+"-instructions.md")
		compactPath := filepath.Join(homeDir, ".codex", uninstallDxrkMemoryKey+"-compact-prompt.md")
		return []operation{
...
func rewriteMarkdownFile(path string, mutate func(content string) (string, bool)) operation {
	return operation{
		...
		apply: func(path string) (bool, bool, error) {
			...
			_, err = filemerge.WriteFileAtomic(path, []byte(updated), 0o600)
			if err != nil {
				return false, false, err
			}
...
func rewriteJSONFile(path string, jsonPaths ...jsonPath) operation {
	return operation{
		...
		apply: func(path string) (bool, bool, error) {
			...
			_, err = filemerge.WriteFileAtomic(path, updated, 0o600)
			if err != nil {
				return false, false, err
			}
			return true, false, nil
	...
	for _, hookKey := range []string{"UserPromptSubmit", "SessionStart"} {
		...
		for _, entry := range entries {
			...
			for _, hook := range hooks {
				...
				if ok && strings.Contains(cmd, "dxrk skill-registry refresh") {
					changed = true
					continue
				}
...
func globalBackupTargets(homeDir string) []string {
	return []string{
		dxrkguardian.ConfigPath(homeDir),
		dxrkguardian.AgentsTemplatePath(homeDir),
	}
}
