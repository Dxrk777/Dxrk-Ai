...
func TestExecutePlanReportsManualCleanupForNonEmptyDirectory(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(nonEmptyDir, "user-skill.md"), []byte("keep me"), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	statePath := filepath.Join(homeDir, ".dxrk", "state.json")
	if err := os.MkdirAll(filepath.Dir(statePath), 0o750); err != nil {
		t.Fatalf("MkdirAll(state dir) error = %v", err)
	}
	if err := os.WriteFile(statePath, []byte(`{"installed_agents":[]}`), 0o600); err != nil {
...
func TestComponentOperationsSDD_ClaudeRemovesManagedCommandFiles(t *testing.T) {
	...
	for _, name := range managed {
		if err := os.WriteFile(filepath.Join(commandsDir, name), []byte(name), 0o600); err != nil {
			t.Fatalf("WriteFile(%s) error = %v", name, err)
		}
	}
	customPath := filepath.Join(commandsDir, "my-custom-command.md")
	if err := os.WriteFile(customPath, []byte("keep"), 0o600); err != nil {
...
func TestComponentOperationsSDD_OpenCodeRemovesManagedPluginSourcesAndModelVariantsCache(t *testing.T) {
	...
	if err := os.WriteFile(thirdPartyPluginPath, []byte("third-party"), 0o600); err != nil {
		t.Fatalf("WriteFile(%q) error = %v", thirdPartyPluginPath, err)
	}

	cacheDir := filepath.Join(homeDir, ".dxrk", "cache")
...
func TestComponentOperationsEngram_ProjectScopeRemovesWorkspaceDataOnly(t *testing.T) {
	...
	if err := os.WriteFile(settingsPath, []byte(`{"mcp":{"dxrk-memory":{"command":["dxrk-memory"]}}}`), 0o600); err != nil {
		t.Fatalf("WriteFile(settings) error = %v", err)
	}

	projectDataDir := filepath.Join(workspaceDir, ".dxrk-memory")
	if err := os.MkdirAll(projectDataDir, 0o750); err != nil {
		t.Fatalf("MkdirAll(projectDataDir) error = %v", err)
	}
	if err := os.WriteFile(filepath.Join(projectDataDir, "memory.db"), []byte("db"), 0o600); err != nil {
		t.Fatalf("WriteFile(memory.db) error = %v", err)
	}

	svc.SetDxrkMemoryUninstallScope(model.DxrkMemoryUninstallScopeProject)

	ops, _, err := svc.componentOperations(adapter, model.ComponentDxrkMemory)
	if err != nil {
		t.Fatalf("componentOperations() error = %v", err)
	}
	...
	if _, err := os.Stat(projectDataDir); !os.IsNotExist(err) {
		t.Fatalf("project .dxrk-memory dir should be removed; err = %v", err)
	}
	...
	if !strings.Contains(string(raw), `"dxrk-memory"`) {
		t.Fatalf("global dxrk-memory config should be preserved in project scope, got: %s", string(raw))
	}
}
...
func TestComponentOperationsEngram_GlobalScopeKeepsWorkspaceProjectData(t *testing.T) {
	...
	svc.SetDxrkMemoryUninstallScope(model.DxrkMemoryUninstallScopeGlobal)
	...
	if _, err := os.Stat(projectDataDir); err != nil {
		t.Fatalf("project .dxrk-memory dir should be preserved in global scope, err = %v", err)
	}
	...
	if strings.Contains(string(raw), `"dxrk-memory"`) {
		t.Fatalf("global dxrk-memory config should be removed in global scope, got: %s", string(raw))
	}
}
...
func TestComponentOperationsSDD_ClaudeRemovesSkillRegistryHook(t *testing.T) {
  ...
  "hooks": {
    "UserPromptSubmit": [
      {
        "matcher": "",
        "hooks": [
          {"type": "command", "command": "dxrk skill-registry refresh --quiet --no-gitignore --cwd \"${CLAUDE_PROJECT_DIR:-$PWD}\" || true"},
	...
	text := string(raw)
	if strings.Contains(text, "dxrk skill-registry refresh") {
