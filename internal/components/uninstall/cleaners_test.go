...
func FuzzNormalizeJSON_NoPanic(f *testing.F) {
	seeds := [][]byte{
		[]byte(`{"mcpServers":{"dxrk-memory":{"command":"dxrk-memory"}}}`),
		[]byte("{\n  // comment\n  \"mcpServers\": {\n    \"dxrk-memory\": {\n      \"command\": \"dxrk-memory\",\n    },\n  },\n}\n"),
...
func TestRemoveMarkdownSections_RemovesOnlyManagedBlock(t *testing.T) {
	...
	updated, changed := removeMarkdownSections(input, "dxrk-memory-protocol")
	if !changed {
		t.Fatal("removeMarkdownSections() changed = false, want true")
	}
	if strings.Contains(updated, "dxrk:dxrk-memory-protocol") {
...
func TestRemoveManagedPersonaPreamble_PreservesManagedSuffix(t *testing.T) {
	...
	if strings.Contains(updated, "name: Dxrk AI Persona") || strings.Contains(updated, "## Personality") {
		t.Fatalf("managed persona preamble still present:\n%s", updated)
	}
	if !strings.HasPrefix(updated, "<!-- dxrk:sdd-orchestrator -->") {
...
`)
	...
	updated, changed, err := removeJSONPaths(input, jsonPath{"mcpServers", "dxrk-memory"})
	if err != nil {
		t.Fatalf("removeJSONPaths() error = %v", err)
	}
	if !changed {
	...
	if _, exists := mcpServers["dxrk-memory"]; exists {
		t.Fatalf("dxrk-memory should be removed: %#v", mcpServers)
	}
	if _, exists := mcpServers["custom"]; !exists {
...
func TestCleanCodexTOML_RemovesOnlyManagedEntries(t *testing.T) {
	input := strings.Join([]string{
		"model_instructions_file = \"/home/me/.codex/dxrk-memory-instructions.md\"",
		"experimental_compact_prompt_file = \"/home/me/.codex/dxrk-memory-compact-prompt.md\"",
		"custom_top = \"keep\"",
		"",
	...
	if strings.Contains(updated, "[mcp_servers.dxrk-memory]") {
		t.Fatalf("managed dxrk-memory TOML block was not removed:\n%s", updated)
	}
...
func TestMarkdownCleanup_OnRealFileWithTempDir(t *testing.T) {
	...
	updated, changed := removeMarkdownSections(string(raw), "dxrk-memory-protocol")
	if !changed {
		t.Fatal("removeMarkdownSections() changed = false, want true")
	}
	...
	finalRaw, err := os.ReadFile(path) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(final) error = %v", err)
	}
	final := string(finalRaw)
	if strings.Contains(final, "dxrk:dxrk-memory-protocol") {
...
`)
	...
	updated, changed, err := removeJSONPaths(raw, jsonPath{"outputStyle"}, jsonPath{"mcpServers", "dxrk-memory"})
	if err != nil {
		t.Fatalf("removeJSONPaths() error = %v", err)
	}
	if !changed {
	...
	if err := os.WriteFile(path, updated, 0o600); err != nil { //nolint:gosec
		t.Fatalf("WriteFile(updated) error = %v", err)
	}
	...
	mcpServers := got["mcpServers"].(map[string]any)
	if _, exists := mcpServers["dxrk-memory"]; exists {
		t.Fatalf("dxrk-memory should be removed from file JSON: %#v", mcpServers)
	}
	if _, exists := mcpServers["custom"]; !exists {
