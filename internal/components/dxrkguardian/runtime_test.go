...
func TestEnsureRuntimeAssetsOverwritesStalePRMode(t *testing.T) {
	home := t.TempDir()
	path := RuntimePRModePath(home)
	if err := os.MkdirAll(RuntimeLibDir(home), 0o750); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}

	const stale = "#!/usr/bin/env bash\n# stale-version\n"
	if err := os.WriteFile(path, []byte(stale), 0o750); err != nil { //nolint:gosec
...
func TestEnsureRuntimeAssetsIsNoOpWhenContentMatches(t *testing.T) {
	...
	path := RuntimePRModePath(home)
	contentAfterFirst, err := os.ReadFile(path) //nolint:gosec
	if err != nil {
	...
	contentAfterSecond, err := os.ReadFile(path) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}
...
func TestRuntimeBinDir(t *testing.T) {
	tests := []struct {
		...
		{"/home/user", filepath.Join(".local", "share", "dxrk-guardian", "bin")},
		{"/root", filepath.Join(".local", "share", "dxrk-guardian", "bin")},
	}
...
func TestRuntimePS1Path(t *testing.T) {
	tests := []struct {
		...
		{"/home/user", filepath.Join("bin", "dxrk-guardian.ps1")},
		{"/root", filepath.Join("bin", "dxrk-guardian.ps1")},
	}
	for _, tc := range tests {
...
func TestEnsurePowerShellShimCreatesFileWhenMissing(t *testing.T) {
	...
	if !strings.Contains(text, "Get-Command git") {
		t.Fatalf("dxrk-guardian.ps1 missing expected content, got: %s", text)
	}
}
...
func TestEnsurePowerShellShimOverwritesStaleShim(t *testing.T) {
	...
	if string(content) == stale {
		t.Fatalf("EnsurePowerShellShim did not overwrite stale dxrk-guardian.ps1")
	}
	if !strings.Contains(string(content), "Get-Command git") {
		t.Fatalf("overwritten dxrk-guardian.ps1 missing expected embedded content")
	}
}

// TestEnsurePowerShellShimIsNoOpWhenContentMatches verifies idempotency:
// when dxrk-guardian.ps1 already contains the correct embedded content,
...
// TestAssetGGAPS1IsEmbeddedAndReadable verifies the dxrk-guardian.ps1 asset is
...
func TestAssetGGAPS1IsEmbeddedAndReadable(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("assets.Read(\"dxrkguardian/dxrk-guardian.ps1\") error = %v", err)
	}
	if content == "" {
		t.Fatal("assets.Read(\"dxrkguardian/dxrk-guardian.ps1\") returned empty content")
	}
	if !strings.Contains(content, "Get-Command git") {
		t.Fatalf("embedded dxrk-guardian.ps1 missing expected content, got: %s", content)
	}
}
