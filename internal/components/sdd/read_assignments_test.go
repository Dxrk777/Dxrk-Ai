...
func TestReadCurrentModelAssignments(t *testing.T) {
	...
	content := `{
  "agent": {
    "dxrk-orchestrator": { "model": "anthropic:claude-sonnet-4-20250514" },
	...
	tests := []struct {
		...
		{"dxrk-orchestrator", "anthropic", "claude-sonnet-4-20250514"},
...
func TestReadCurrentModelAssignmentsMapsLegacyOrchestratorToDxrkOrchestrator(t *testing.T) {
	dir := t.TempDir()
	settingsPath := filepath.Join(dir, "opencode.json")
  ...
  "agent": {
	...
	if got["dxrk-orchestrator"] != want {
		t.Fatalf("dxrk-orchestrator assignment = %+v, want %+v", got["dxrk-orchestrator"], want)
	}
}
...
func TestReadCurrentModelAssignmentsPartialModels(t *testing.T) {
	...
	content := `{
  "agent": {
    "dxrk-orchestrator": { "model": "anthropic:claude-opus-4-5" },
	...
	// Only dxrk-orchestrator has a model — only it should appear
	if len(got) != 1 {
		t.Errorf("ReadCurrentModelAssignments() len = %d, want 1; got %v", len(got), got)
	}

	a, ok := got["dxrk-orchestrator"]
	if !ok {
		t.Fatal("dxrk-orchestrator missing from result")
	}
	want := model.ModelAssignment{ProviderID: "anthropic", ModelID: "claude-opus-4-5"}
	if a != want {
		t.Errorf("dxrk-orchestrator assignment = %+v, want %+v", a, want)
	}
}
...
func TestReadCurrentModelAssignmentsMalformedModelField(t *testing.T) {
	...
	content := `{
  "agent": {
    "dxrk-orchestrator": { "model": "no-colon-here" },
	...
	// Malformed dxrk-orchestrator skipped, sdd-apply parsed
	if _, ok := got["dxrk-orchestrator"]; ok {
...
func TestReadCurrentModelAssignmentsSlashSeparator(t *testing.T) {
	...
	content := `{
  "agent": {
    "dxrk-orchestrator": { "model": "zai-coding-plan/glm-5-turbo" }
  }
}`
	if err := os.WriteFile(settingsPath, []byte(content), 0o600); err != nil {
		t.Fatalf("write settings: %v", err)
	...
	a, ok := got["dxrk-orchestrator"]
	if !ok {
		t.Fatal("dxrk-orchestrator missing from result — slash-separated format not parsed")
	}
