...
func TestProfileLifecycle_FullCRUD(t *testing.T) {
  ...
  "model": "anthropic:claude-sonnet-4-5"
	...
	if err := os.WriteFile(settingsPath, []byte(initialJSON), 0o600); err != nil {
	...
	baseJSON, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
	...
	if _, writeErr := filemerge.WriteFileAtomic(settingsPath, merged, 0o600); writeErr != nil {
	...
	baseJSON2, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
	...
	if _, writeErr := filemerge.WriteFileAtomic(settingsPath, merged2, 0o600); writeErr != nil {
...
func TestProfileLifecycle_TwoProfiles(t *testing.T) {
	...
	if err := os.WriteFile(settingsPath, []byte(`{"model": "anthropic:claude-sonnet-4-5"}`), 0o600); err != nil {
