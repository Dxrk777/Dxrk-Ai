...
func TestInjectClaudePreservesExistingSections(t *testing.T) {
	...
	existing := "# My Config\n\nSome user content.\n"
	if err := os.WriteFile(filepath.Join(claudeDir, "CLAUDE.md"), []byte(existing), 0o600); err != nil {
	...
	content, err := os.ReadFile(filepath.Join(claudeDir, "CLAUDE.md")) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}
...
func TestInjectClaudeWritesCommandFiles(t *testing.T) {
	...
	content, err := os.ReadFile(commandPath) //nolint:gosec
	if err != nil {
...
func TestInjectClaudeCustomModelAssignments(t *testing.T) {
	...
	content, err := os.ReadFile(filepath.Join(home, ".claude", "CLAUDE.md")) //nolint:gosec
	if err != nil {
	...
	for _, want := range []string{
		...
		"Dxrk AI does not configure the main orchestrator model",
	} {
	...
	for _, want := range []string{
		...
		"If you are about to call Agent and have not chosen",
	} {
		if !strings.Contains(text, want) {
...
func TestInjectOpenCodeWritesCommandFiles(t *testing.T) {
	...
	settingsContent, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
	...
	if !strings.Contains(settingsText, `"dxrk-orchestrator"`) {
		t.Fatal("opencode.json missing dxrk-orchestrator agent")
	}
	...
	skillContent, err := os.ReadFile(skillPath) //nolint:gosec
	if err != nil {
...
func TestInjectOpenCodePreservesExistingOrchestratorPromptWhenRequested(t *testing.T) {
  ...
  "agent": {
	...
	settingsBytes, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(opencode.json) error = %v", err)
	}
...
func TestInjectOpenCodeMigratesPreservedLegacyOrchestratorPromptReferences(t *testing.T) {
  ...
  "agent": {
	...
	for _, wanted := range []string{
		"Bind this to the dedicated `dxrk-orchestrator` agent only.",
		"agent.dxrk-orchestrator.model",
	} {
		if !strings.Contains(text, wanted) {
...
func TestInjectOpenCodeMigratesLegacyBaseOrchestratorToDxrkOrchestrator(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
  ...
  "agent": {
	...
	dxrkOrchestratorAgent, ok := agentMap["dxrk-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("dxrk-orchestrator agent not found or wrong type")
	}
	if prompt, _ := dxrkOrchestratorAgent["prompt"].(string); prompt != legacyPrompt {
		t.Fatalf("dxrk-orchestrator prompt = %q, want migrated legacy prompt", prompt)
	}
}

func TestInjectOpenCodeMigratesMisnamedDxrkSDDOrchestrator(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
	...
	const priorPrompt = "MISNAMED_DXRK_SDD_ORCHESTRATOR_PROMPT_TO_MIGRATE"
	seed := `{
  "agent": {
    "dxrk": {
      "mode": "primary",
      "description": "Dxrk SDD Orchestrator - coordinates sub-agents",
      "prompt": "` + priorPrompt + `"
    }
  }
}`
	...
	if _, exists := agentMap["dxrk"]; exists {
		t.Fatal("misnamed SDD dxrk agent should be removed")
	}
	dxrkOrchestratorAgent, ok := agentMap["dxrk-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("dxrk-orchestrator agent not found or wrong type")
	}
	if prompt, _ := dxrkOrchestratorAgent["prompt"].(string); prompt != priorPrompt {
		t.Fatalf("dxrk-orchestrator prompt = %q, want migrated misnamed prompt", prompt)
	}
}

func TestInjectOpenCodeDeletesRevokedDxrkAgent(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
  ...
  "agent": {
    "dxrk": {
      ...
      "prompt": "REVOKED_DXRK_PROMPT_SHOULD_NOT_SURVIVE"
    },
    "dxrk-orchestrator": {
      "mode": "primary",
      "prompt": "CURRENT_DXRK_ORCHESTRATOR_PROMPT"
    }
  }
}`
	if err := os.WriteFile(settingsPath, []byte(seed), 0o600); err != nil {
	...
	if _, exists := agentMap["dxrk"]; exists {
		t.Fatal("revoked dxrk agent should be removed")
	}
	dxrkOrchestratorAgent, ok := agentMap["dxrk-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("dxrk-orchestrator agent not found or wrong type")
	}
	if prompt, _ := dxrkOrchestratorAgent["prompt"].(string); prompt != "CURRENT_DXRK_ORCHESTRATOR_PROMPT" {
		t.Fatalf("dxrk-orchestrator prompt = %q, want preserved current prompt", prompt)
	}
}
...
func TestInjectOpenCodeMigratesLegacyAgentsKey(t *testing.T) {
  ...
  "agents": {
	...
	if _, ok := agentMap["dxrk-orchestrator"]; !ok {
		t.Fatal("dxrk-orchestrator agent missing after merge")
	}
...
func TestInjectCursorWritesSDDOrchestratorAndSkills(t *testing.T) {
	...
	content, readErr := os.ReadFile(promptPath) //nolint:gosec
	if readErr != nil {
...
func TestInjectKimiWritesNativeAgentFilesAndGlobalSkills(t *testing.T) {
	...
	sddModule, err := os.ReadFile(sddModulePath) //nolint:gosec
	if err != nil {
	...
	rootAgentPath := filepath.Join(home, ".kimi", "agents", "dxrk.yaml")
	rootAgent, err := os.ReadFile(rootAgentPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(%q) error = %v", rootAgentPath, err)
	}

	rootText := string(rootAgent)
	if !strings.Contains(rootText, "name: dxrk") {
		t.Fatal("dxrk.yaml should define a named root custom agent")
	}
	if strings.Contains(rootText, "kimi_cli.tools.agent:Agent") {
		t.Fatal("dxrk.yaml should inherit Kimi's default tool set instead of hardcoding the old Agent tool path")
	}
	if !strings.Contains(rootText, "../KIMI.md") {
		t.Fatal("dxrk.yaml should load the installed KIMI.md system prompt")
	}
...
func TestInjectFileAppendMigratesLegacyHeading(t *testing.T) {
	...
	if err := os.WriteFile(promptPath, []byte(existing), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
...
func TestInjectFileAppendMigratesFullLegacyOrchestratorBlock(t *testing.T) {
	...
	if strings.Count(text, "<!-- dxrk:dxrk-memory-protocol -->") != 1 {
...
<!-- END:agent-teams-lite -->`
	...
	sddSection := "<!-- dxrk:sdd-orchestrator -->\nYou are a COORDINATOR.\n<!-- /dxrk:sdd-orchestrator -->\n"
...
func TestInjectOpenCodeMultiMode(t *testing.T) {
	...
	// Multi overlay must contain dxrk-orchestrator + 10 sub-agents = 11 agents.
	if len(agentMap) != 11 {
		t.Fatalf("agent count = %d, want 11", len(agentMap))
	}

	// Verify dxrk-orchestrator is present.
	orchestratorRaw, ok := agentMap["dxrk-orchestrator"]
	if !ok {
		t.Fatal("missing dxrk-orchestrator agent")
	}
	orchestratorAgent, ok := orchestratorRaw.(map[string]any)
	if !ok {
		t.Fatalf("dxrk-orchestrator has unexpected type: %T", orchestratorRaw)
	}
	toolsRaw, ok := orchestratorAgent["tools"].(map[string]any)
	if !ok {
		t.Fatalf("dxrk-orchestrator tools has unexpected type: %T", orchestratorAgent["tools"])
	}
	for _, toolName := range []string{"delegate", "delegation_read", "delegation_list"} {
		value, ok := toolsRaw[toolName].(bool)
		if !ok || !value {
			t.Fatalf("dxrk-orchestrator missing multi-mode tool %q", toolName)
		}
	}
	...
	pluginContent, err := os.ReadFile(pluginPath) //nolint:gosec
	if err != nil {
...
func TestInjectOpenCodeMultiModeIdempotent(t *testing.T) {
	...
	pluginPath := filepath.Join(home, ".config", "opencode", "plugins", "background-agents.ts")
	content, err := os.ReadFile(pluginPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(background-agents.ts) error = %v", err)
	}
...
func TestInjectOpenCodeSubagentPromptsStayExecutorScoped(t *testing.T) {
	...
	for _, phase := range []string{"sdd-init", "sdd-explore", "sdd-propose", "sdd-spec", "sdd-design", "sdd-tasks", "sdd-apply", "sdd-verify", "sdd-archive"} {
		...
		promptFileData, readErr := os.ReadFile(promptFilePath) //nolint:gosec
		if readErr != nil {
...
func TestInjectOpenCodeEmptySDDModeDefaultsSingle(t *testing.T) {
	...
	// Empty mode defaults to single — dxrk-orchestrator + 10 sub-agents = 11 agents.
	if _, ok := agentMap["dxrk-orchestrator"]; !ok {
		t.Fatal("missing dxrk-orchestrator agent")
	}
	if len(agentMap) != 11 {
	...
	if mode, _ := orchestratorAgent["mode"].(string); mode != "primary" {
		t.Fatalf("dxrk-orchestrator mode = %q, want %q", mode, "primary")
	}
...
func TestInjectClaudeIgnoresSDDMode(t *testing.T) {
	...
	multiContent, err := os.ReadFile(filepath.Join(home, ".claude", "CLAUDE.md")) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(multi) error = %v", err)
	}
	singleContent, err := os.ReadFile(filepath.Join(homeBaseline, ".claude", "CLAUDE.md")) //nolint:gosec
	if err != nil {
...
func TestInjectOpenCodeSingleToMultiSwitch(t *testing.T) {
	...
	content, _ := os.ReadFile(settingsPath) //nolint:gosec
	...
	if _, ok := agentMap["dxrk-orchestrator"]; !ok {
		t.Fatal("missing dxrk-orchestrator after switch to multi")
	}
	if _, ok := agentMap["sdd-orchestrator"]; ok {
...
func TestInjectClaudeDeduplicatesBareOrchestratorSection(t *testing.T) {
	...
	content, readErr := os.ReadFile(filepath.Join(claudeDir, "CLAUDE.md")) //nolint:gosec
	if readErr != nil {
		t.Fatalf("ReadFile() error = %v", readErr)
	}
...
func TestInjectOpenClawWritesWorkspaceAgentsProtocolSectionsAndNoToolsProtocol(t *testing.T) {
	...
	content, err := os.ReadFile(agentsPath) //nolint:gosec
	if err != nil {
	...
	if strings.Contains(toolsText, "dxrk:sdd-orchestrator") || strings.Contains(toolsText, "Strict TDD Mode") {
	...
	if count := strings.Count(string(updated), "<!-- dxrk:sdd-orchestrator -->"); count != 1 {
...
func TestInjectOpenCodeMultiModeUsesRootModelForUnassignedAgents(t *testing.T) {
	...
	if err := os.WriteFile(settingsPath, []byte(`{"model":"openai/gpt-5"}`), 0o600); err != nil {
		t.Fatalf("WriteFile(opencode.json) error = %v", err)
	}
	...
	for _, phase := range []string{"dxrk-orchestrator", "sdd-init", "sdd-verify"} {
		agentDef, ok := agentMap[phase].(map[string]any)
		if !ok {
			t.Fatalf("phase %q agent not found or wrong type", phase)
		}
...
func TestInjectClaudeDoesNotStripMarkedSection(t *testing.T) {
	...
	after1, err := os.ReadFile(filepath.Join(claudeDir, "CLAUDE.md")) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}
...
func TestInjectOpenCodePluginBunPreferredOverNpm(t *testing.T) {
	...
	npmRun = func(dir string, args ...string) ([]byte, error) {
		...
		if err := os.MkdirAll(nmPath, 0o750); err != nil {
			return nil, err
		}
		return []byte(""), nil
	}
...
func TestInjectModelAssignmentsFunction(t *testing.T) {
  ...
  "agent": {
	...
	initAgent := agentMap["sdd-init"].(map[string]any)
	if m, _ := initAgent["model"].(string); m != "anthropic/claude-sonnet-4-20250514" {
		t.Fatalf("sdd-init model = %q, want %q", m, "anthropic/claude-sonnet-4-20250514")
	}
...
func TestInjectWindsurf_WorkflowContentMatchesAsset(t *testing.T) {
	...
	got, err := os.ReadFile(filepath.Join(workspace, ".windsurf", "workflows", "sdd-new.md")) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}
...
func TestInjectOpenCodeMultiModeWithPreExistingMinimalConfig(t *testing.T) {
	...
	if err := os.WriteFile(settingsPath, []byte(minimal), 0o600); err != nil {
		t.Fatalf("WriteFile(opencode.json) error = %v", err)
	}
	...
	if _, ok := agentMap["dxrk-orchestrator"]; !ok {
		t.Fatal("missing dxrk-orchestrator after merge with pre-existing config")
	}
	if _, ok := agentMap["sdd-orchestrator"]; ok {
...
`
	...
	// All multi-mode agents must be present with dxrk-orchestrator as the base orchestrator.
	for _, agentName := range []string{
		"dxrk-orchestrator", "sdd-init", "sdd-explore", "sdd-propose",
...
// ---------------------------------------------------------------------------
// dxrk-orchestrator agent model assignment from SDD coordinator selection
// ---------------------------------------------------------------------------

// TestInjectOpenCodeMultiModeAssignsDxrkOrchestratorModelFromLegacyOrchestratorKey
// verifies that historical TUI assignments keyed by sdd-orchestrator are
// migrated to the current dxrk-orchestrator base coordinator.
func TestInjectOpenCodeMultiModeAssignsDxrkOrchestratorModelFromLegacyOrchestratorKey(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
	...
	// Pre-existing opencode.json with dxrk-orchestrator agent.
	existing := `{
  "agent": {
    "dxrk-orchestrator": {
      "mode": "primary"
	...
	// dxrk-orchestrator must receive the historical sdd-orchestrator assignment.
	dxrkOrchestratorAgent, ok := agentMap["dxrk-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("dxrk-orchestrator agent not found or wrong type")
	}
	if m, _ := dxrkOrchestratorAgent["model"].(string); m != "openai/gpt-4o" {
		t.Fatalf("dxrk-orchestrator model = %q, want %q", m, "openai/gpt-4o")
	}
}

// TestInjectOpenCodeMultiModeInstallsDxrkOrchestratorWithModel verifies that the base
// SDD overlay owns the dxrk-orchestrator coordinator.
func TestInjectOpenCodeMultiModeInstallsDxrkOrchestratorWithModel(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
...
func TestInjectCursorWritesSubAgentFiles(t *testing.T) {
	...
	for _, phase := range []string{"sdd-explore", "sdd-verify"} {
		content, err := os.ReadFile(filepath.Join(agentsDir, phase+".md")) //nolint:gosec
		if err != nil {
...
func TestFindProjectRootPnpmMonorepo(t *testing.T) {
	...
	if err := os.MkdirAll(subPkg, 0o750); err != nil {
		t.Fatalf("MkdirAll(subPkg): %v", err)
	}
	if err := os.WriteFile(filepath.Join(subPkg, "package.json"), []byte(`{"name":"app"}`), 0o600); err != nil {
		t.Fatalf("write package.json: %v", err)
	}

	// Also add a package.json at the monorepo root
	if err := os.WriteFile(filepath.Join(root, "package.json"), []byte(`{"name":"monorepo"}`), 0o600); err != nil {
...
func TestFindProjectRootNxMonorepo(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "nx.json"), []byte(`{"version":2}`), 0o600); err != nil {
	...
	if err := os.WriteFile(filepath.Join(subPkg, "package.json"), []byte(`{"name":"web"}`), 0o600); err != nil {
		t.Fatalf("write package.json: %v", err)
	}
...
func TestFindProjectRootTurboMonorepo(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "turbo.json"), []byte(`{"$schema":"..."}`), 0o600); err != nil {
	...
	if err := os.WriteFile(filepath.Join(subPkg, "package.json"), []byte(`{"name":"ui"}`), 0o600); err != nil {
		t.Fatalf("write package.json: %v", err)
	}
...
func TestFindProjectRootGitTakesPrecedence(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(subDir, "package.json"), []byte(`{"name":"frontend"}`), 0o600); err != nil {
		t.Fatalf("write package.json: %v", err)
	}
...
func TestFindProjectRootPackageJsonFallback(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "package.json"), []byte(`{"name":"app"}`), 0o600); err != nil {
		t.Fatalf("write package.json: %v", err)
	}
...
func TestFindProjectRootMultiplePackageJsonPicksHighest(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(appDir, "package.json"), []byte(`{"name":"app"}`), 0o600); err != nil {
...
func TestFindProjectRootAllMarkers(t *testing.T) {
	...
	for _, tt := range allMarkers {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.isDir {
				_ = os.MkdirAll(markerPath, 0o750)
			} else {
...
func TestInjectOpenCodePostCheckDiskFallback(t *testing.T) {
	...
	// Pre-create a minimal config file with dxrk-orchestrator already present.
	...
	// Write a config that already has dxrk-orchestrator (simulating previous install)
	existingConfig := `{
  "agent": {
    "dxrk-orchestrator": {
      "description": "Dxrk AI SDD Orchestrator",
      "mode": "primary"
    }
  }
}`
	if err := os.WriteFile(settingsPath, []byte(existingConfig), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	...
	// Verify the file on disk still has dxrk-orchestrator and not the legacy base key.
	diskContent, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}
	if !strings.Contains(string(diskContent), "dxrk-orchestrator") {
		t.Fatal("File on disk lost dxrk-orchestrator after inject")
	}
...
func TestInjectClaudeSubAgentsScopedTools(t *testing.T) {
	...
	tests := []struct {
		...
		{
			phase:       "sdd-explore",
			mustContain: []string{"Read", "Grep", "Glob", "WebFetch", "WebSearch", "mcp__plugin_dxrk-memory_dxrk-memory__mem_save"},
			mustNotHave: []string{"Edit", "Write", "Bash", "Task"},
		},
		{
			phase:       "sdd-propose",
			mustContain: []string{"Read", "Edit", "Write", "Grep", "Glob", "mcp__plugin_dxrk-memory_dxrk-memory__mem_search", "mcp__plugin_dxrk-memory_dxrk-memory__mem_get_observation", "mcp__plugin_dxrk-memory_dxrk-memory__mem_save"},
		...
		{
			phase:       "sdd-apply",
			mustContain: []string{"Read", "Edit", "Write", "Bash", "mcp__plugin_dxrk-memory_dxrk-memory__mem_search", "mcp__plugin_dxrk-memory_dxrk-memory__mem_get_observation", "mcp__plugin_dxrk-memory_dxrk-memory__mem_save", "mcp__plugin_dxrk-memory_dxrk-memory__mem_update"},
			mustNotHave: []string{"Task"},
		},
		{
			phase:       "sdd-verify",
			mustContain: []string{"Read", "Bash", "mcp__plugin_dxrk-memory_dxrk-memory__mem_search", "mcp__plugin_dxrk-memory_dxrk-memory__mem_get_observation", "mcp__plugin_dxrk-memory_dxrk-memory__mem_save"},
			mustNotHave: []string{"Edit", "Write", "Task"},
		},
		{
			phase:       "sdd-archive",
			mustContain: []string{"Read", "Edit", "Write", "mcp__plugin_dxrk-memory_dxrk-memory__mem_search", "mcp__plugin_dxrk-memory_dxrk-memory__mem_get_observation", "mcp__plugin_dxrk-memory_dxrk-memory__mem_save"},
			mustNotHave: []string{"Bash", "Task"},
		},
	}
...
func TestEnsureClaudeSkillRegistryHookAppendsIdempotently(t *testing.T) {
  ...
  "hooks": {
	...
	text := string(data)
	if strings.Count(text, "dxrk skill-registry refresh") != 1 {
...
func TestEnsureClaudeSkillRegistryHookRejectsMalformedSettings(t *testing.T) {
	...
	after, readErr := os.ReadFile(settingsPath) //nolint:gosec
	if readErr != nil {
		t.Fatal(readErr)
	}
