...
type InjectOptions struct {
	...
	// opencode.json agent.dxrk-orchestrator.prompt value during sync.
...
func Inject(homeDir string, adapter agents.Adapter, sddMode model.SDDModeID, options ...InjectOptions) (InjectionResult, error) {
	...
	// orchestrator instructions must be scoped to the OpenCode dxrk-orchestrator agent only,
	...
	if adapter.Agent() != model.AgentOpenCode && adapter.Agent() != model.AgentKilocode {
		...
		case model.StrategyJinjaModules:
			...
			writeResult, err := filemerge.WriteFileAtomic(modulePath, []byte(content), 0o600)
			if err != nil {
				return InjectionResult{}, err
			}
	...
	if opts.StrictTDD && adapter.Agent() != model.AgentOpenCode && adapter.Agent() != model.AgentKilocode {
		if adapter.SystemPromptStrategy() == model.StrategyJinjaModules {
			...
			writeResult, writeErr := filemerge.WriteFileAtomic(promptPath, []byte(updated), 0o600)
			if writeErr != nil {
	...
	if adapter.SupportsSlashCommands() {
		...
		if commandsDir != "" {
			...
			for _, entry := range commandEntries {
				...
				writeResult, err := filemerge.WriteFileAtomic(path, []byte(content), 0o600)
				if err != nil {
					return InjectionResult{}, err
				}
	...
	// 2b. OpenCode /sdd-* commands reference agent: dxrk-orchestrator.
	...
	if adapter.Agent() == model.AgentOpenCode || adapter.Agent() == model.AgentKilocode {
		...
		if settingsPath != "" {
			...
			for _, profile := range opts.Profiles {
				if profile.Name == "" || profile.Name == PhaseDefault {
					continue
				}
	...
	if adapter.SupportsSubAgents() {
		...
		for _, entry := range entries {
			...
			writeResult, err := filemerge.WriteFileAtomic(outPath, []byte(contentStr), 0o600)
			if err != nil {
		...
		for _, phase := range []string{PhaseApply, PhaseVerify} {
			found := false
	...
	if adapter.Agent() == model.AgentOpenCode {
		...
		if len(mergedSettingsBytes) == 0 {
			if diskBytes, readErr := os.ReadFile(settingsPath); readErr == nil { //nolint:gosec
				settingsText = string(diskBytes)
			}
		}

		if !hasOpenCodeAgentKey(settingsText, "dxrk-orchestrator") {
			// In-memory check failed — try reading from disk as last resort.
			if diskBytes, readErr := os.ReadFile(settingsPath); readErr == nil { //nolint:gosec
				settingsText = string(diskBytes)
			}
			if !hasOpenCodeAgentKey(settingsText, "dxrk-orchestrator") {
				return InjectionResult{}, fmt.Errorf("post-check: %q missing dxrk-orchestrator agent definition — OpenCode /sdd-* commands will fail", settingsPath)
			}
		}
	...
	if adapter.SupportsSkills() {
		skillDir := adapter.SkillsDir(homeDir)
		if skillDir != "" {
			for _, skill := range []string{"sdd-init", PhaseApply, PhaseVerify} {
...
func inlineOpenCodeSDDPrompts(overlayBytes []byte, homeDir, settingsPath string, preserveExistingOrchestratorPrompt bool) ([]byte, error) {
	...
	orchestratorRaw, ok := agentsMap["dxrk-orchestrator"]
	if !ok {
		return overlayBytes, nil
	}
	...
	if preserveExistingOrchestratorPrompt {
		existingPrompt, err := readOpenCodeAgentPrompt(settingsPath, "dxrk-orchestrator")
		if err != nil {
			return nil, err
		}
		if existingPrompt == "" {
		...
		if existingPrompt == "" {
			existingPrompt, err = readMisnamedOpenCodeDxrkSDDPrompt(settingsPath)
			if err != nil {
				return nil, err
			}
		}
...
func readMisnamedOpenCodeDxrkSDDPrompt(settingsPath string) (string, error) {
...
func ensureClaudeSkillRegistryHook(settingsPath string) (bool, error) {
	root := map[string]any{}
	if data, err := os.ReadFile(settingsPath); err == nil && len(strings.TrimSpace(string(data))) > 0 { //nolint:gosec
		if err := json.Unmarshal(data, &root); err != nil {
	...
	const command = `dxrk skill-registry refresh --quiet --no-gitignore --cwd "${CLAUDE_PROJECT_DIR:-$PWD}" || true`
	...
	out = append(out, '\n')
	wr, err := filemerge.WriteFileAtomic(settingsPath, out, 0o600)
	if err != nil {
		return false, err
	}
...
func installOpenCodePlugins(homeDir string, adapter agents.Adapter) (InjectionResult, error) {
	...
	for _, name := range []string{"background-agents.ts", "model-variants.ts"} {
		...
		writeResult, err := filemerge.WriteFileAtomic(pluginPath, []byte(content), 0o600)
		if err != nil {
...
func mergeJSONFile(path string, overlay []byte) (mergeJSONResult, error) {
	baseJSON, err := os.ReadFile(path) //nolint:gosec
	if err != nil {
		if !os.IsNotExist(err) {
	...
	writeResult, err := filemerge.WriteFileAtomic(path, merged, 0o600)
	if err != nil {
		return mergeJSONResult{}, err
	}
...
// dxrk-orchestrator primary agent; named profile agents such as
// sdd-orchestrator-cheap intentionally remain untouched because they are
// generated profile-specific coordinators. The old OpenCode "dxrk" agent
// key is revoked and is removed during sync; if it clearly contains the old SDD
// conductor prompt and no dxrk-orchestrator exists yet, its prompt is migrated
...
func migrateLegacyOpenCodeSDDOrchestrator(baseJSON []byte) ([]byte, error) {
	...
	revokedDxrk, hasRevokedDxrk := agentsMap["dxrk"]
	dxrkLooksLikeConductor := hasRevokedDxrk && looksLikeOpenCodeSDDConductor(revokedDxrk)
	if !hasLegacy && !hasRevokedDxrk {
		return baseJSON, nil
	}
	if !hasLegacy && dxrkLooksLikeConductor {
		legacy = revokedDxrk
		hasLegacy = true
	}

	if _, hasDxrkOrchestrator := agentsMap["dxrk-orchestrator"]; !hasDxrkOrchestrator && hasLegacy {
		agentsMap["dxrk-orchestrator"] = legacy
	}
	delete(agentsMap, "sdd-orchestrator")
	if hasRevokedDxrk {
		delete(agentsMap, "dxrk")
	}
...
func looksLikeOpenCodeSDDConductor(agentRaw any) bool {
	...
	for _, field := range []string{jsonDescription, jsonPrompt} {
...
const instructionsFrontmatter = "---\n" +
	...
	"description: Dxrk persona with SDD orchestration and DxrkMemory protocol\n" +
	"applyTo: \"**\"\n" +
	"---\n"
...
func renderClaudeModelAssignmentsSection(assignments map[string]model.ClaudeModelAlias) string {
	...
	b.WriteString("The Claude Code session model is controlled by Claude Code itself; Dxrk AI does not configure the main orchestrator model. This table applies only to Agent tool calls for SDD phase sub-agents and general delegation.\n\n")
...
func injectModelAssignments(overlayBytes []byte, assignments map[string]model.ModelAssignment, rootModelID string, existingAgentKeys map[string]bool) ([]byte, error) {
	...
	agentMap, ok := agentsRaw.(map[string]any)
	if !ok {
		return overlayBytes, nil
	}

	for phase, agentDef := range agentMap {
...
// current base coordinator key: dxrk-orchestrator. Named profile keys remain unchanged.
func normalizeOpenCodeSDDModelAssignments(assignments map[string]model.ModelAssignment) map[string]model.ModelAssignment {
	...
	if _, hasDxrkOrchestrator := assignments["dxrk-orchestrator"]; hasDxrkOrchestrator {
		return assignments
	}

	normalized := make(map[string]model.ModelAssignment, len(assignments))
	for key, assignment := range assignments {
		if key == PhaseOrchestrator {
			continue
		}
		normalized[key] = assignment
	}
	normalized[dxrkOrchestrator] = legacyAssignment
	return normalized
}
