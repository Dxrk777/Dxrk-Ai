...
// opencode.json. It includes the sub-agent phases plus the dxrk-orchestrator coordinator.
...
func buildSDDPhaseSet() map[string]bool {
	...
	set[dxrkOrchestrator] = true
	// Backward-compatible read alias for configs that have not been synced yet.
	set[PhaseOrchestrator] = true
	return set
}

const dxrkOrchestrator = "dxrk-orchestrator"
...
// "dxrk-orchestrator" are included. Legacy "sdd-orchestrator" entries are read as
// "dxrk-orchestrator" until the next sync migrates the config. Agents without a "model" field, or with a
...
func ReadCurrentModelAssignments(settingsPath string) (map[string]model.ModelAssignment, error) {
	...
	for name, defRaw := range agentMap {
		...
		assignmentKey := name
		if name == PhaseOrchestrator {
			assignmentKey = dxrkOrchestrator
			if _, hasDxrkOrchestrator := result[assignmentKey]; hasDxrkOrchestrator {
				continue
			}
		}
