...
func ProfileAgentKeys(name string) []string {
	...
	keys = append(keys, PhaseOrchestrator+suffix)
...
func DetectProfiles(settingsPath string) ([]model.Profile, error) {
	...
	const orchPrefix = PhaseOrchestrator + "-"
	...
	for _, profileName := range profileNames {
		orchKey := PhaseOrchestrator + "-" + profileName
...
func GenerateProfileOverlay(profile model.Profile, homeDir string) ([]byte, error) {
	...
	phaseDescriptions := map[string]string{
		PhaseInit:    "Bootstrap SDD context and project configuration",
		PhaseExplore: "Investigate codebase and think through ideas",
		PhasePropose: "Create change proposals from explorations",
		PhaseSpec:    "Write detailed specifications from proposals",
		PhaseDesign:  "Create technical design from proposals",
		PhaseTasks:   "Break down specs and designs into implementation tasks",
		PhaseApply:   "Implement code changes from task definitions",
		PhaseVerify:  "Validate implementation against specs",
		PhaseArchive: "Archive completed change artifacts",
		PhaseOnboard: "Guide user through a complete SDD cycle using their real codebase",
	}
...
func buildProfileOrchestratorPrompt(profile model.Profile) string {
	...
	base = replacePhaseRef(base, PhaseOrchestrator, PhaseOrchestrator+suffix)
...
func renderProfileModelAssignmentsSection(profile model.Profile) string {
	...
	fmt.Fprintf(&b, "| orchestrator | %s | Coordinates, makes decisions |\n", orchModel)

	// Phase rows
	phaseReasons := map[string]string{
		PhaseInit:    "Bootstrap SDD context",
		PhaseExplore: "Reads code, structural - not architectural",
		PhasePropose: "Architectural decisions",
		PhaseSpec:    "Structured writing",
		PhaseDesign:  "Architecture decisions",
		PhaseTasks:   "Mechanical breakdown",
		PhaseApply:   "Implementation",
		PhaseVerify:  "Validation against spec",
		PhaseArchive: "Copy and close",
		PhaseOnboard: "Guided walkthrough",
	}
...
func RemoveProfileAgents(settingsPath string, profileName string) error {
	if profileName == "" || profileName == PhaseDefault {
	...
	_, err = filemerge.WriteFileAtomic(settingsPath, out, 0o600)
	return err
}
