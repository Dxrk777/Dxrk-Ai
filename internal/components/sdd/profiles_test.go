...
func TestResolveProfileStrategy_ExplicitWins(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(profilesDir, "active.json"), []byte(`{"name":"external"}`), 0o600); err != nil {
...
func TestResolveProfileStrategy_AutoDetectsExternalProfiles(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(profilesDir, "cheap.json"), []byte(`{"name":"cheap"}`), 0o600); err != nil {
...
func TestGenerateProfileOverlay_Structure(t *testing.T) {
	...
	if modelName, _ := orch["model"].(string); modelName != "anthropic/claude-haiku-3-5" {
		t.Errorf("sdd-orchestrator-cheap model = %q, want %q", modelName, "anthropic/claude-haiku-3-5")
	}
...
func TestDefaultOverlayTaskPermissions_ExplicitAllowlist(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			agentMap := root["agent"].(map[string]any)
			orch := agentMap["dxrk-orchestrator"].(map[string]any)
