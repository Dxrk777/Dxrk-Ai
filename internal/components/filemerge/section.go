...
// written by the standalone ATL installer before dxrk superseded it.
...
// <!-- dxrk:sdd-orchestrator --> section, so keeping both wastes ~150
...
func InjectMarkdownSection(existing, sectionID, content string) string {
	openMark := openMarker(sectionID)
	closeMark := closeMarker(sectionID)

	openIdx := strings.Index(existing, openMark)
	...
	if openIdx >= 0 && closeIdx >= 0 && closeIdx > openIdx {
		...
		var sb strings.Builder
		sb.WriteString(before)
		sb.WriteString(openMark)
		sb.WriteString("\n")
		sb.WriteString(content)
		if !strings.HasSuffix(content, "\n") {
			sb.WriteString("\n")
		}
		sb.WriteString(closeMark)
		sb.WriteString(after)
		return sb.String()
	}
