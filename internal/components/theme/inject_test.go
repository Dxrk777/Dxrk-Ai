...
func TestInjectMergesThemeOverlayIntoAdapterSettings(t *testing.T) {
	...
	if root.Theme != "dxrk-kanagawa" {
		t.Fatalf("theme = %q, want dxrk-kanagawa", root.Theme)
	}
	...
	if _, err := os.Stat(filepath.Join(home, ".claude", "themes", "dxrk.json")); !os.IsNotExist(err) {
...
func TestInjectClaudeThemeIsIdempotent(t *testing.T) {
	...
	path := filepath.Join(home, ".claude", "themes", "dxrk.json")
...
func TestInjectClaudeThemeWritesDxrkThemeFile(t *testing.T) {
	home := t.TempDir()
	...
	if root.Name != "Dxrk" || root.Base != "dark" {
		t.Fatalf("theme identity = %q/%q, want Dxrk/dark", root.Name, root.Base)
	}
