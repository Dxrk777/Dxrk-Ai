...
var dxrkClaudeTheme = claudeTheme{
	Name: "Dxrk",
	Base: "dark",
	Overrides: map[string]string{
		...
		"briefLabelYou":             chromeYellow,
		"rainbow_yellow":            chromeYellow,
		"yellow_FOR_SUBAGENTS_ONLY": chromeYellow,
	},
}
...
func InjectClaudeTheme(homeDir string, adapter agents.Adapter) (InjectionResult, error) {
	...
	themePath := filepath.Join(homeDir, ".claude", "themes", "dxrk.json")
	content, err := json.MarshalIndent(dxrkClaudeTheme, "", "  ")
	if err != nil {
		return InjectionResult{}, err
	}
	content = append(content, '\n')

	writeResult, err := filemerge.WriteFileAtomic(themePath, content, 0o600)
	if err != nil {
		return InjectionResult{}, err
	}
