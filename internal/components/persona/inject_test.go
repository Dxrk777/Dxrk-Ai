...
func assertDxrkLanguageGuardrails(t *testing.T, text string, required []string, banned []string) {
	t.Helper()
...
func TestInjectClaudeDxrkWritesSectionWithRealContent(t *testing.T) {
	...
	assertDxrkLanguageGuardrails(t, text,
		[]string{
...
func TestInjectKimiDxrkIncludesProjectInstructionsAndLoadedSkills(t *testing.T) {
	home := t.TempDir()

	result, err := Inject(home, kimiAdapter(), model.PersonaDxrk)
	if err != nil {
	...
	content, err := os.ReadFile(templatePath) //nolint:gosec
	if err != nil {
	...
	// output-style.md module should contain the Dxrk style content.
	outputStylePath := filepath.Join(home, ".kimi", "output-style.md")
	styleContent, err := os.ReadFile(outputStylePath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(%q) error = %v", outputStylePath, err)
	}
	if !strings.Contains(string(styleContent), "Dxrk Output Style") {
		t.Fatal("output-style.md missing Dxrk Output Style content")
	}
	assertDxrkLanguageGuardrails(t, string(styleContent),
		[]string{
	...
	personaContent, err := os.ReadFile(personaPath) //nolint:gosec
	if err != nil {
		t.Fatalf("persona.md not written: %v", err)
	}
	assertDxrkLanguageGuardrails(t, string(personaContent),
		[]string{
			"Match the user's current language in your REPLY ONLY",
			"Do not switch languages unless the user does, asks you to, or you are quoting/translating content.",
			"When replying to the user in English, keep the full reply in natural English with the same warm energy.",
...
func TestInjectClaudeDxrkWritesOutputStyleFile(t *testing.T) {
	...
	content, err := os.ReadFile(stylePath) //nolint:gosec
	if err != nil {
...
func TestInjectClaudeDxrkMergesOutputStyleIntoSettings(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(settingsDir, "settings.json"), []byte(existingSettings), 0o600); err != nil {
...
func TestInjectOpenCodeDxrkDoesNotCreateSDDConductor(t *testing.T) {
	home := t.TempDir()
	...
	if strings.Contains(text, `"dxrk-orchestrator"`) {
		t.Fatal("persona injection must not create SDD conductor; SDD component owns dxrk-orchestrator")
	}
	if !strings.Contains(text, `"dxrk"`) {
		t.Fatal("persona injection should still create the dxrk persona agent")
	}
}
...
func TestInjectOpenClawWritesPersonaToWorkspaceSoulAndNotAgents(t *testing.T) {
	...
	result, err := Inject(workspace, adapter, model.PersonaDxrk)
	if err != nil {
	...
	soulContent, err := os.ReadFile(soulPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(SOUL.md) error = %v", err)
	}
	soulText := string(soulContent)
	if !strings.Contains(soulText, "<!-- dxrk:persona -->") {
	...
	if strings.Contains(agentsText, "<!-- dxrk:persona -->") || strings.Contains(agentsText, "Senior Architect") {
...
func TestInjectOpenClawSoulPersonaIsIdempotentAndPreservesUserContent(t *testing.T) {
	...
	adapter := openclawAdapter()
	first, err := Inject(workspace, adapter, model.PersonaDxrk)
	if err != nil {
	...
	second, err := Inject(workspace, adapter, model.PersonaDxrk)
	if err != nil {
	...
	if count := strings.Count(text, "<!-- dxrk:persona -->"); count != 1 {
...
func TestInjectOpenClawRejectsAmbiguousWorkspacePath(t *testing.T) {
	...
	result, err := Inject("", openclawAdapter(), model.PersonaDxrk)
	if err == nil {
...
func TestInjectOpenCodeDoesNotStripLookalikeUserContent(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(lookalike), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
...
func TestInjectOpenCodeReplacesExactLegacyAssetWithoutDuplication(t *testing.T) {
	...
	legacyContent := assets.MustRead("opencode/persona-dxrk.md")
	if err := os.WriteFile(path, []byte(legacyContent), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
...
func TestInjectOpenCodePreservesUserPrefaceAboveManagedMarkers(t *testing.T) {
	...
	if !strings.Contains(text, "<!-- dxrk:dxrk-memory-protocol -->") {
		t.Fatal("existing dxrk-memory section was lost")
	}
}
...
func TestInjectOpenCodeNeutralPreservesManagedSections(t *testing.T) {
	...
	// Simulate SDD and dxrk-memory sections appended by sdd.Inject and dxrkmemory.Inject
	existing, err := os.ReadFile(path) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}
	withSections := string(existing) + "\n\n<!-- dxrk:sdd-orchestrator -->\nSDD orchestrator content here\n<!-- /dxrk:sdd-orchestrator -->\n\n<!-- dxrk:dxrk-memory-protocol -->\nDxrkMemory protocol content here\n<!-- /dxrk:dxrk-memory-protocol -->\n"
	if err := os.WriteFile(path, []byte(withSections), 0o600); err != nil { //nolint:gosec
		t.Fatalf("WriteFile() error = %v", err)
	}
...
func TestInjectNeutralPreservesWhenMarkerAtByteZero(t *testing.T) {
	...
	if err := os.WriteFile(promptPath, []byte(markerOnly), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	...
	content, err := os.ReadFile(promptPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}
	text := string(content)
...
func TestInjectNeutralIdempotentWithManagedSections(t *testing.T) {
	...
	if err := os.WriteFile(promptPath, []byte(initial), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
...
func TestInjectWindsurfIsIdempotent(t *testing.T) {
	...
	contentAfterFirst, err := os.ReadFile(promptPath) //nolint:gosec
	if err != nil {
	...
	contentAfterSecond, err := os.ReadFile(promptPath) //nolint:gosec
	if err != nil {
...
func TestInjectClaudeAutoHealsStaleFreeTextPersona(t *testing.T) {
	...
	if err := os.WriteFile(claudeMD, []byte(stalePreamble), 0o600); err != nil {
		t.Fatalf("WriteFile error = %v", err)
	}
	...
	content, err := os.ReadFile(claudeMD) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}
	text := string(content)
...
func TestInjectClaudeAutoHealStalePersonaOnlyFile(t *testing.T) {
	...
	if err := os.WriteFile(claudeMD, []byte(legacyClaudePersonaBlock), 0o600); err != nil {
		t.Fatalf("WriteFile error = %v", err)
	}
...
func TestInjectClaudeHealDoesNotTouchNonPersonaContent(t *testing.T) {
	...
	if err := os.WriteFile(claudeMD, []byte(userContent), 0o600); err != nil {
		t.Fatalf("WriteFile error = %v", err)
	}
...
func TestInjectVSCodeCleansLegacyGitHubPersonaFile(t *testing.T) {
	...
	if err := os.WriteFile(legacyPath, []byte(oldContent), 0o600); err != nil {
		t.Fatalf("WriteFile error = %v", err)
	}
	...
	content, readErr := os.ReadFile(newPath) //nolint:gosec
	if readErr != nil {
...
func TestInjectVSCodePreservesNonPersonaGitHubFile(t *testing.T) {
	...
	if err := os.WriteFile(legacyPath, []byte(userContent), 0o600); err != nil {
		t.Fatalf("WriteFile error = %v", err)
	}
	...
	remaining, readErr := os.ReadFile(legacyPath) //nolint:gosec
	if readErr != nil {
...
func TestInjectClaude_SwitchDxrkToNeutral_CleansOutputStyle(t *testing.T) {
	home := t.TempDir()

	// Step 1: install dxrk — creates output-styles/dxrk.md and sets outputStyle in settings.json.
	_, err := Inject(home, claudeAdapter(), model.PersonaDxrk)
	if err != nil {
		t.Fatalf("Inject(dxrk) error = %v", err)
	}
	...
	if _, statErr := os.Stat(stylePath); os.IsNotExist(statErr) {
		t.Fatal("precondition: dxrk.md must exist after dxrk install")
	}

	settingsPath := filepath.Join(home, ".claude", "settings.json")
	settingsRaw, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
		t.Fatalf("precondition: settings.json must exist after dxrk install: %v", err)
	}
	...
	if settingsBefore["outputStyle"] != "Dxrk" {
		t.Fatalf("precondition: outputStyle must be 'Dxrk', got %v", settingsBefore["outputStyle"])
	}
	...
	if !result.Changed {
		t.Fatal("Inject(neutral) should report changed when cleaning dxrk residuals")
	}

	// output-styles/dxrk.md must be gone.
	if _, statErr := os.Stat(stylePath); !os.IsNotExist(statErr) {
		t.Fatal("dxrk.md must be removed when switching to neutral")
	}
...
func TestInjectClaude_Neutral_PreservesUserOutputStyle(t *testing.T) {
	...
	// Pre-create settings.json with a user-defined outputStyle that is NOT "Dxrk".
	settingsDir := filepath.Join(home, ".claude")
	if err := os.MkdirAll(settingsDir, 0o750); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
...
func TestInjectClaude_SwitchDxrkToNeutral_IsIdempotent(t *testing.T) {
	home := t.TempDir()

	// Install dxrk, then switch to neutral twice — second switch must be a no-op.
	_, err := Inject(home, claudeAdapter(), model.PersonaDxrk)
	if err != nil {
		t.Fatalf("Inject(dxrk) error = %v", err)
	}
	...
	if !first.Changed {
		t.Fatal("first neutral inject after dxrk should report changed")
	}
...
func TestInjectOpenCode_SwitchDxrkToNeutral_CleansAgentOverlay(t *testing.T) {
	home := t.TempDir()

	// Step 1: install dxrk — agent.dxrk key must appear in opencode.json.
	_, err := Inject(home, opencodeAdapter(), model.PersonaDxrk)
	if err != nil {
		t.Fatalf("Inject(dxrk) error = %v", err)
	}
	...
	settingsRaw, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
		t.Fatalf("precondition: opencode.json must exist after dxrk install: %v", err)
	}
	...
	if !ok {
		t.Fatal("precondition: 'agent' key must be present after dxrk install")
	}
	if _, ok := agentBefore["dxrk"]; !ok {
		t.Fatal("precondition: agent.dxrk must be present after dxrk install")
	}
	...
	// Step 2: switch to neutral — agent.dxrk must be removed.
	result, err := Inject(home, opencodeAdapter(), model.PersonaNeutral)
	if err != nil {
		t.Fatalf("Inject(neutral) error = %v", err)
	}
	if !result.Changed {
		t.Fatal("Inject(neutral) should report changed when cleaning agent.dxrk residual")
	}
	...
	// agent.dxrk must be gone.
	if agentAfter, ok := after["agent"].(map[string]any); ok {
		if _, stillPresent := agentAfter["dxrk"]; stillPresent {
			t.Fatal("agent.dxrk must be removed from opencode.json after switching to neutral")
		}
		// User-defined agent must survive.
		if _, ok := agentAfter["my-custom-agent"]; !ok {
			t.Fatal("user-defined agent 'my-custom-agent' was removed — only agent.dxrk should be cleaned")
		}
	}
...
func TestInjectKilocode_SwitchDxrkToNeutral_CleansAgentOverlay(t *testing.T) {
	home := t.TempDir()

	_, err := Inject(home, kilocodeAdapter(), model.PersonaDxrk)
	if err != nil {
		t.Fatalf("Inject(dxrk) error = %v", err)
	}
	...
	if !strings.Contains(string(data), `"dxrk"`) {
		t.Fatal("precondition: kilo/opencode.json should have dxrk agent after Dxrk install")
	}
	...
	if !result.Changed {
		t.Fatal("Inject(neutral) should report changed when cleaning up dxrk agent overlay")
	}
	...
	if strings.Contains(string(data), `"dxrk"`) {
		t.Fatal("kilo/opencode.json must not have dxrk agent key after switching to Neutral")
	}
}
...
func TestInjectOpenCode_NeutralFresh_IsNoOp(t *testing.T) {
	...
	if _, statErr := os.Stat(settingsPath); !os.IsNotExist(statErr) {
		data, _ := os.ReadFile(settingsPath) //nolint:gosec
		if strings.Contains(string(data), `"dxrk"`) {
			t.Fatal("Neutral fresh install must not create dxrk agent key")
		}
	}
}

func TestInjectOpenCode_DxrkOnly_WritesAgentOverlay(t *testing.T) {
	home := t.TempDir()
	...
	if !strings.Contains(string(data), `"dxrk"`) {
		t.Fatal("Dxrk install must write dxrk agent overlay in opencode.json")
	}
}
...
func TestInjectOpenCode_MalformedJSON_DoesNotPanic(t *testing.T) {
	...
	malformed := `{ "agent": { "dxrk": {invalid json`
	if err := os.WriteFile(filepath.Join(settingsDir, "opencode.json"), []byte(malformed), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
...
func TestInjectClaude_MalformedJSON_DoesNotPanic(t *testing.T) {
	...
	malformed := `{ "outputStyle": "Dxrk", invalid`
	if err := os.WriteFile(filepath.Join(settingsDir, "settings.json"), []byte(malformed), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
...
func TestInjectKimi_SwitchDxrkToNeutral_NoResidualPersonaContent(t *testing.T) {
	home := t.TempDir()

	if _, err := Inject(home, kimiAdapter(), model.PersonaDxrk); err != nil {
		t.Fatalf("Inject(dxrk) error = %v", err)
	}
	...
	outputStylePath := filepath.Join(home, ".kimi", "output-style.md")
	data, err := os.ReadFile(outputStylePath) //nolint:gosec
	if err != nil {
	...
	if strings.Contains(content, "Dxrk Output Style") {
		t.Error("output-style.md still contains 'Dxrk Output Style' after switching to neutral")
	}
...
func TestInjectForSync_OpenCodeNeutral_DoesNotCleanAgentDxrk(t *testing.T) {
	home := t.TempDir()

	if _, err := Inject(home, opencodeAdapter(), model.PersonaDxrk); err != nil {
		t.Fatalf("Inject(dxrk) error = %v", err)
	}

	settingsPath := filepath.Join(home, ".config", "opencode", "opencode.json")
	before, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(opencode.json) after install error = %v", err)
	}
	if !strings.Contains(string(before), `"dxrk"`) {
		t.Fatalf("opencode.json missing dxrk agent after install; got:\n%s", string(before))
	}
	...
	after, err := os.ReadFile(settingsPath) //nolint:gosec
	if err != nil {
		t.Fatalf("ReadFile(opencode.json) after sync error = %v", err)
	}
	if !strings.Contains(string(after), `"dxrk"`) {
		t.Fatalf("opencode.json lost dxrk agent after InjectForSync(neutral) — this is by-design and must not regress;\ngot:\n%s", string(after))
	}
}

func TestInjectForSync_ClaudeDxrkToNeutral_CleansOutputStyle(t *testing.T) {
	home := t.TempDir()

	if _, err := Inject(home, claudeAdapter(), model.PersonaDxrk); err != nil {
		t.Fatalf("Inject(dxrk) error = %v", err)
	}

	stylePath := filepath.Join(home, ".claude", "output-styles", "dxrk.md")
	if _, err := os.Stat(stylePath); os.IsNotExist(err) {
		t.Fatal("dxrk.md not written by Inject(dxrk) — precondition failed")
	}
	...
	if _, err := os.Stat(stylePath); !os.IsNotExist(err) {
		t.Fatal("dxrk.md still present after InjectForSync(neutral) — residue not cleaned")
	}

	afterRaw, err := os.ReadFile(settingsPath) //nolint:gosec
