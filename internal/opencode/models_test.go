...
func TestDefaultVariantsCachePath(t *testing.T) {
	...
	if !strings.HasSuffix(got, filepath.Join(".dxrk", "cache", "model-variants.json")) {
		t.Fatalf("expected path suffix .dxrk/cache/model-variants.json, got %q", got)
	}
	legacy := filepath.Join(".cache", "dxrk")
