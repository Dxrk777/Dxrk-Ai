...
// Write/Read. Both `dxrk install` (CLI in run.go) and the TUI app
// (internal/app/app.go) write this field after a successful install so that
// `dxrk sync` regenerates the persona the user actually selected — not a
// hard-coded default.
func TestPersonaRoundTrip(t *testing.T) {
	for _, persona := range []string{"dxrk", "neutral", "custom"} {
...
func TestReadCorrupt(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(home, stateDir), 0o750); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	if err := os.WriteFile(Path(home), []byte("not valid json {{{{"), 0o600); err != nil {
...
func TestModelAssignmentStateEffortLegacyMissing(t *testing.T) {
	...
	if err := os.WriteFile(Path(home), []byte(legacy), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
...
func TestBackwardCompatNoAssignments(t *testing.T) {
	...
	if err := os.WriteFile(Path(home), legacy, 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
