...
type InstallState struct {
	...
	// `dxrk sync` preserves the user's model choices instead of falling
	...
	// Persona records the persona the user installed ("dxrk", "neutral",
	// "custom"). Persisted so that `dxrk sync` regenerates the same persona
	// the user originally chose instead of defaulting to Dxrk every time.
	// Empty for state files written before persona persistence was added —
	// callers fall back to PersonaDxrk in that case.
...
// It creates the .dxrk directory if it does not already exist.
