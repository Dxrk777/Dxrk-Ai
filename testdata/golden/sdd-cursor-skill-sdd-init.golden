...
- In `dxrk-memory` mode, do **not** create `openspec/`.
- In `openspec` mode, follow `../_shared/openspec-convention.md` and write file artifacts.
- In `hybrid` mode, write both openspec files and Dxrk-Memory observations.
- Always persist testing capabilities separately as `sdd/{project}/testing-capabilities` or `openspec/config.yaml` `testing:`.
- Always build `.atl/skill-registry.md`; also save `skill-registry` to Dxrk-Memory when available.
...
| Input | Action |
|---|---|
| `mode=dxrk-memory` | Save context and capabilities to Dxrk-Memory only. |
| `mode=openspec` | Create/update openspec bootstrap files only. |
| `mode=hybrid` | Do both Dxrk-Memory and openspec persistence. |
...
- [references/init-details.md](references/init-details.md) — detection checklist, Dxrk-Memory payloads, config skeleton, and output templates.
- `../_shared/dxrk-memory-convention.md` — Dxrk-Memory artifact naming.
