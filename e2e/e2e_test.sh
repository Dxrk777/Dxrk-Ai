#!/usr/bin/env bash
# e2e_test.sh — End-to-end tests for dxrk installer
#
...
test_version_command() {
    ...
    if echo "$output" | grep -q "dxrk"; then
...
test_dry_run_preset_full() {
    log_test "Dry-run with --preset full-dxrk"

    output=$($BINARY install --preset full-dxrk --dry-run 2>&1) || true

    assert_output_contains "$output" "Preset: full-dxrk" "Shows full-dxrk preset"
}
...
test_preset_minimal_components() {
    log_test "Preset minimal produces only dxrk-memory component"

    output=$($BINARY install --preset minimal --agent claude-code --dry-run 2>&1) || true

    # The component list should contain dxrk-memory
    assert_output_contains "$output" "dxrk-memory" "Minimal preset includes dxrk-memory"
...
test_preset_ecosystem_components() {
    ...
    # ecosystem-only = dxrk-memory, sdd, skills, context7, dxrk-guardian
    local components_line
    components_line=$(echo "$output" | grep "Components order:")

    assert_output_contains "$components_line" "dxrk-memory" "Ecosystem includes dxrk-memory"
    ...
    assert_output_contains "$components_line" "dxrk-guardian" "Ecosystem includes dxrk-guardian"
...
test_preset_full_components() {
    log_test "Preset full-dxrk includes core and optional Dxrk components"

    output=$($BINARY install --preset full-dxrk --agent claude-code --dry-run 2>&1) || true

    local components_line
    components_line=$(echo "$output" | grep "Components order:")

    assert_output_contains "$components_line" "dxrk-memory" "Full includes dxrk-memory"
    ...
    assert_output_contains "$components_line" "dxrk-guardian" "Full includes dxrk-guardian"
    assert_output_contains "$components_line" "claude-theme" "Full includes Claude Dxrk theme"
    assert_output_contains "$components_line" "opencode-dxrk-logo" "Full includes OpenCode Dxrk logo"
}

test_dry_run_full_preset_persona_before_sdd() {
    log_test "Dry-run: persona appears before dxrk-memory and sdd in component order"

    output=$($BINARY install --preset full-dxrk --agent opencode --dry-run 2>&1) || true
    ...
    assert_output_contains "$components_line" "dxrk-memory" "Full preset has dxrk-memory"
    assert_output_contains "$components_line" "sdd" "Full preset has sdd"

    # Verify ordering: persona before dxrk-memory, persona before sdd
...
test_preset_no_legacy_theme_in_any_preset() {
    ...
    for preset in minimal ecosystem-only full-dxrk; do
...
test_preset_custom_no_components() {
    ...
    local components_line
    components_line=$(echo "$output" | grep "Components order:")
    assert_output_not_contains "$components_line" "dxrk-memory" "Custom preset without components excludes dxrk-memory"
...
test_preset_custom_explicit_components() {
    ...
    output=$($BINARY install --preset custom --agent claude-code --component dxrk-memory --component sdd --component skills --dry-run 2>&1) || true

    local components_line
    components_line=$(echo "$output" | grep "Components order:")
    assert_output_contains "$components_line" "dxrk-memory" "Custom + explicit components includes dxrk-memory"
...
test_dry_run_component_dxrk-memory() {
    log_test "Dry-run with --component dxrk-memory"
    output=$($BINARY install --agent claude-code --component dxrk-memory --dry-run 2>&1) || true
    assert_output_contains "$output" "dxrk-memory" "Shows dxrk-memory component"
}
...
test_dry_run_component_dxrk-guardian() {
    log_test "Dry-run with --component dxrk-guardian"
    output=$($BINARY install --agent claude-code --component dxrk-guardian --dry-run 2>&1) || true
    assert_output_contains "$output" "dxrk-guardian" "Shows dxrk-guardian component"
}
...
test_cc_dxrk-memory_injection() {
    log_test "Claude Code: dxrk-memory injection (MCP + CLAUDE.md)"
    cleanup_test_env

    if $BINARY install --agent claude-code --component dxrk-memory --persona neutral 2>&1; then
        # MCP config
        assert_file_exists "$HOME/.claude/mcp/dxrk-memory.json" "dxrk-memory.json MCP config"
        assert_file_contains "$HOME/.claude/mcp/dxrk-memory.json" '"command"' "dxrk-memory.json has 'command' key"
        assert_file_contains "$HOME/.claude/mcp/dxrk-memory.json" 'dxrk-memory' "dxrk-memory.json command points to dxrk-memory binary (absolute or relative)"
        assert_valid_json "$HOME/.claude/mcp/dxrk-memory.json" "dxrk-memory.json is valid JSON"

        # CLAUDE.md section
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "dxrk:dxrk-memory-protocol" "CLAUDE.md has dxrk-memory-protocol section marker"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "mem_save" "CLAUDE.md has real Dxrk-Memory content (mem_save)"
        assert_file_size_min "$HOME/.claude/CLAUDE.md" 500 "CLAUDE.md has substantial content"
    else
        log_fail "dxrk-memory install command failed"
    fi
}
...
test_cc_sdd_injection() {
    ...
    if $BINARY install --agent claude-code --component sdd --persona neutral 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "dxrk:sdd-orchestrator" "CLAUDE.md has SDD section marker"
        ...
        assert_file_contains "$HOME/.claude/agents/sdd-explore.md" "mcp__plugin_dxrk-memory_dxrk-memory__mem_save" "Claude explore sub-agent includes Dxrk-Memory save"
        ...
        assert_file_contains "$HOME/.claude/agents/sdd-apply.md" "mcp__plugin_dxrk-memory_dxrk-memory__mem_search" "Claude apply sub-agent includes Dxrk-Memory search"
        assert_file_contains "$HOME/.claude/agents/sdd-apply.md" "mcp__plugin_dxrk-memory_dxrk-memory__mem_get_observation" "Claude apply sub-agent includes Dxrk-Memory read"
        assert_file_contains "$HOME/.claude/agents/sdd-apply.md" "mcp__plugin_dxrk-memory_dxrk-memory__mem_save" "Claude apply sub-agent includes Dxrk-Memory save"
        assert_file_contains "$HOME/.claude/agents/sdd-apply.md" "mcp__plugin_dxrk-memory_dxrk-memory__mem_update" "Claude apply sub-agent includes Dxrk-Memory update"
        ...
        assert_file_contains "$HOME/.claude/agents/sdd-verify.md" "mcp__plugin_dxrk-memory_dxrk-memory__mem_search" "Claude verify sub-agent includes Dxrk-Memory search"
        assert_file_contains "$HOME/.claude/agents/sdd-verify.md" "mcp__plugin_dxrk-memory_dxrk-memory__mem_get_observation" "Claude verify sub-agent includes Dxrk-Memory read"
        assert_file_contains "$HOME/.claude/agents/sdd-verify.md" "mcp__plugin_dxrk-memory_dxrk-memory__mem_save" "Claude verify sub-agent includes Dxrk-Memory save"
    else
        log_fail "SDD install command failed"
    fi
}

test_cc_persona_dxrk() {
    log_test "Claude Code: persona injection (dxrk)"
    cleanup_test_env

    if $BINARY install --agent claude-code --component persona --persona dxrk 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "dxrk:persona" "CLAUDE.md has persona section marker"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "Senior Architect" "Dxrk persona has 'Senior Architect'"
        ...
        assert_file_contains "$HOME/.claude/settings.json" "Dxrk" "settings.json outputStyle is Dxrk"
    else
        log_fail "persona (dxrk) install command failed"
    fi
}
...
test_cc_skills_full() {
    log_test "Claude Code: skills injection (full-dxrk = 9 foundation skills)"
    cleanup_test_env

    if $BINARY install --agent claude-code --component skills --preset full-dxrk --persona neutral 2>&1; then
        local skills_dir="$HOME/.claude/skills"
        assert_dir_exists "$skills_dir" "Claude skills directory"
...
test_cc_custom_no_skills_flag_installs_nothing() {
    ...
    if $BINARY install --agent claude-code --preset custom --component skills --persona neutral 2>&1; then
        local skills_dir="$HOME/.claude/skills"
        # --component skills auto-resolves sdd as a hard dependency (graph: skills → sdd → dxrk-memory).
...
test_cc_custom_sdd_plus_skills() {
    ...
    if $BINARY install --agent claude-code --preset custom --component dxrk-memory --component sdd --component skills --skills go-testing,branch-pr --persona neutral 2>&1; then
        local skills_dir="$HOME/.claude/skills"
        assert_dir_exists "$skills_dir" "Claude skills directory"
...
test_cc_theme_injection() {
    ...
    if $BINARY install --agent claude-code --component theme --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" 'dxrk-kanagawa' "Has dxrk-kanagawa theme"
        assert_valid_json "$settings" "settings.json is valid JSON"
    else
...
test_oc_dxrk-memory_injection() {
    log_test "OpenCode: dxrk-memory injection (opencode.json)"
    cleanup_test_env

    if $BINARY install --agent opencode --component dxrk-memory --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" '"dxrk-memory"' "Has dxrk-memory MCP entry"
        assert_file_contains "$settings" '"command"' "Has command key"
        assert_file_contains "$settings" '"type": "local"' "Dxrk-Memory uses local MCP type"
        assert_valid_json "$settings" "opencode.json is valid JSON"

        # Fallback safety: AGENTS.md must include dxrk-memory protocol section.
        assert_file_exists "$agents_md" "OpenCode AGENTS.md"
        assert_file_contains "$agents_md" 'dxrk:dxrk-memory-protocol' "AGENTS.md has dxrk-memory-protocol section"
        assert_file_contains "$agents_md" 'mem_save' "AGENTS.md has memory protocol content"
    else
        log_fail "OpenCode dxrk-memory install command failed"
    fi
}
...
test_oc_persona_dxrk() {
    log_test "OpenCode: persona injection (dxrk)"
    cleanup_test_env

    if $BINARY install --agent opencode --component persona --persona dxrk 2>&1; then
        local agents_md="$HOME/.config/opencode/AGENTS.md"
        assert_file_exists "$agents_md" "AGENTS.md exists"
        assert_file_contains "$agents_md" "Senior Architect" "Dxrk persona has 'Senior Architect'"
        assert_file_size_min "$agents_md" 200 "AGENTS.md has substantial content"
    else
        log_fail "OpenCode persona (dxrk) install command failed"
    fi
}
...
test_oc_skills_full() {
    log_test "OpenCode: skills injection (full-dxrk = 9 foundation skills)"
    cleanup_test_env
...
test_qwen_dxrk-memory_injection() {
    log_test "Qwen: dxrk-memory injection (settings.json)"
    cleanup_test_env

    if $BINARY install --agent qwen-code --component dxrk-memory --persona neutral 2>&1; then
        ...
        log_fail "Qwen dxrk-memory install command failed"
    fi
}

test_qwen_dxrk-memory_idempotency() {
    log_test "Qwen: dxrk-memory injection is idempotent"
    cleanup_test_env
    ...
    $BINARY install --agent qwen-code --component dxrk-memory --persona neutral > /dev/null 2>&1 || true
    if [ ! -f "$settings" ]; then
...
test_full_preset_claude_code() {
    log_test "Full-dxrk preset: Claude Code (all components coexist)"
    cleanup_test_env

    # full-dxrk has: dxrk-memory, sdd, skills, context7, persona, permissions, dxrk-guardian
    # Dxrk-Memory/Dxrk-Guardian need binary install (go install) — may fail but injection components
    ...
    if $BINARY install --agent claude-code --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-dxrk --persona dxrk 2>&1; then
        local claude_md="$HOME/.claude/CLAUDE.md"
        local settings="$HOME/.claude/settings.json"

        # CLAUDE.md should have all 3 sections coexisting
        assert_file_exists "$claude_md" "CLAUDE.md exists"
        assert_file_contains "$claude_md" "dxrk:sdd-orchestrator" "Has SDD section"
        assert_file_contains "$claude_md" "dxrk:persona" "Has persona section"
...
test_full_preset_opencode() {
    log_test "Full-dxrk preset: OpenCode (all components coexist)"
    cleanup_test_env

    if $BINARY install --agent opencode --component dxrk-memory --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-dxrk --persona dxrk 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
        local agents_md="$HOME/.config/opencode/AGENTS.md"
        ...
        # AGENTS.md for persona + dxrk-memory (SDD orchestrator is in opencode.json for OpenCode, NOT AGENTS.md)
        assert_file_exists "$agents_md" "AGENTS.md exists"
        assert_file_contains "$agents_md" "Senior Architect" "Dxrk persona"
        assert_file_contains "$agents_md" "dxrk:dxrk-memory-protocol" "AGENTS.md has dxrk-memory protocol"
        assert_no_duplicate_section "$agents_md" "dxrk-memory-protocol" "No duplicate dxrk-memory section in AGENTS.md"
        # SDD orchestrator for OpenCode lives in opencode.json as an agent definition (not AGENTS.md)
        assert_file_contains "$settings" '"dxrk-orchestrator"' "opencode.json has dxrk-orchestrator agent"
        ...
        assert_file_not_contains "$agents_md" "<!-- dxrk:sdd-orchestrator -->" "AGENTS.md has no SDD section marker (opencode uses json agent)"
...
test_minimal_preset_opencode_only_dxrk-memory_no_persona() {
    log_test "Minimal preset: OpenCode (dxrk-memory only, no persona side effect)"
    cleanup_test_env
    ...
    if $BINARY install --agent opencode --preset minimal --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" '"dxrk-memory"' "OpenCode has dxrk-memory MCP"

        # Minimal preset should NOT silently install persona.
        if [ -f "$agents_md" ]; then
            assert_file_not_contains "$agents_md" "dxrk:persona" "No persona marker in minimal preset"
...
test_minimal_preset_claude_only_dxrk-memory() {
    log_test "Minimal preset: Claude Code (only dxrk-memory, nothing else)"
    cleanup_test_env

    if $BINARY install --agent claude-code --preset minimal --persona neutral 2>&1; then
        # Dxrk-Memory should be installed (MCP + CLAUDE.md)
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "dxrk:dxrk-memory-protocol" "Dxrk-Memory protocol section"

        # SDD should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "dxrk:sdd-orchestrator" "No SDD in minimal"
        # Persona should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "dxrk:persona" "No persona in minimal"
...
test_ecosystem_both_agents() {
    ...
    if $BINARY install --agent claude-code --agent opencode --component sdd --component skills --component context7 --preset ecosystem-only --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "dxrk:sdd-orchestrator" "Claude has SDD"
...
test_content_claude_md_sections_substantial() {
    ...
    # Install SDD + persona + dxrk-memory (all inject into CLAUDE.md)
    $BINARY install --agent claude-code --component sdd --component persona --persona dxrk 2>&1 || true
    $BINARY install --agent claude-code --component dxrk-memory --persona dxrk 2>&1 || true
...
test_content_skills_are_real() {
    ...
    $BINARY install --agent claude-code --component skills --preset full-dxrk --persona neutral 2>&1 || true
...
test_content_mcp_json_valid() {
    ...
    $BINARY install --agent claude-code --component dxrk-memory --persona neutral 2>&1 || true
...
test_idempotent_dxrk-memory_claude() {
    log_test "Idempotency: dxrk-memory on Claude Code (no duplicate sections)"
    cleanup_test_env
    ...
    local claude_md="$HOME/.claude/CLAUDE.md"
    if [ -f "$claude_md" ]; then
        assert_no_duplicate_section "$claude_md" "dxrk-memory-protocol" "No duplicate dxrk-memory section after 2 runs"

        # Also check MCP JSON is identical
        local mcp_file="$HOME/.claude/mcp/dxrk-memory.json"
        if [ -f "$mcp_file" ]; then
            assert_valid_json "$mcp_file" "dxrk-memory.json still valid after 2 runs"
        fi
    else
        log_fail "CLAUDE.md not found"
    fi
...
test_gemini_dxrk-memory_tools_flag() {
    log_test "Gemini: dxrk-memory injection uses --tools=agent"
    cleanup_test_env

    if $BINARY install --agent gemini-cli --component dxrk-memory --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" '"dxrk-memory"' "Has dxrk-memory entry"
        assert_file_contains "$settings" '"--tools=agent"' "Dxrk-Memory args include --tools=agent"
        assert_valid_json "$settings" "settings.json is valid JSON"
    else
        log_fail "Gemini dxrk-memory install command failed"
    fi
}

# ─── Codex parity tests ───────────────────────────────────────────────────────

test_codex_dxrk-memory_injection() {
    log_test "Codex: dxrk-memory injection writes config.toml + instruction files"
    cleanup_test_env

    if $BINARY install --agent codex --component dxrk-memory --persona neutral 2>&1; then
        local config_toml="$HOME/.codex/config.toml"
        local instructions="$HOME/.codex/dxrk-memory-instructions.md"
        local compact="$HOME/.codex/dxrk-memory-compact-prompt.md"

        assert_file_exists "$config_toml" "Codex config.toml"
        assert_file_contains "$config_toml" '[mcp_servers.dxrk-memory]' "config.toml has [mcp_servers.dxrk-memory]"
        assert_file_contains "$config_toml" 'command = ".*dxrk-memory"' "config.toml has correct command"
        ...
        assert_file_exists "$instructions" "dxrk-memory-instructions.md"
        assert_file_contains "$instructions" 'mem_save' "Instructions have memory protocol content"

        assert_file_exists "$compact" "dxrk-memory-compact-prompt.md"
        assert_file_contains "$compact" 'FIRST ACTION REQUIRED' "Compact prompt has required sentinel"
    else
        log_fail "Codex dxrk-memory install command failed"
    fi
}

test_codex_dxrk-memory_idempotent() {
    log_test "Codex: dxrk-memory injection is idempotent (no duplicate blocks)"
    cleanup_test_env

    $BINARY install --agent codex --component dxrk-memory --persona neutral 2>&1 || true
    $BINARY install --agent codex --component dxrk-memory --persona neutral 2>&1 || true

    local config_toml="$HOME/.codex/config.toml"
    if [ -f "$config_toml" ]; then
        local count
        count=$(grep -c '\[mcp_servers\.dxrk-memory\]' "$config_toml" || true)
        if [ "$count" -ne 1 ]; then
            log_fail "config.toml has $count [mcp_servers.dxrk-memory] blocks after 2 runs (want exactly 1)"
        else
            log_pass "config.toml has exactly 1 [mcp_servers.dxrk-memory] block after 2 runs"
        fi
    else
...
test_idempotent_full_claude() {
    ...
    $BINARY install --agent claude-code --component sdd --component persona --component context7 --component permissions --component theme --preset full-dxrk --persona dxrk 2>&1 || true
    local first_md_hash
    first_md_hash=$(md5sum "$HOME/.claude/CLAUDE.md" 2>/dev/null | cut -d' ' -f1)
    # Snapshot settings.json for semantic comparison (dxrk-memory setup may reorder
    # top-level keys on re-run — see dxrk-memory binary's non-deterministic map
...
test_edge_persona_switch() {
    log_test "Edge case: switching persona from dxrk to neutral"
    cleanup_test_env

    # First install with dxrk (Rioplatense language present)
    $BINARY install --agent claude-code --component persona --persona dxrk 2>&1 || true
    assert_file_contains "$HOME/.claude/CLAUDE.md" "Senior Architect" "First install: dxrk persona"
...
test_edge_persona_switch_preserves_sections_opencode() {
    ...
    # Step 1: Install full stack with dxrk
    $BINARY install --agent opencode --component persona --component dxrk-memory --component sdd --persona dxrk 2>&1 || true

    local agents_md="$HOME/.config/opencode/AGENTS.md"
    assert_file_exists "$agents_md" "AGENTS.md after full install"
    assert_file_contains "$agents_md" "dxrk:dxrk-memory-protocol" "Dxrk-Memory section present before switch"
    ...
    assert_file_contains "$agents_md" "dxrk:dxrk-memory-protocol" "Dxrk-Memory section survived persona switch"
    assert_no_duplicate_section "$agents_md" "dxrk-memory-protocol" "No duplicate dxrk-memory after switch"
}
...
# --- Category: Dxrk-Guardian tests ---

test_dxrk-guardian_config() {
    log_test "Dxrk-Guardian component writes config file"
    cleanup_test_env

    # Dxrk-Guardian binary install may fail in Docker (go install needs time/network),
    # but we test the output regardless.
    if $BINARY install --agent claude-code --component dxrk-guardian --persona neutral 2>&1; then
        local config="$HOME/.config/dxrk-guardian/config"
        assert_file_exists "$config" "Dxrk-Guardian config"
        assert_file_contains "$config" 'PROVIDER=' "Has provider key"
        assert_file_contains "$config" 'FILE_PATTERNS=' "Has file patterns key"

        local agents_md="$HOME/.config/dxrk-guardian/AGENTS.md"
        assert_file_exists "$agents_md" "Dxrk-Guardian AGENTS.md template"
    else
        log_skip "Dxrk-Guardian install failed (expected — binary install may require network)"
    fi
}

test_dxrk-guardian_runtime_pr_mode_installed() {
    log_test "Dxrk-Guardian runtime includes pr_mode.sh"
    cleanup_test_env

    if $BINARY install --agent claude-code --component dxrk-guardian --persona neutral 2>&1; then
        local pr_mode="$HOME/.local/share/dxrk-guardian/lib/pr_mode.sh"
        assert_file_exists "$pr_mode" "Dxrk-Guardian pr_mode.sh exists"
...
test_dxrk-guardian_reinstall_is_idempotent() {
    log_test "Dxrk-Guardian install is idempotent on second run"
    cleanup_test_env

    if $BINARY install --agent claude-code --component dxrk-guardian --persona neutral 2>&1; then
        if $BINARY install --agent claude-code --component dxrk-guardian --persona neutral 2>&1; then
            log_pass "Second Dxrk-Guardian install completed successfully"
        else
            log_fail "Second Dxrk-Guardian install failed"
        fi
    else
        log_skip "Dxrk-Guardian first install failed (expected — binary install may require network)"
    fi
}
...
test_windsurf_persona_and_sdd_content() {
    ...
    if $BINARY install --agent windsurf --component persona --component sdd --persona dxrk 2>&1; then
        ...
        assert_file_contains "$rules" "dxrk:sdd-orchestrator" "SDD orchestrator marker present"
        assert_file_contains "$rules" "skill_resolution" "SDD has skill_resolution field"
        assert_file_contains "$rules" "Dxrk-Memory Topic Key" "SDD has Dxrk-Memory Topic Key section"
        assert_file_contains "$rules" "dxrk:dxrk-memory-protocol" "Dxrk-Memory protocol marker present"
...
test_codex_context7_not_in_toml() {
    ...
    # Install dxrk-memory first (creates config.toml) then try context7
    $BINARY install --agent codex --component dxrk-memory --persona neutral 2>&1 || true

    local config_toml="$HOME/.codex/config.toml"
    if [ -f "$config_toml" ]; then
        # Dxrk-Memory created config.toml — now try context7 on top
        ...
        # config.toml wasn't created by dxrk-memory, so no context7 either
        $BINARY install --agent codex --component context7 --persona neutral 2>&1 || true
...
test_integrity_sdd_orchestrator_in_opencode_json() {
    log_test "Integrity: opencode.json contains dxrk-orchestrator agent after SDD install"
    cleanup_test_env

    if $BINARY install --agent opencode --component sdd --persona neutral 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
        assert_file_exists "$settings" "opencode.json exists"
        assert_file_contains "$settings" '"dxrk-orchestrator"' "Has dxrk-orchestrator agent"
...
test_integrity_full_preset_all_skills_nonempty() {
    ...
    if $BINARY install --agent opencode --component sdd --component skills --preset full-dxrk --persona dxrk 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        local all_ok=true
        local empty_count=0
...
test_integrity_sdd_orchestrator_agent_structure() {
    log_test "Integrity: dxrk-orchestrator agent has required fields in opencode.json"
    cleanup_test_env

    if $BINARY install --agent opencode --component sdd --persona dxrk 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
        assert_file_contains "$settings" '"dxrk-orchestrator"' "Has dxrk-orchestrator"
...
test_integrity_skills_plus_sdd_coexist() {
    ...
    if $BINARY install --agent opencode --component sdd --component skills --preset full-dxrk --persona neutral 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        ...
        # opencode.json should have dxrk-orchestrator as the base coordinator
        assert_file_contains "$HOME/.config/opencode/opencode.json" '"dxrk-orchestrator"' "dxrk-orchestrator present"
...
test_backup_created_on_install() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local backup_count
        backup_count=$(find "$HOME/.dxrk/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
...
test_backup_contains_original_files() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local latest_backup
        latest_backup=$(find "$HOME/.dxrk/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -1)
...
if [ "${RUN_FULL_E2E:-0}" = "1" ]; then
    ...
    # Category 6b: Gemini/Codex dxrk-memory parity
    test_gemini_dxrk-memory_tools_flag
    test_codex_dxrk-memory_injection
    test_codex_dxrk-memory_idempotent
