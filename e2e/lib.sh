#!/usr/bin/env bash
# lib.sh — shared test helpers for dxrk E2E tests
...
# Binary resolution
# ---------------------------------------------------------------------------
# The binary should be built and placed at /usr/local/bin/dxrk inside
...
# This ensures `go build ./cmd/dxrk && bash e2e/e2e_test.sh` always
...
cleanup_test_env() {
    ...
    rm -rf "$HOME/.config/dxrk-guardian" 2>/dev/null || true
...
# setup_fake_dxrk-memory_binary — install a deterministic local dxrk-memory shim for E2E.
#
# Full Docker E2E validates dxrk's agent/config injection behavior, not the
# external Dxrk-Memory release CDN. The real installer skips the network download when
# an `dxrk-memory` binary already exists on PATH, so this shim keeps coverage of the
# install pipeline while avoiding flaky GitHub API/rate-limit failures.
#
# Set DXRK_E2E_REAL_DXRK_MEMORY=1 to opt out and exercise the live download path.
setup_fake_dxrk-memory_binary() {
    if [ "${DXRK_E2E_REAL_DXRK_MEMORY:-0}" = "1" ]; then
        log_info "Using real Dxrk-Memory binary/download path for E2E"
        return 0
    fi
...
case "${1:-}" in
  ...
  mcp)
    ...
    # require real Dxrk-Memory services or network access.
    exit 0
    ;;
  version|--version|-v)
    printf 'dxrk-memory e2e-shim\n'
    exit 0
    ;;
  *)
    exit 0
...
# Checks that the dxrk section marker appears exactly once (no duplicates).
