...
class OpenClawAdapter(Adapter):
    ...
    def global_config_dir(self, home_dir: str = "") -> str:
        return str(Path(home_dir) / ".openclaw")

    def system_prompt_dir(self, home_dir: str = "") -> str:
        return str(Path(home_dir))

    def system_prompt_file(self, home_dir: str = "") -> str:
        return str(Path(home_dir) / "AGENTS.md")

    def skills_dir(self, home_dir: str = "") -> str:
        return str(Path(home_dir) / ".openclaw" / "skills")

    def settings_path(self, home_dir: str = "") -> str:
        return str(Path(home_dir) / ".openclaw" / "openclaw.json")
