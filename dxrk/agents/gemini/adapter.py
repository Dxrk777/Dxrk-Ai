...
class GeminiCLIAdapter(Adapter):
    ...
    def install_command(self, profile) -> list[list[str]]:
        return [["npm", "install", "-g", "--ignore-scripts", "@google/gemini-cli"]]
    ...
    def skills_dir(self, home_dir: str = "") -> str:
        return str(Path(home_dir) / ".gemini" / "skills")

    def settings_path(self, home_dir: str = "") -> str:
        return str(Path(home_dir) / ".gemini" / "settings.json")
