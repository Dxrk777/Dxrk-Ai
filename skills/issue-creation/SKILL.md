---
name: dxrk-memory-issue-creation
description: >
  Issue creation workflow for Dxrk-Memory following the issue-first enforcement system.
...
4. **Questions go to [Discussions](https://github.com/Dxrk777/dxrk-memory/discussions)**, not issues
...
| **Dxrk-Memory Version** | Output of `dxrk-memory version` |
...
### Steps to Reproduce
1. Start two dxrk-memory instances pointing to the same DB
...
### Problem Description
When scripting with dxrk-memory, parsing the human-readable output of mem search is fragile. There's no machine-readable output format.

### Proposed Solution
Add a \`--json\` flag to \`dxrk-memory mem search\` that outputs results as JSON.

Example:
\`\`\`bash
dxrk-memory mem search \"auth middleware\" --json
\`\`\`
