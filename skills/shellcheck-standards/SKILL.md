# Skill: dxrk-guardian-shellcheck-standards

## Purpose
Maintain shell script quality and portability across all Dxrk-Guardian bash scripts.
...
shellcheck -x -e SC1090,SC1091,SC2162,SC2129 bin/dxrk-guardian lib/*.sh
```
...
| Code | Rule | Why Dxrk-Guardian Excludes It |
|------|------|---------------------|
| SC1090 | Can't follow non-constant source | Dxrk-Guardian uses dynamic paths for lib loading |
| SC1091 | Not following sourced file | Same — dynamic lib sourcing |
| SC2162 | read without -r | Dxrk-Guardian intentionally handles backslash input |
...
# Dxrk-Guardian pattern — detect and branch:
