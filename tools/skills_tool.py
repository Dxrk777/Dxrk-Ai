...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    for scan_dir in dirs_to_scan:
        for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
            ...
            try:
                ...
                # Register conditional skills (with paths frontmatter)
                skill_paths = frontmatter.get("paths")
                if skill_paths:
                    import shlex
                    paths_list = shlex.split(str(skill_paths)) if isinstance(skill_paths, str) else (
                        list(skill_paths) if isinstance(skill_paths, (list, tuple)) else []
                    )
                    if paths_list:
                        register_conditional_skills([{
                            "name": name,
    ...
    # Merge dynamically discovered skills (from .claude/skills/ near files)
    discovered = get_discovered_skills()
    seen_discovered = set()
    for s in discovered:
        if s["name"] not in seen_names:
            seen_names.add(s["name"])
            seen_discovered.add(s["name"])
            if s["name"] in disabled:
                continue
            skills.append(s)
    if discovered:
        logger.debug(
            "Merged %d discovered skills into listing",
            len(seen_discovered),
        )
