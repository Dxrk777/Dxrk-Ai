...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # Check for .claude/skills/ dirs near this file, activate conditional skills.
        try:
            discovered = discover_skills_for_paths([resolved_str])
            activated = activate_conditional_skills_for_paths([resolved_str])
            if discovered or activated:
                logger.debug(
                    "Skill discovery: %d new, %d activated via read",
                    len(discovered), len(activated),
                )
        except Exception:
            logger.debug("Skill discovery failed (non-fatal)", exc_info=True)
    ...
    try:
        ...
        with file_state.lock_path(_resolved):
            ...
            if not result_dict.get("error"):
                ...
                # Skill auto-discovery
                try:
                    discovered = discover_skills_for_paths([path])
                    activated = activate_conditional_skills_for_paths([path])
                    if discovered or activated:
                        logger.debug(
                            "Skill discovery: %d new, %d activated via write",
                            len(discovered), len(activated),
                        )
                except Exception:
                    logger.debug("Skill discovery failed (non-fatal)", exc_info=True)
    ...
    try:
        ...
        with ExitStack() as _locks:
            ...
            if not result_dict.get("error"):
                ...
                # Skill auto-discovery
                try:
                    discovered = discover_skills_for_paths(_paths_to_check)
                    activated = activate_conditional_skills_for_paths(_paths_to_check)
                    if discovered or activated:
                        logger.debug(
                            "Skill discovery: %d new, %d activated via patch",
                            len(discovered), len(activated),
                        )
                except Exception:
                    logger.debug("Skill discovery failed (non-fatal)", exc_info=True)
...
def search_tool(pattern: str, target: str = "content", path: str = ".",
    ...
    try:
        ...
        # Skill auto-discovery
        try:
            searched_paths = [path] if path and path != "." else []
            if searched_paths:
                discovered = discover_skills_for_paths(searched_paths)
                activated = activate_conditional_skills_for_paths(searched_paths)
                if discovered or activated:
                    logger.debug(
                        "Skill discovery: %d new, %d activated via search",
                        len(discovered), len(activated),
                    )
        except Exception:
            logger.debug("Skill discovery failed (non-fatal)", exc_info=True)
