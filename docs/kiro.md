...
This document explains how dxrk integrates with **Kiro IDE** and what is installed in your local Kiro configuration.

## Overview

dxrk supports Kiro as a **native-subagent** platform (`kiro-ide`).

When configured, dxrk installs:

| Artifact | Path |
|----------|------|
| Steering file | `~/.kiro/steering/dxrk.md` |
...
> **Auto-install not supported.** Kiro must be installed manually before running dxrk.
...
dxrk uses **two signals** to detect Kiro:
...
In practice: **the installer detects Kiro from `~/.kiro`**, not from `PATH`. If you have Kiro installed but `~/.kiro` hasn't been created yet (e.g., before first launch), run Kiro once to initialize its config dir, then re-run `dxrk install`.
...
This follows the same SDD architecture used in dxrk: orchestrator coordinates, phase agents execute, Dxrk-Memory persists artifacts across phases.
...
Kiro has a built-in spec workflow that dxrk leverages. For medium and large changes, the orchestrator will use native Kiro artifacts at:
...
| Medium | Kiro native specs (`.kiro/specs/`) + Dxrk-Memory |
...
The steering file written by dxrk uses the following frontmatter:
...
| Steering file | `%USERPROFILE%\.kiro\steering\dxrk.md` |
...
Kiro uses a **split-root layout** — dxrk managed files and IDE settings live in different directories:

- **Steering, skills, and native agents** → `~/.kiro/` (or `%USERPROFILE%\.kiro\` on Windows)
  - `~/.kiro/steering/dxrk.md` — orchestrator persona
...
If dxrk skills or steering are missing, check `~/.kiro/skills/` and `~/.kiro/steering/`.
