...
End-to-end tests that validate the `dxrk` installer binary inside Docker containers running real Linux distributions.
...
docker build -f e2e/Dockerfile.arch -t dxrk-e2e-arch .
docker run --rm -e RUN_FULL_E2E=1 dxrk-e2e-arch
...
4. Build the `dxrk` binary for `linux/amd64`
