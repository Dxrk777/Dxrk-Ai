...
| Persistent memory wiring | Does Dxrk-Memory MCP setup stay stable across terminals and IDE-launched agents? |
...
- **Do not touch local runtime state**: `.dxrk-memory/dxrk-memory.db`, `.dxrk-memory/cloud.json`, and accidental `main` files are not documentation inputs.
- **Do not invent external internals**: dashboard, cloud, and Dxrk-Memory store implementation must be backed by source or linked external docs.
...
- **Do not duplicate references**: link to [Usage](../usage.md), [Dxrk-Memory Commands](../dxrk-memory.md), or specialized docs for detailed command/API behavior.
