...
Dxrk integration code should stay thin: adapters describe where and how an agent accepts configuration; components decide what managed content to inject.
...
| Dxrk-Memory MCP | `internal/components/dxrk-memory/` | Add external Dxrk-Memory MCP server entries. |
...
| Boundary | Rule |
|---|---|
| Binary installation | Install only external tools this component owns, such as Dxrk-Memory or Dxrk-Guardian. |
...
OpenCode community plugins are optional integrations. Dxrk only ensures `~/.config/opencode/tui.json` exists and contains the plugin package name. It does not vendor, execute, or own plugin runtime code.
