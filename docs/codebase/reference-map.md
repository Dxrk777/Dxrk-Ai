...
| `docs/dxrk-memory.md` | Dxrk-Memory command and MCP tool reference. |
...
|---|---|
| `cmd/dxrk/main.go` | Binary entrypoint and version handoff. |
...
| `internal/components/dxrk-memory/` | Dxrk-Memory install, setup, MCP injection, and verification wiring. |
...
| `internal/state/state.go` | Persisted install state in `~/.dxrk/state.json`. |
...
| Dxrk-Memory store schema | Sessions, observations, prompts, relations, and sync mutations are external Dxrk-Memory runtime concepts, not Dxrk source files. |
| Full API reference | No `DOCS.md` exists in this repository; use existing focused docs and external Dxrk-Memory docs when needed. |
