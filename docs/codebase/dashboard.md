...
Dxrk does not currently contain dashboard source code. This page exists to prevent accidental invention of dashboard, HTMX, auth, or admin behavior while documenting the codebase.
...
| Dxrk-Memory memory browser command mention | `docs/dxrk-memory.md` documents `dxrk-memory tui`; implementation is external. |
...
- **No direct local DB assumptions**: do not read `.dxrk-memory/dxrk-memory.db` from Dxrk dashboard code unless the architecture explicitly chooses that boundary.
