...
Dxrk exposes a CLI and TUI. It configures MCP for agents. It does not expose a local HTTP API in this repository.

## Interface map

| Interface | Status in this repo | Primary files | Read for details |
|---|---|---|---|
| CLI | Implemented | `cmd/dxrk/main.go`, `internal/app/`, `internal/cli/` | [Usage](../usage.md) |
| TUI | Implemented | `internal/tui/model.go`, `internal/tui/router.go`, `internal/tui/screens/` | [Usage](../usage.md) |
| MCP | Configured, not hosted by Dxrk | `internal/components/dxrk-memory/`, `internal/components/mcp/` | [Dxrk-Memory Commands](../dxrk-memory.md) |
| Local HTTP API | Not present in this source tree | No dashboard/server package found | Use external Dxrk-Memory docs if needed |
...
Dxrk writes agent-specific MCP configuration so agents can call external servers. Dxrk-Memory configuration usually invokes:

```text
dxrk-memory mcp --tools=agent
```
