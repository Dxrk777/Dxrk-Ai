...
This guide helps maintainers find the right code path before changing Dxrk. It is an index, not a full API reference.
...
Dxrk is a Go CLI/TUI that configures AI coding agents. It installs and syncs managed assets such as SDD prompts, skills, MCP entries, permissions, personas, Dxrk-Guardian support, and Dxrk-Memory wiring.

```text
User
  |
  v
dxrk CLI / Bubbletea TUI
  |
...
Agent config roots (~/.claude, ~/.config/opencode, ~/.cursor, ...)
  |
  v
External tools and agents: Dxrk-Memory, Context7, Dxrk-Guardian, supported AI CLIs/IDEs
```
...
| [Memory core](codebase/memory-core.md) | Understand the Dxrk-Memory boundary and what this repo wires vs owns. |
| [Interfaces](codebase/interfaces.md) | Compare CLI, MCP, local HTTP API boundary, and TUI surfaces. |
| [Sync and cloud](codebase/sync-and-cloud.md) | Separate Dxrk config sync from Dxrk-Memory memory/cloud sync. |
...
- [Dxrk-Memory Commands](dxrk-memory.md) — Dxrk-Memory user commands and MCP tool overview.
