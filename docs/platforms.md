...
- **Scoop** is the supported Windows install path for Dxrk AI.
...
- **Dxrk-Guardian on Windows** works from both Git Bash and PowerShell. dxrk installs a `dxrk-guardian.ps1` shim that automatically delegates to Git Bash, so no manual shell switching is required.
...
| Pi | `%USERPROFILE%\.pi\` (Pi config, project agents/chains, Dxrk AI support assets) |
