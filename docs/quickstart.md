...
- Scoop installed. Dxrk AI recommends Scoop as the Windows install path.
...
For a Pi-only install, the plan shows the Pi package stack instead of Dxrk AI components. It installs `dxrk-pi`, `dxrk-dxrk-memory`, and `pi-mcp-adapter`, runs `pi-dxrk-memory init` through the pinned `dxrk-dxrk-memory` package, then installs `pi-subagents`, `pi-intercom`, `@juicesharp/rpiv-ask-user-question`, `pi-web-access`, `pi-lens`, `@juicesharp/rpiv-todo`, and `pi-btw`.

## Hardening recommendations for users

Dxrk AI pins versions and disables postinstall scripts on every npm install it generates. For broader protection across npm packages you install yourself, set these once on your machine:
