...
import pytest
from dxrk.models import AgentID, ComponentID, PersonaID, PresetID, SDDModeID, SkillID, MCPStrategy
...
class TestParseSyncFlags:
    ...
    def test_profile_phase_flag(self):
        from dxrk.cli.install import parse_sync_flags
        flags = parse_sync_flags([
            "--profile", "my-profile:anthropic/claude-3-opus",
            "--profile-phase", "my-profile:sdd-propose:anthropic/claude-3-haiku",
        ])
        assert len(flags.profiles) == 1
        assert flags.profiles[0]["name"] == "my-profile"
...
class TestNormalizeComponents:
    def test_empty_uses_preset(self, monkeypatch):
        ...
        monkeypatch.setattr(dxrk.components.assets, 'mvp_components', lambda: [ComponentID.ENGRAM], raising=False)
    ...
    def test_custom_list(self, monkeypatch):
        import dxrk.components.assets
        monkeypatch.setattr(dxrk.components.assets, 'mvp_components', lambda: [ComponentID.ENGRAM, ComponentID.SDD], raising=False)
        from dxrk.cli.install import normalize_components
        result = normalize_components(["engram", "sdd"], PresetID.CUSTOM)
        assert result == [ComponentID.ENGRAM, ComponentID.SDD]

    def test_duplicates_removed(self, monkeypatch):
        import dxrk.components.assets
        monkeypatch.setattr(dxrk.components.assets, 'mvp_components', lambda: [ComponentID.ENGRAM], raising=False)
        from dxrk.cli.install import normalize_components
        result = normalize_components(["engram", "engram"], PresetID.CUSTOM)
        assert result == [ComponentID.ENGRAM]
...
class TestNormalizeSkills:
    ...
    def test_valid_skills(self, monkeypatch):
        import dxrk.components.assets
        monkeypatch.setattr(dxrk.components.assets, 'mvp_skills', lambda: [SkillID.GO_TESTING, SkillID.SKILL_CREATOR], raising=False)
        from dxrk.cli.install import normalize_skills
    ...
    def test_unsupported_raises(self, monkeypatch):
        import dxrk.components.assets
        monkeypatch.setattr(dxrk.components.assets, 'mvp_skills', lambda: [SkillID.GO_TESTING], raising=False)
        from dxrk.cli.install import normalize_skills
...
class TestNormalizeInstallFlags:
    def test_minimal_preset(self, tmp_path):
        ...
        assert result.selection.components == [ComponentID.ENGRAM]
...
class TestParseProfilePhaseFlag:
    def test_valid(self):
        ...
        name, phase, assignment = _parse_profile_phase_flag("my-profile:sdd-propose:anthropic/claude-3-haiku")
