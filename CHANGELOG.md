...
### Added
- OpenAlex provider for scholar search: search works and fetch by DOI, with abstract reconstruction from the inverted index
- PubMed provider for scholar search: search and fetch by DOI via NCBI E-utilities (esearch + esummary)
- Scholar search tool now supports source filtering for `openalex` and `pubmed`
